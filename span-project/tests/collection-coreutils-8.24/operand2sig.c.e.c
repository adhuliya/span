...
operand2sig (char const *operand, char *signame)
{
  ...
  if (((unsigned int) (*operand) - '0' <= 9))
    {
      char *endp;
      long int l = ((*__errno_location ()) = 0, strtol (operand, &endp, 10));
      int i = l;
      signum = (operand == endp || *endp || (*__errno_location ()) || i != l ? -1
                : (((signed char) (((i) & 0x7f) + 1) >> 1) > 0) ? ((i) & 0x7f) : i);
    }
  else
    {
  ...
  if (signum < 0 || sig2str (signum, signame) != 0)
    {
      error (0, 0, gettext ("%s: invalid signal"), operand);
      return -1;
    }
