              ...
              rel_mode_t rel_mode)
{
  ...
  if ((block_mode || (rel_mode && rsize < 0)) && fstat (fd, &sb) != 0)
    {
      error (0, (*__errno_location ()), gettext ("cannot fstat %s"), quote (fname));
      return 0;
    }
  if (block_mode)
    {
      off_t const blksize = ((0 < (sb).st_blksize && (sb).st_blksize <= ((size_t)-1) / 8 + 1) ? (sb).st_blksize : 512);
      if (ssize < ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) 0 : ((off_t) ~ (off_t) 0 < (off_t) -1) ? ~ (off_t) 0 : ~ ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))))) / blksize || ssize > ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) / blksize)
        {
          error (0, 0,
  ...
  if (rel_mode)
    {
      ...
      if (rel_mode == rm_min)
        nsize = (((fsize)>((uintmax_t) ssize))?(fsize):((uintmax_t) ssize));
...

        {
          ...
          if (overflow > ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))))
            {
              error (0, 0, gettext ("overflow rounding up size of file %s"),
                     quote (fname));
              return 0;
            }
          nsize = overflow;
        ...
        {
          if (ssize > ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - (off_t)fsize)
            {
  ...
  if (ftruncate (fd, nsize) == -1)
    {
      error (0, (*__errno_location ()),
             gettext ("failed to truncate %s at %" "l" "d" " bytes"), quote (fname),
             (intmax_t) nsize);
      return 0;
    }
  ...
  while ((c = getopt_long (argc, argv, "cor:s:", longopts, ((void*)0))) != -1)
    {
      switch (c)
        ...
        case 's':
          ...
          size = xdectoimax (optarg, ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) 0 : ((off_t) ~ (off_t) 0 < (off_t) -1) ? ~ (off_t) 0 : ~ ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))))), ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))), "EgGkKmMPtTYZ0",
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "truncate", "GNU coreutils", Version, proper_name_utf8 ("Padraig Brady", "P\303\241draig Brady"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (ref_file)
    {
      ...
      if (stat (ref_file, &sb) != 0)
        error (1, (*__errno_location ()), gettext ("cannot stat %s"), quote (ref_file));
      if (usable_st_size (&sb))
        file_size = sb.st_size;
      else
        {
      ...
      if (file_size < 0)
        error (1, (*__errno_location ()), gettext ("cannot get the size of %s"),
               quote (ref_file));
      if (!got_size)
        size = file_size;
      else
      ...
      if (fd != -1)
        {
          ...
          if (close (fd) != 0)
            {
              error (0, (*__errno_location ()), gettext ("failed to close %s"), quote (fname));
              errors = 1;
            }
        }
    }
