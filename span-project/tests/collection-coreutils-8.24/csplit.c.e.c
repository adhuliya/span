  ...
  while (1)
    {
      ...
      if (((size_t) (sizeof (ptrdiff_t) <= sizeof (size_t) ? -1 : -2) / (b->bytes_alloc) < (2)))
        xalloc_die ();
...
find_line (uintmax_t linenum)
{
  ...
  for (b = head;;)
    {
      ((void) sizeof ((b) ? 1 : 0), __extension__ ({ if (b) ; else __assert_fail ("b", "csplit.c", 617, __extension__ __PRETTY_FUNCTION__); }));
...
set_input_file (const char *name)
{
  if (! (strcmp (name, "-") == 0) && fd_reopen (0, name, 00, 0) < 0)
    error (1, (*__errno_location ()), gettext ("cannot open %s for reading"), quote (name));
}
...
write_to_file (uintmax_t last_line, _Bool ignore, int argnum)
{
  ...
  if (first_line > last_line)
    {
      error (0, 0, gettext ("%s: line number out of range"), global_argv[argnum]);
      cleanup_fatal ();
    }
  ...
  if (output_stream)
    {
      if (ferror_unlocked (output_stream))
        {
          error (0, 0, gettext ("write error for %s"), quote (output_filename));
          output_stream = ((void*)0);
          cleanup_fatal ();
        }
      if (rpl_fclose (output_stream) != 0)
        {
          error (0, (*__errno_location ()), "%s", output_filename);
          output_stream = ((void*)0);
          cleanup_fatal ();
        }
...
{
  fwrite_unlocked (line->str,sizeof (char),line->len,output_stream);
...
new_control_record (void)
{
  ...
  if (control_used == control_allocated)
    controls = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(controls) != 1) ? 1 : -1; })), x2nrealloc (controls, &control_allocated, sizeof *(controls)));
...
check_for_offset (struct control *p, const char *str, const char *num)
{
  if (xstrtoimax (num, ((void*)0), 10, &p->offset, "") != LONGINT_OK)
    error (1, 0, gettext ("%s: integer expected after delimiter"), str);
}
...
parse_repeat_count (int argnum, struct control *p, char *str)
{
  ...
  if (*end != '}')
    error (1, 0, gettext ("%s: '}' is required in repeat count"), str);
  *end = '\0';
  ...
  if (str+1 == end-1 && *(str+1) == '*')
  ...
  else
    {
      if (xstrtoumax (str + 1, ((void*)0), 10, &val, "") != LONGINT_OK)
        {
          error (1, 0,
                 gettext ("%s}: integer required between '{' and '}'"),
                 global_argv[argnum]);
        }
      p->repeat = val;
    }
...
extract_regexp (int argnum, _Bool ignore, char const *str)
{
  ...
  if (closing_delim == ((void*)0))
    error (1, 0,
           gettext ("%s: closing delimiter '%c' missing"), str, delim);
  ...
  if (err)
    {
      error (0, 0, gettext ("%s: invalid regular expression: %s"), str, err);
      cleanup_fatal ();
    }
      ...
      if (*argv[i] == '/' || *argv[i] == '%')
      ...
      else
        {
          ...
          if (xstrtoumax (argv[i], ((void*)0), 10, &val, "") != LONGINT_OK)
            error (1, 0, gettext ("%s: invalid pattern"), argv[i]);
          if (val == 0)
            error (1, 0,
                   gettext ("%s: line number must be greater than zero"),
                   argv[i]);
          if (val < last_val)
            {
              char buf[(((((sizeof (uintmax_t) * 8 - (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1))) + 1)];
              error (1, 0,
               gettext ("line number %s is smaller than preceding line number, %s"),
...
check_format_conv_type (char *format, int flags)
{
  ...
  switch (ch)
    ...
    case 0:
      error (1, 0, gettext ("missing conversion specifier in suffix"));
      break;

    default:
      if (((*__ctype_b_loc ())[(int) ((ch))] & (unsigned short int) _ISprint))
        error (1, 0,
               gettext ("invalid conversion specifier in suffix: %c"), ch);
      else
        error (1, 0,
               gettext ("invalid conversion specifier in suffix: \\%.3o"), ch);
    }

  if (flags & ~ compatible_flags)
    error (1, 0,
           gettext ("invalid flags in conversion specification: %%%c%c"),
...
max_out (char *format)
{
    ...
    if (*f == '%' && *++f != '%')
      {
        if (percent)
          error (1, 0,
                 gettext ("too many %% conversion specifications in suffix"));
        percent = 1;
        int flags;
  ...
  if (! percent)
    error (1, 0,
           gettext ("missing %% conversion specification in suffix"));
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -m, --suppress-matched     suppress the lines matching PATTERN\n"),stdout);
