...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (sparse_type_string) / sizeof *(sparse_type_string)) == (sizeof (sparse_type) / sizeof *(sparse_type)) + 1) ? 1 : -1; }))];
...
extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (reflink_type_string) / sizeof *(reflink_type_string)) == (sizeof (reflink_type) / sizeof *(reflink_type)) + 1) ? 1 : -1; }))];
  ...
  for (p = attr_list; p; p = p->next)
    {
      ...
      if (x->preserve_mode)
        ...
        {
          if (chmod (dst_name, p->st.st_mode) != 0)
            {
                          ...
                          const struct cp_options *x)
{
  ...
  *attr_list = ((void*)0);
  ...
  if (stat (dst_dir, &stats) != 0)
          ...
          if (missing_dir)
            {
              ...
              *new_dst = 1;
              ...
              mkdir_mode = src_mode & (04000 | 02000 | 01000 | (0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3)) & ~omitted_permissions;
              ...
              if ((stats.st_mode & (0400|0200|0100)) != (0400|0200|0100))
                {
                  ...
                  if (chmod (dir, stats.st_mode | (0400|0200|0100)) != 0)
                    {
    ...
    {
      error (0, 0, gettext ("%s exists but is not a directory"), quote (dst_dir));
      return 0;
    }
  else
    {
...
# 569 "cp.c"
static _Bool
target_directory_operand (char const *file, struct stat *st, _Bool *new_dst)
{
  ...
  if (no_target_directory)
    {
      ...
      (__extension__ ({ __typeof__ (target_directory_operand (file[n_files -1], &sb, &new_dst)) __x = (target_directory_operand (file[n_files -1], &sb, &new_dst)); (void) __x; }));
    }
      ...
      for (i = 0; i < n_files; i++)
        {
          ...
          if (parents_option)
          ...
          else
            {
              ...
              do { char *tmp_abns_; do { tmp_abns_ = (__extension__ ({ const char *__old = ((arg)); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0); arg_base = last_component (tmp_abns_); strip_trailing_slashes (arg_base); } while (0);
...
decode_preserve_arg (char const *arg, struct cp_options *x, _Bool on_off)
{
  ...
  extern int (*_gl_verify_function4 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (preserve_args) / sizeof *(preserve_args)) == (sizeof (preserve_vals) / sizeof *(preserve_vals)) + 1) ? 1 : -1; }))];
  ...
  do
    {
      ...
      val = ((preserve_vals) [__xargmatch_internal (on_off ? "--preserve" : "--no-preserve", s, preserve_args, (char const *) (preserve_vals), sizeof *(preserve_vals), argmatch_die)]);
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "abdfHilLnprst:uvxPRS:TZ",
         ...
         != -1)
    {
      switch (c)
        {
        case SPARSE_OPTION:
          x.sparse_mode = ((sparse_type) [__xargmatch_internal ("--sparse", optarg, sparse_type_string, (char const *) (sparse_type), sizeof *(sparse_type), argmatch_die)]);
        ...
        case REFLINK_OPTION:
          if (optarg == ((void*)0))
            ...
            x.reflink_mode = ((reflink_type) [__xargmatch_internal ("--reflink", optarg, reflink_type_string, (char const *) (reflink_type), sizeof *(reflink_type), argmatch_die)]);
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "cp", "GNU coreutils", Version, proper_name_utf8 ("Torbjorn Granlund", "Torbj\303\266rn Granlund"), ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (x.require_preserve_context && ! selinux_enabled)
    error (1, 0,
           gettext ("cannot preserve security context " "without an SELinux-enabled kernel"));
# 1199 "cp.c"
  ...
  if (x.require_preserve_xattr)
    error (1, 0, gettext ("cannot preserve extended attributes, cp is " "built without xattr support"));
