...
writeline (struct linebuffer const *line, FILE *stream, int class)
{
  switch (class)
    ...
    case 2:
      ...
      if (only_file_1)
        fputs_unlocked (delimiter,stream);
      break;
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      if (!streams[i])
        error (1, (*__errno_location ()), "%s", infiles[i]);
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "123", long_options, ((void*)0))) != -1)
    switch (c)
      {
      case '1':
        only_file_1 = 0;
      ...
      case OUTPUT_DELIMITER_OPTION:
        if (delimiter && !(strcmp (delimiter, optarg) == 0))
          error (1, 0, gettext ("multiple delimiters specified"));
        delimiter = optarg;
        if (!*delimiter)
          {
            error (1, 0, gettext ("empty %s not allowed"),
