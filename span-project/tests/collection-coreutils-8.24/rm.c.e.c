...
{
  {"force", 0, ((void*)0), 'f'},
  {"interactive", 2, ((void*)0), INTERACTIVE_OPTION},

  {"one-file-system", 0, ((void*)0), ONE_FILE_SYSTEM},
...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (interactive_args) / sizeof *(interactive_args)) == (sizeof (interactive_types) / sizeof *(interactive_types)) + 1) ? 1 : -1; }))];
  ...
  for (i = 1; i < argc; i++)
    {
      ...
      if (arg[0] == '-' && arg[1] && lstat (arg, &st) == 0)
        {
          fprintf (stderr,
                   gettext ("Try '%s ./%s' to remove the file %s.\n"),
                   argv[0],
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Remove (unlink) the FILE(s).\n\n  -f, --force           ignore nonexistent files and arguments, never prompt\n  -i                    prompt before every removal\n"),stdout);





      fputs_unlocked (gettext ("  -I                    prompt once before removing more than three files, or\n                          when removing recursively; less intrusive than -i,\n                          while still giving protection against most mistakes\n      --interactive[=WHEN]  prompt according to WHEN: never, once (-I), or\n                          always (-i); without WHEN, prompt always\n"),stdout);
      ...
      fputs_unlocked (gettext ("      --one-file-system  when removing a hierarchy recursively, skip any\n                          directory that is on a file system different from\n                          that of the corresponding command line argument\n"),stdout);




      fputs_unlocked (gettext ("      --no-preserve-root  do not treat '/' specially\n      --preserve-root   do not remove '/' (default)\n  -r, -R, --recursive   remove directories and their contents recursively\n  -d, --dir             remove empty directories\n  -v, --verbose         explain what is being done\n"),stdout);
...
rm_option_init (struct rm_options *x)
{
  ...
  x->require_restore_cwd = 0;
}

int
main (int argc, char **argv)
{
  _Bool preserve_root = 1;
  struct rm_options x;
  _Bool prompt_once = 0;
  int c;
  ...
  while ((c = getopt_long (argc, argv, "dfirvIR", long_opts, ((void*)0))) != -1)
    {
      switch (c)
        ...
        case INTERACTIVE_OPTION:
          {
            int i;
            if (optarg)
              i = ((interactive_types) [__xargmatch_internal ("--interactive", optarg, interactive_args, (char const *) (interactive_types), sizeof *(interactive_types), argmatch_die)]);
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "rm", "GNU coreutils", Version, ("Paul Rubin"), ("David MacKenzie"), ("Richard M. Stallman"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
  ...
  if (prompt_once && (x.recursive || 3 < n_files))
    {
      fprintf (stderr,
               (x.recursive
                ? ngettext ("%s: remove %""l" "u"" argument recursively? ",
                            "%s: remove %""l" "u"" arguments recursively? ",
                            select_plural (n_files))
                : ngettext ("%s: remove %""l" "u"" argument? ",
                            "%s: remove %""l" "u"" arguments? ",
                            select_plural (n_files))),
  ...
  ((void) sizeof ((((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (status)", "rm.c", 354, __extension__ __PRETTY_FUNCTION__); }));
