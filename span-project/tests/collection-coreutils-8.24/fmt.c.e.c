...
{
  typedef uintptr_t unused_word_type;
  void const *vp;
  char const *cbuf = buf;
  unused_word_type const *wp = buf;
...
static _Bool get_paragraph (FILE *f);
...
{
  {"crown-margin", 0, ((void*)0), 'c'},
  {"prefix", 1, ((void*)0), 'p'},
  {"split-only", 0, ((void*)0), 's'},
  {"tagged-paragraph", 0, ((void*)0), 't'},
  {"uniform-spacing", 0, ((void*)0), 'u'},
  {"width", 1, ((void*)0), 'w'},
  {"goal", 1, ((void*)0), 'g'},
...
main (int argc, char **argv)
{
  int optchar;
  _Bool ok = 1;
  char const *max_width_option = ((void*)0);
  char const *goal_width_option = ((void*)0);
  ...
  while ((optchar = getopt_long (argc, argv, "0123456789cstuw:p:g:",
    ...
    switch (optchar)
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "fmt", "GNU coreutils", Version, ("Ross Paterson"), (char *) ((void*)0)); exit (0); break;;
...
static void
set_other_indent (_Bool same_paragraph)
{
  if (split)
...
get_paragraph (FILE *f)
{
  ...
  ((void) sizeof ((unused_word_type < word_limit) ? 1 : 0), __extension__ ({ if (unused_word_type < word_limit) ; else __assert_fail ("word < word_limit", "fmt.c", 607, __extension__ __PRETTY_FUNCTION__); }));

  (word_limit - 1)->period = (word_limit - 1)->final = 1;
  next_char = c;
  return 1;
}
  ...
  do
    {
      ...
      while (c != (-1) && !((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISspace));
...
check_punctuation (WORD *w)
{
  ...
  w->paren = (strchr ("(['`\"", *start) != ((void*)0));
  w->punct = !! ((*__ctype_b_loc ())[(int) ((fin))] & (unsigned short int) _ISpunct);
  while (start < finish && (strchr (")]'\"", *finish) != ((void*)0)))
    finish--;
  w->period = (strchr (".?!", *finish) != ((void*)0));
}
...
flush_paragraph (void)
{
  ...
  if (word_limit == unused_word_type)
    {
      fwrite_unlocked (parabuf,sizeof *parabuf,wptr - parabuf,stdout);
      wptr = parabuf;
      return;
    }
