# 1 "find-mount-point.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 341 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "find-mount-point.c" 2
# 17 "find-mount-point.c"
# 1 "../lib/config.h" 1
# 18 "find-mount-point.c" 2
# 1 "../lib/sys/types.h" 1
...
find_mount_point (char const *file, struct stat const *file_stat)
{
  ...
  if (((((file_stat->st_mode)) & 0170000) == (0040000)))

    {
      ...
      if (chdir (file) < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot change to directory %s"), quote (file));
          return ((void*)0);
        }
    }
  else

    {
      ...
      char *dir;
      do { dir = (__extension__ ({ const char *__old = (xdir); size_t __len = strlen (__old) + 1; char *__new = (char *) __builtin_alloca (__len); (char *) memcpy (__new, __old, __len); })); } while (0);
      free (xdir);

      if (chdir (dir) < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot change to directory %s"), quote (dir));
          return ((void*)0);
        }

      if (stat (".", &last_stat) < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot stat current directory (now %s)"),
                 quote (dir));
          goto done;
        }
    }
  ...
  while (1)
    {
      ...
      if (stat ("..", &st) < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot stat %s"), quote (".."));
          goto done;
        }
      ...
      if (chdir ("..") < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot change to directory %s"), quote (".."));
          goto done;
        }
      last_stat = st;
    }
