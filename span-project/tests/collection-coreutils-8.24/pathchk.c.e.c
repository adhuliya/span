...
# 27 "pathchk.c" 2
# 65 "pathchk.c"
static _Bool validate_file_name (char *, _Bool, _Bool);



enum
{
  PORTABILITY_OPTION = 127 + 1
};

static struct option const longopts[] =
{
  {"portability", 0, ((void*)0), PORTABILITY_OPTION},
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Diagnose invalid or unportable file names.\n\n  -p                  check for most POSIX systems\n  -P                  check for empty names and leading \"-\"\n      --portability   check for all POSIX systems (equivalent to -p -P)\n"),stdout);
...
{
  _Bool ok = 1;
  _Bool check_basic_portability = 0;
  _Bool check_extra_portability = 0;
  int optc;
  ...
  while ((optc = getopt_long (argc, argv, "+pP", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case PORTABILITY_OPTION:
          check_basic_portability = 1;
          check_extra_portability = 1;
          break;
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "pathchk", "GNU coreutils", Version, ("Paul Eggert"), ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
      ...
      || (! file_exists && 256 <= filelen))
    {
      ...
      if (check_basic_portability)
      ...
      else
        {
          ...
          maxsize = (((size)<(9223372036854775807L))?(size):(9223372036854775807L));
        }
      ...
      for (start = file; *(start = component_start (start)); )
        {
          ...
          if (known_name_max)
          ...
          else
            {
              ...
              *start = c;
              if (0 <= len)
                name_max = (((len)<(9223372036854775807L))?(len):(9223372036854775807L));
              else
                switch ((*__errno_location ()))
                  ...
                  default:
                    *start = '\0';
                    error (0, (*__errno_location ()), "%s", dir);
                    *start = c;
                    return 0;
                  }
            }
