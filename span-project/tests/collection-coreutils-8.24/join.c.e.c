...
extract_field (struct line *line, char *field, size_t len)
{
  if (line->nfields >= line->nfields_allocated)
    {
      line->fields = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(line->fields) != 1) ? 1 : -1; })), x2nrealloc (line->fields, &line->nfields_allocated, sizeof *(line->fields)));
    }
...
xfields (struct line *line)
{
  ...
  if (0 <= tab && tab != '\n')
  ...
  else if (tab < 0)
    {
      ...
      while (((*__ctype_b_loc ())[(int) ((to_uchar (*ptr)))] & (unsigned short int) _ISblank))
        if (++ptr == lim)
          return;
      ...
      do
        {
          char *sep;
          for (sep = ptr + 1; sep != lim && ! ((*__ctype_b_loc ())[(int) ((to_uchar (*sep)))] & (unsigned short int) _ISblank); sep++)
            continue;
          extract_field (line, ptr, sep - ptr);
          if (sep == lim)
            return;
          for (ptr = sep + 1; ptr != lim && ((*__ctype_b_loc ())[(int) ((to_uchar (*ptr)))] & (unsigned short int) _ISblank); ptr++)
            continue;
        }
      while (ptr != lim);
    }
...
free_spareline (void)
{
  ...
  for (i = 0; i < (sizeof (spareline) / sizeof *(spareline)); i++)
    {
      if (spareline[i])
        {
...
getseq (FILE *fp, struct seq *seq, int whichfile)
{
  if (seq->count == seq->alloc)
    {
      size_t i;
      seq->lines = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(seq->lines) != 1) ? 1 : -1; })), x2nrealloc (seq->lines, &seq->alloc, sizeof *(seq->lines)));
...
add_field (int file, size_t field)
{
  ...
  ((void) sizeof ((file == 0 || file == 1 || file == 2) ? 1 : 0), __extension__ ({ if (file == 0 || file == 1 || file == 2) ; else __assert_fail ("file == 0 || file == 1 || file == 2", "join.c", 823, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((file != 0 || field == 0) ? 1 : 0), __extension__ ({ if (file != 0 || field == 0) ; else __assert_fail ("file != 0 || field == 0", "join.c", 824, __extension__ __PRETTY_FUNCTION__); }));
...
string_to_join_field (char const *str)
{
  ...
  extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((18446744073709551615UL) <= (9223372036854775807L *2UL+1UL)) ? 1 : -1; }))];

  strtol_error s_err = xstrtoul (str, ((void*)0), 10, &val, "");
...
decode_field_spec (const char *s, int *file_index, size_t *field_index)
{
  ...
  switch (s[0])
      ...
      if (s[1])
        {
          ...
          error (1, 0, gettext ("invalid field specifier: %s"), quote (s));
        }
      *file_index = 0;
      *field_index = 0;
      break;
    ...
    default:
      error (1, 0,
             gettext ("invalid file number in field spec: %s"), quote (s));
  ...
  if (*var != (18446744073709551615UL) && *var != val)
    {
      ...
      error (1, 0, gettext ("incompatible join fields %lu, %lu"),
             var1, val1);
    }
  *var = val;
}
  ...
  if (n == 2)
    {
      ...
      switch (operand_status[op0])
        ...
        case MUST_BE_OPERAND:
          error (0, 0, gettext ("extra operand %s"), quote (name));
          usage (1);
...
main (int argc, char **argv)
{
        ...
        case 'a':
          {
            ...
            if (xstrtoul (optarg, ((void*)0), 10, &val, "") != LONGINT_OK
                || (val != 1 && val != 2))
              error (1, 0,
                     gettext ("invalid field number: %s"), quote (optarg));
            if (val == 1)
  ...
  if (!fp1)
    error (1, (*__errno_location ()), "%s", g_names[0]);
  fp2 = (strcmp (g_names[1], "-") == 0) ? stdin : fopen_safer (g_names[1], "r");
  if (!fp2)
    error (1, (*__errno_location ()), "%s", g_names[1]);
  if (fp1 == fp2)
    error (1, (*__errno_location ()), gettext ("both files cannot be standard input"));
  system_join (fp1, fp2);
