...
verify_numeric (const char *s, const char *end)
{
  if ((*__errno_location ()))
    {
      error (0, (*__errno_location ()), "%s", s);
      exit_status = 1;
    }
  else if (*end)
    {
      if (s == end)
        error (0, 0, gettext ("%s: expected a numeric value"), s);
      else
        error (0, 0, gettext ("%s: value not completely converted"), s);
      exit_status = 1;
    }
}
# 182 "printf.c"
...
static long double vstrtold (char const *s) { char *end; long double val; if ((*s == '\"' || *s == '\'') && *(s + 1)) { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (c_strtold (s, &end)); verify_numeric (s, end); } return val; }
  ...
  if (*p == 'x')
  ...
  else if (*p == 'u' || *p == 'U')
    {
      ...
      if ((uni_value <= 0x9f
        ...
        error (1, 0, gettext ("invalid universal character name \\%c%0*x"),
      ...
      switch (*f)
        ...
        no_more_flag_characters:
              ...
              if (argc > 0)
                {
                  ...
                  if ((-2147483647 -1) <= width && width <= 2147483647)
                    ...
                    error (1, 0, gettext ("invalid field width: %s"),
                           *argv);
                  ++argv;
                  --argc;
                }
                  ...
                  if (argc > 0)
                    {
                      ...
                      if (prec < 0)
                        ...
                        error (1, 0, gettext ("invalid precision: %s"),
                               *argv);
                      else
                        precision = prec;
                      ++argv;
...

          {
            ...
            if (! ok[conversion])
              error (1, 0,
                     gettext ("%.*s: invalid conversion specification"),
