...
set_suffix_length (uintmax_t n_units, enum Split_type split_type)
{
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines
      || split_type == type_rr)
    {
      ...
      _Bool alphabet_slop = (n_units_end % alphabet_len) != 0;
  ...
  if (suffix_length)
    {
      if (suffix_length < suffix_needed)
        {
          error (1, 0,
                 gettext ("the suffix length needs to be at least %""l" "u"),
                 suffix_needed);
        }
      suffix_auto = 0;
      return;
    ...
    suffix_length = (((2)>(suffix_needed))?(2):(suffix_needed));
}
  ...
  if (size < bufsize)
    {
      ...
      if (bufsize <= size && lseek (fd, - size, 1) < 0)
        error (1, (*__errno_location ()), "%s", infile);
    }
...
new_name:
      ...
      if (numeric_suffix_start)
        {
          ((void) sizeof ((! widen) ? 1 : 0), __extension__ ({ if (! widen) ; else __assert_fail ("! widen", "split.c", 365, __extension__ __PRETTY_FUNCTION__); }));
  ...
  if (!filter_command)
    {
      ...
      if (fstat (fd, &out_stat_buf) != 0)
        error (1, (*__errno_location ()), gettext ("failed to stat %s"), quote (name));
      if (((in_stat_buf).st_ino == (out_stat_buf).st_ino && (in_stat_buf).st_dev == (out_stat_buf).st_dev))
        error (1, 0, gettext ("%s would overwrite input; aborting"),
               quote (name));
      if (ftruncate (fd, 0) != 0)
        error (1, (*__errno_location ()), gettext ("%s: error truncating"), quote (name));
  ...
  else
    {
      ...
      if (setenv ("FILE", name, 1) != 0)
        error (1, (*__errno_location ()),
               gettext ("failed to set FILE environment variable"));
      if (verbose)
        fprintf (stdout, gettext ("executing with FILE=%s\n"), quote (name));
      if (pipe (fd_pair) != 0)
        error (1, (*__errno_location ()), gettext ("failed to create pipe"));
      child_pid = fork ();
      if (child_pid == 0)
        {
          ...
          for (j = 0; j < n_open_pipes; ++j)
            if (close (open_pipes[j]) != 0)
              error (1, (*__errno_location ()), gettext ("closing prior pipe"));
          if (close (fd_pair[1]))
            error (1, (*__errno_location ()), gettext ("closing output pipe"));
          if (fd_pair[0] != 0)
            {
              if (dup2 (fd_pair[0], 0) != 0)
                error (1, (*__errno_location ()), gettext ("moving input pipe"));
              if (close (fd_pair[0]) != 0)
                error (1, (*__errno_location ()), gettext ("closing input pipe"));
            }
          sigprocmask (2, &oldblocked, ((void*)0));
          execl (shell_prog, last_component (shell_prog), "-c",
                 filter_command, (char *) ((void*)0));
          error (1, (*__errno_location ()), gettext ("failed to run command: \"%s -c %s\""),
                 shell_prog, filter_command);
        }
      if (child_pid == -1)
        error (1, (*__errno_location ()), gettext ("fork system call failed"));
      if (close (fd_pair[0]) != 0)
        error (1, (*__errno_location ()), gettext ("failed to close input pipe"));
...
closeout (FILE *fp, int fd, pid_t pid, char const *name)
{
  if (fp != ((void*)0) && rpl_fclose (fp) != 0 && ! ignorable ((*__errno_location ())))
    error (1, (*__errno_location ()), "%s", name);
  if (fd >= 0)
    {
  ...
  if (pid > 0)
    {
      ...
      if (waitpid (pid, &wstatus, 0) == -1 && (*__errno_location ()) != 10)
        error (1, (*__errno_location ()), gettext ("waiting for child process"));
      if ((((signed char) (((wstatus) & 0x7f) + 1) >> 1) > 0))
      ...
      else
        {
          ...
          error (1, 0,
                 gettext ("unknown status from command (0x%X)"), wstatus + 0u);
        }
    }
}
...
cwrite (_Bool new_file_flag, const char *bp, size_t bytes)
{
  ...
  if (full_write (output_desc, bp, bytes) != bytes && ! ignorable ((*__errno_location ())))
    error (1, (*__errno_location ()), "%s", outfile);
}
      ...
      while (1)
        {
          if (to_read < to_write)
          ...
          else
            {
              ...
              if (!new_file_flag && ignorable ((*__errno_location ())))
                {
...
{
  ((void) sizeof ((n && k <= n && n <= file_size) ? 1 : 0), __extension__ ({ if (n && k <= n && n <= file_size) ; else __assert_fail ("n && k <= n && n <= file_size", "split.c", 806, __extension__ __PRETTY_FUNCTION__); }));
                     ...
                     size_t initial_read, off_t file_size)
{
  ...
  ((void) sizeof ((k && n && k <= n && n <= file_size) ? 1 : 0), __extension__ ({ if (k && n && k <= n && n <= file_size) ; else __assert_fail ("k && n && k <= n && n <= file_size", "split.c", 928, __extension__ __PRETTY_FUNCTION__); }));
  ...
  while (start < end)
    {
      ...
      if (full_write (1, buf, n_read) != n_read
        ...
        error (1, (*__errno_location ()), "%s", quote ("-"));
      start += n_read;
    }
}
...
# 1017 "split.c"
          ...
          if (!((*__errno_location ()) == 24 || (*__errno_location ()) == 23))
            error (1, (*__errno_location ()), "%s", files[i_check].of_name);
          ...
          if (rpl_fclose (files[i_reopen].ofile) != 0)
            error (1, (*__errno_location ()), "%s", files[i_reopen].of_name);
              ...
              if (unbuffered)
                {
                  ...
                  if (full_write (files[i_file].ofd, bp, to_write) != to_write
                      && ! ignorable ((*__errno_location ())))
                    error (1, (*__errno_location ()), "%s", files[i_file].of_name);
                }
...
main (int argc, char **argv)
{
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 't':
          {
            ...
            if (! neweol)
              error (1, 0, gettext ("empty record separator"));
            if (optarg[1])
              {
                if ((strcmp (optarg, "\\0") == 0))
                ...
                else
                  {
                    ...
                    error (1, 0, gettext ("multi-character separator %s"),
                           quote (optarg));
                  }
              }
            ...
            if (0 <= eolchar && neweol != eolchar)
              {
                error (1, 0,
                       gettext ("multiple separator characters specified"));
              }
        ...
        case '9':
          ...
          if (!( (void) (&(n_units) == (uintmax_t *) ((void*)0)), (void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (! ((uintmax_t) 0 < (uintmax_t) -1))) ? 1 : -1; })), (void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (n_units) == sizeof (uintmax_t)) ? 1 : -1; })), (((uintmax_t) -1 / 10 < (n_units) || (uintmax_t) ((n_units) * 10 + (c - '0')) < (n_units)) ? 0 : (((n_units) = (n_units) * 10 + (c - '0')), 1)) ))
            {
              ...
              error (1, 0,
                     gettext ("line count option -%s%c... is too large"),
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "split", "GNU coreutils", Version, proper_name_utf8 ("Torbjorn Granlund", "Torbj\303\266rn Granlund"), ("Richard M. Stallman"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (! (strcmp (infile, "-") == 0)
    ...
    error (1, (*__errno_location ()), gettext ("cannot open %s for reading"),
           quote (infile));
  ...
  if (split_type == type_chunk_bytes || split_type == type_chunk_lines)
    {
      off_t input_offset = lseek (0, 0, 1);
      if (0 <= input_offset)
        {
          if (usable_st_size (&in_stat_buf) && ! specified_buf_size)
            {
              ((void) sizeof ((((0 < (in_stat_buf).st_blksize && (in_stat_buf).st_blksize <= ((size_t)-1) / 8 + 1) ? (in_stat_buf).st_blksize : 512) <= in_blk_size) ? 1 : 0), __extension__ ({ if (((0 < (in_stat_buf).st_blksize && (in_stat_buf).st_blksize <= ((size_t)-1) / 8 + 1) ? (in_stat_buf).st_blksize : 512) <= in_blk_size) ; else __assert_fail ("ST_BLKSIZE (in_stat_buf) <= in_blk_size", "split.c", 1512, __extension__ __PRETTY_FUNCTION__); }));
              file_size = input_file_size (0, in_stat_buf.st_size,
                                           buf, in_blk_size);
            ...
            {
              file_size = lseek (0, 0, 2);
              input_offset = (file_size < 0
                              ? file_size
                              : lseek (0, input_offset, 0));
              file_size -= input_offset;
            }
        }
      if (input_offset < 0)
        error (1, 0, gettext ("%s: cannot determine file size"),
               quote (infile));
      ...
      if (((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) < n_units)
        {
          char buffer[(((((sizeof (uintmax_t) * 8 - (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (uintmax_t)) 0 < (__typeof__ (uintmax_t)) -1))) + 1)];
          error (1, 75, "%s: %s",
                 gettext ("invalid number of chunks"),
