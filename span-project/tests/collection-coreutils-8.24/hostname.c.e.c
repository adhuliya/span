...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "hostname", "GNU coreutils", Version,
  ...
  if (argc == optind + 1)
    {
      ...
      if (sethostname (name, strlen (name)) != 0)
        error (1, (*__errno_location ()), gettext ("cannot set name to %s"), quote (name));
  ...
  if (argc <= optind)
    {
      ...
      if (hostname == ((void*)0))
        error (1, (*__errno_location ()), gettext ("cannot determine hostname"));
      printf ("%s\n", hostname);
    }
