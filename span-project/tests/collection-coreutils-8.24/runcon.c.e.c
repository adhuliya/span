...
main (int argc, char **argv)
{
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case 'r':
          if (role)
            error (1, 0, gettext ("multiple roles"));
          role = optarg;
          break;
        case 't':
          if (type)
            error (1, 0, gettext ("multiple types"));
          type = optarg;
          break;
        case 'u':
          if (user)
            error (1, 0, gettext ("multiple users"));
          user = optarg;
          break;
        case 'l':
          if (range)
            error (1, 0, gettext ("multiple levelranges"));
          range = optarg;
          break;
        case 'c':
          compute_trans = 1;
  ...
  if (0 != 1)
    error (1, 0, gettext ("%s may be used only on a SELinux kernel"),
           program_name);
  ...
  if (context)
    {
      ...
      if (!con)
        error (1, (*__errno_location ()), gettext ("failed to create security context: %s"),
  ...
  else
    {
      ...
      if (compute_trans)
        {
          ...
          if (security_compute_create (cur_context, file_context,
                                       0, &new_context) != 0)
            error (1, (*__errno_location ()), gettext ("failed to compute a new context"));
      ...
      if (user && context_user_set (con, user))
        error (1, (*__errno_location ()), gettext ("failed to set new user %s"), user);
      if (type && context_type_set (con, type))
        error (1, (*__errno_location ()), gettext ("failed to set new type %s"), type);
      if (range && context_range_set (con, range))
        error (1, (*__errno_location ()), gettext ("failed to set new range %s"), range);
      if (role && context_role_set (con, role))
        error (1, (*__errno_location ()), gettext ("failed to set new role %s"), role);
    }
  ...
  if (setexeccon (context_str (con)) != 0)
    error (1, (*__errno_location ()), gettext ("unable to set security context %s"),
  ...
  error (0, (*__errno_location ()), "%s", argv[optind]);
  return exit_status;
}
