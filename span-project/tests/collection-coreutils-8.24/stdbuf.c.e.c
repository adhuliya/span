...
{
  uintmax_t tmp_size;
  enum strtol_error e = xstrtoumax (str, ((void*)0), 10, &tmp_size, "EGkKMPTYZ0");
  if (e == LONGINT_OK && (18446744073709551615UL) < tmp_size)
    e = LONGINT_OVERFLOW;
  ...
  (*__errno_location ()) = (e == LONGINT_OVERFLOW ? 75 : (*__errno_location ()));
  return -1;
}
  ...
  else
    {
      ...
      if (path)
      ...
      else if ((path = getenv ("PATH")))
        {
          char *dir;
          path = xstrdup (path);
          for (dir = strtok (path, ":"); dir != ((void*)0); dir = strtok (((void*)0), ":"))
            {
              char *candidate = file_name_concat (dir, arg, ((void*)0));
...
# 214 "stdbuf.c"
  ...
  while (1)
    {
      ...
      ret = rpl_asprintf (&libstdbuf, "%s/%s", *path, "libstdbuf.so");
      if (ret < 0)
        xalloc_die ();
  ...
  if (old_libs)
    ret = rpl_asprintf (&LD_PRELOAD, "%s=%s:%s", preload_env, old_libs, libstdbuf);
  else
    ret = rpl_asprintf (&LD_PRELOAD, "%s=%s", preload_env, libstdbuf);
      ...
      if (stdbuf[i].optarg)
        {
          ...
          if (*stdbuf[i].optarg == 'L')
            ret = rpl_asprintf (&var, "%s%c=L", "_STDBUF_",
                            toupper (stdbuf[i].optc));
          else
            ret = rpl_asprintf (&var, "%s%c=%" "l" "u", "_STDBUF_",
                            toupper (stdbuf[i].optc),
  ...
  while ((c = getopt_long (argc, argv, "+i:o:e:", longopts, ((void*)0))) != -1)
    {
      ...
      switch (c)
        ...
        case 'o':
          ...
          ((void) sizeof ((0 <= opt_fileno && opt_fileno < (sizeof (stdbuf) / sizeof *(stdbuf))) ? 1 : 0), __extension__ ({ if (0 <= opt_fileno && opt_fileno < (sizeof (stdbuf) / sizeof *(stdbuf))) ; else __assert_fail ("0 <= opt_fileno && opt_fileno < ARRAY_CARDINALITY (stdbuf)", "stdbuf.c", 335, __extension__ __PRETTY_FUNCTION__); }));
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "stdbuf", "GNU coreutils", Version, proper_name_utf8 ("Padraig Brady", "P\303\241draig Brady"), (char *) ((void*)0)); exit (0); break;;
