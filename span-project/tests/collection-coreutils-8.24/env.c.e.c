...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "+iu:0", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        {
        case 'i':
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "env", "GNU coreutils", Version, ("Richard Mlynarik"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (EXIT_CANCELED);
        }
    }
  ...
  while ((optc = getopt_long (argc, argv, "+iu:0", longopts, ((void*)0))) != -1)
    if (optc == 'u' && unsetenv (optarg))
      error (EXIT_CANCELED, (*__errno_location ()), gettext ("cannot unset %s"), quote (optarg));
  ...
  while (optind < argc && (eq = strchr (argv[optind], '=')))
    {
      if (putenv (argv[optind]))
        {
          *eq = '\0';
          error (EXIT_CANCELED, (*__errno_location ()), gettext ("cannot set %s"),
                 quote (argv[optind]));
        }
      optind++;
    }
  ...
  if (opt_nul_terminate_output)
    {
      error (0, (*__errno_location ()), gettext ("cannot specify --null (-0) with command"));
      usage (EXIT_CANCELED);
    }
