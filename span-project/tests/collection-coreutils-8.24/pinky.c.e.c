...
static _Bool include_idle = 1;


static _Bool include_heading = 1;


static _Bool include_fullname = 1;


static _Bool include_project = 1;


static _Bool include_plan = 1;



static _Bool include_home_and_shell = 1;


static _Bool do_short_format = 1;



static _Bool include_where = 1;
  ...
  if (ampersands != 0)
    {
      ...
      if (((size_t) (sizeof (ptrdiff_t) <= sizeof (size_t) ? -1 : -2) / (ampersands) < (ulen)) || rsize < product)
        xalloc_die ();
    }
      ...
      if (*gecos_name == '&')
        {
          ...
          if (((*__ctype_b_loc ())[(int) ((to_uchar (*uname)))] & (unsigned short int) _ISlower))
...
{
  static time_t now = 0;
  static char buf[((((sizeof (long int) * 8 - (! ((__typeof__ (long int)) 0 < (__typeof__ (long int)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (long int)) 0 < (__typeof__ (long int)) -1))) + 2];
  time_t seconds_idle;
...
print_entry (const STRUCT_UTMP *utmp_ent)
{
  ...
  printf ("%-8.*s", UT_USER_SIZE, ((utmp_ent)->ut_user));
  ...
  if (include_fullname)
    {
      ...
      stzncpy (name, ((utmp_ent)->ut_user), UT_USER_SIZE);
      pw = getpwnam (name);
      if (pw == ((void*)0))
      ...
      if (stream)
        {
          ...
          printf (gettext ("Project: "));

          while ((bytes = fread_unlocked (buf,1,sizeof (buf),stream)) > 0)
            fwrite_unlocked (buf,1,bytes,stdout);
          rpl_fclose (stream);
        }
          ...
          if (argc_names)
            {
              ...
              for (i = 0; i < argc_names; i++)
                if ((strncmp (((utmp_buf)->ut_user), argv_names[i], UT_USER_SIZE) == 0))
                  {
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -f              omit the line of column headings in short format\n  -w              omit the user's full name in short format\n  -i              omit the user's full name and remote host in short format\n  -q              omit the user's full name, remote host and idle time\n                  in short format\n"),stdout);
  ...
  while ((optc = getopt_long (argc, argv, "sfwiqbhlp", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "pinky", "GNU coreutils", Version, ("Joseph Arceneaux"), ("David MacKenzie"), ("Kaveh Ghazi"), (char *) ((void*)0)); exit (0); break;;
