...
# 1 "../lib/unistd--.h" 1
# 21 "../lib/unistd--.h"
# 1 "../lib/unistd-safer.h" 1
# 20 "../lib/unistd-safer.h"
int dup_safer (int);
int fd_safer (int);
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "nohup", "GNU coreutils", Version,
                      usage, ("Jim Meyering"), (char const *) ((void*)0));
  ...
  ignoring_input = isatty (0);
  redirecting_stdout = isatty (1);
  stdout_is_closed = (!redirecting_stdout && (*__errno_location ()) == 9);
  redirecting_stderr = isatty (2);
  ...
  if (ignoring_input)
    {
      if (fd_reopen (0, "/dev/null", 01, 0) < 0)
        error (exit_internal_failure, (*__errno_location ()),
               gettext ("failed to render standard input unusable"));
  ...
  if (redirecting_stdout || (redirecting_stderr && stdout_is_closed))
    {
      ...
      int flags = 0100 | 01 | 02000;
      mode_t mode = 0400 | 0200;
      mode_t umask_value = umask (~mode);
      out_fd = (redirecting_stdout
                ? fd_reopen (1, file, flags, mode)
          ...
          if (home)
            {
              ...
              out_fd = (redirecting_stdout
                        ? fd_reopen (1, in_home, flags, mode)
                        : open (in_home, flags, mode));
            }
          if (out_fd < 0)
            {
              int saved_errno2 = (*__errno_location ());
              error (0, saved_errno, gettext ("failed to open %s"), quote (file));
              if (in_home)
                error (0, saved_errno2, gettext ("failed to open %s"),
                       quote (in_home));
      ...
      umask (umask_value);
      error (0, 0,
             gettext (ignoring_input ? "ignoring input and appending output to %s" : "appending output to %s"),
  ...
  if (redirecting_stderr)
    {
      ...
      if (0 <= saved_stderr_fd
          && set_cloexec_flag (saved_stderr_fd, 1) != 0)
        error (exit_internal_failure, (*__errno_location ()),
               gettext ("failed to set the copy of stderr to close on exec"));

      if (!redirecting_stdout)
        error (0, 0,
               gettext (ignoring_input ? "ignoring input and redirecting stderr to stdout" : "redirecting stderr to stdout"));



      if (dup2 (out_fd, 2) < 0)
        error (exit_internal_failure, (*__errno_location ()),
               gettext ("failed to redirect standard error"));
  ...
  if (dup2 (saved_stderr_fd, 2) == 2)
    error (0, saved_errno, gettext ("failed to run command %s"), quote (*cmd));
