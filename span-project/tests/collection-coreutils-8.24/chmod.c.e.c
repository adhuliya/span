...
static _Bool force_silent;




static _Bool diagnose_surprises;
              ...
              mode_t old_mode, mode_t new_mode)
{
  if (new_mode & (04000 | 02000 | 01000))
    {
      ...
      if (fstatat (dir_fd, file, &new_stats, 0) != 0)
        {
          if (! force_silent)
            error (0, (*__errno_location ()), gettext ("getting new attributes of %s"),
  ...
  return ((old_mode ^ new_mode) & (04000 | 02000 | 01000 | (0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3))) != 0;
}
  ...
  switch (changed)
    ...
    case CH_NO_CHANGE_REQUESTED:
      ...
      printf (fmt, quote (file),
              (unsigned long int) (mode & (04000 | 02000 | 01000 | (0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3))), &perms[1]);
      return;
    default:
      abort ();
    }
  printf (fmt, quote (file),
          (unsigned long int) (old_mode & (04000 | 02000 | 01000 | (0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3))), &old_perms[1],
          (unsigned long int) (mode & (04000 | 02000 | 01000 | (0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3))), &perms[1]);
}
...
process_file (FTS *fts, FTSENT *ent)
{
  ...
  switch (ent->fts_info)
    ...
    case 13:
      if (! force_silent)
        error (0, 0, gettext ("cannot operate on dangling symlink %s"),
               quote (file_full_name));
      ok = 0;
      break;
  ...
  if (ok && (root_dev_ino && ((*file_stats).st_ino == (*root_dev_ino).st_ino && (*file_stats).st_dev == (*root_dev_ino).st_dev)))
    {
  ...
  if (ok)
    {
      ...
      new_mode = mode_adjust (old_mode, ((((old_mode)) & 0170000) == (0040000)) != 0, umask_value,
                              change, ((void*)0));

      if (! ((((old_mode)) & 0170000) == (0120000)))
        {
          if (chmodat (fts->fts_cwd_fd, file, new_mode) == 0)
            ...
            {
              if (! force_silent)
                error (0, (*__errno_location ()), gettext ("changing permissions of %s"),
                       quote (file_full_name));
              ok = 0;
            }
        }
  ...
  if (chmod_succeeded && diagnose_surprises)
    {
      mode_t naively_expected_mode =
        mode_adjust (old_mode, ((((old_mode)) & 0170000) == (0040000)) != 0, 0, change, ((void*)0));
      if (new_mode & ~naively_expected_mode)
        {
          ...
          error (0, 0,
                 gettext ("%s: new permissions are %s, not %s"),
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Change the mode of each FILE to MODE.\nWith --reference, change the mode of each FILE to that of RFILE.\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("      --reference=RFILE  use RFILE's mode instead of MODE values\n"),stdout);
  ...
  while ((c = getopt_long (argc, argv,
         ...
         != -1)
    {
      switch (c)
            ...
            if (mode_alloc <= new_mode_len)
              {
                ...
                mode = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(mode) == 1) ? 1 : -1; })), x2realloc (mode, &mode_alloc));
              }
            mode[mode_len] = ',';
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "chmod", "GNU coreutils", Version, ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
