...
# 218 "shuf.c"
      ...
      if (! n_lines)
        error (1, 75, gettext ("too many input lines"));
...
main (int argc, char **argv)
{
    ...
    switch (optc)
      ...
      case 'i':
        {
          ...
          if (input_range)
            error (1, 0, gettext ("multiple -i options specified"));
          input_range = 1;
          ...
          if (invalid)
            error (1, (*__errno_location ()), "%s: %s", gettext ("invalid input range"),
                   quote (optarg));
        }
        break;
      ...
      case 'n':
        {
          ...
          strtol_error e = xstrtoul (optarg, ((void*)0), 10, &argval, ((void*)0));

          if (e == LONGINT_OK)
            head_lines = (((head_lines)<(argval))?(head_lines):(argval));
          else if (e != LONGINT_OVERFLOW)
            error (1, 0, gettext ("invalid line count: %s"),
                   quote (optarg));
        }
        break;
      ...
      case 'o':
        if (outfile && !(strcmp (outfile, optarg) == 0))
          error (1, 0, gettext ("multiple output files specified"));
        outfile = optarg;
        break;
  ...
  if (echo)
  ...
  else
    {
      ...
      if (n_operands == 1)
        if (! ((strcmp (operand[0], "-") == 0) || ! head_lines
               || freopen_safer (operand[0], "r", stdin)))
          error (1, (*__errno_location ()), "%s", operand[0]);
  ...
  if (! repeat)
    head_lines = (((head_lines)<(n_lines))?(head_lines):(n_lines));
  ...
  if (outfile && ! freopen_safer (outfile, "w", stdout))
    error (1, (*__errno_location ()), "%s", quotearg_colon (outfile));
  ...
  if (repeat)
    {
      if (head_lines == 0)
        ...
        {
          if (n_lines == 0)
            error (1, 0, gettext ("no lines to repeat"));
          if (input_range)
