...
# 1 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 1 3 4
# 23 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 3 4
# 1 "/usr/include/paths.h" 1 3 4
# 24 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 2 3 4

# 1 "../lib/sys/types.h" 1 3 4
# 21 "../lib/sys/types.h" 3
# 26 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 2 3 4
# 1 "/usr/include/x86_64-linux-gnu/bits/wordsize.h" 1 3 4
# 27 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 2 3 4
# 36 "/usr/include/x86_64-linux-gnu/bits/utmp.h" 3 4
struct lastlog
  {
...
enum { UT_USER_SIZE = sizeof (((STRUCT_UTMP *) 0)->ut_user) };
# 208 "../lib/readutmp.h"
enum
  {
...
{
  static time_t now = ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) 0 : ((time_t) ~ (time_t) 0 < (time_t) -1) ? ~ (time_t) 0 : ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1)))));

  if (now == ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) 0 : ((time_t) ~ (time_t) 0 < (time_t) -1) ? ~ (time_t) 0 : ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1))))))
    time (&now);
...
{
  static char buf[((((sizeof (intmax_t) * 8 - (! ((__typeof__ (intmax_t)) 0 < (__typeof__ (intmax_t)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (intmax_t)) 0 < (__typeof__ (intmax_t)) -1))) + sizeof "-%m-%d %H:%M"];
            ...
            const char *comment, const char *exitstr)
{
  ...
  char *buf;
  char x_idle[1 + 6 + 1];
  char x_pid[1 + ((((sizeof (pid_t) * 8 - (! ((__typeof__ (pid_t)) 0 < (__typeof__ (pid_t)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (pid_t)) 0 < (__typeof__ (pid_t)) -1))) + 1];
  char *x_exitstr;
  int err;
  ...
  x_exitstr = xmalloc (include_exit ? 1 + (((12)>(strlen (exitstr)))?(12):(strlen (exitstr))) + 1 : 1);
  if (include_exit)
    sprintf (x_exitstr, " %-12s", exitstr);
  else
    *x_exitstr = '\0';

  err = rpl_asprintf (&buf,
                  "%-8.*s"
                  "%s"
                  " %-12.*s"
                  " %-*s"
...
print_user (const STRUCT_UTMP *utmp_ent, time_t boottime)
{
  ...
  char line[sizeof (utmp_ent->ut_line) + (sizeof ("/dev/") - 1) + 1];
  char *p = line;
  ...
  print_line (sizeof ((utmp_ent)->ut_user), ((utmp_ent)->ut_user), mesg,
...
{
  char *comment = xmalloc (strlen (gettext ("id=")) + sizeof ((utmp_ent)->ut_id) + 1);

  strcpy (comment, gettext ("id="));
  strncat (comment, ((utmp_ent)->ut_id), sizeof ((utmp_ent)->ut_id));
  return comment;
}
...
print_clockchange (const STRUCT_UTMP *utmp_ent)
{
  ...
  print_line (-1, "", ' ', -1, gettext ("clock change"),
              time_string (utmp_ent), "", "", "", "");
}
...
list_entries_who (size_t n, const STRUCT_UTMP *utmp_buf)
{
  ...
  while (n--)
    {
      if ((((utmp_buf)->ut_user)[0] && (((utmp_buf)->ut_type == (7)) || (0 && ((utmp_buf)->ut_tv.tv_sec) != 0))))
        {
          char *trimmed_name;
...
print_heading (void)
{
  print_line (-1, gettext ("NAME"), ' ', -1, gettext ("LINE"), gettext ("TIME"), gettext ("IDLE"),
              gettext ("PID"), gettext ("COMMENT"), gettext ("EXIT"));
}
...
{
  char *ttyname_b ;
  time_t boottime = ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) 0 : ((time_t) ~ (time_t) 0 < (time_t) -1) ? ~ (time_t) 0 : ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1)))));
  ...
  while (n--)
    {
      if (!my_line_only
        ...
        {
          if (need_users && (((utmp_buf)->ut_user)[0] && (((utmp_buf)->ut_type == (7)) || (0 && ((utmp_buf)->ut_tv.tv_sec) != 0))))
...
who (const char *filename, int options)
{
  ...
  if (read_utmp (filename, &n_users, &utmp_buf, options) != 0)
    error (1, (*__errno_location ()), "%s", filename);
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "abdlmpqrstuwHT", longopts, ((void*)0)))
         != -1)
    {
      switch (optc)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "who", "GNU coreutils", Version, ("Joseph Arceneaux"), ("David MacKenzie"), ("Michael Stone"), (char *) ((void*)0)); exit (0); break;;
  ...
  switch (argc - optind)
    ...
    case -1:
    case 0:
      who ("/var/run/utmp", READ_UTMP_CHECK_PIDS);
      break;
