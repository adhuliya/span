...
{
  return err == 2 || err == 36 || err == 20 || err == 40;
}
...
target_directory_operand (char const *file)
{
  ...
  int err = (stat_result == 0 ? 0 : (*__errno_location ()));
  _Bool is_a_dir = !err && ((((st.st_mode)) & 0170000) == (0040000));
  if (err && ! errno_nonexisting ((*__errno_location ())))
...
do_link (const char *source, const char *dest)
{
  ...
  char *dest_backup = ((void*)0);
  char *rel_source = ((void*)0);
  _Bool dest_lstat_ok = 0;
  _Bool source_is_dir = 0;
  _Bool ok;
  ...
  if (!symbolic_link)
    {
      ...
      if ((logical ? stat (source, &source_stats)
          ...
          != 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to access %s"), quote (source));
          return 0;
        }

      if (((((source_stats.st_mode)) & 0170000) == (0040000)))
        {
          source_is_dir = 1;
          if (! hard_dir_link)
            {
  ...
  if (remove_existing_files || interactive || backup_type != no_backups)
    {
      ...
      if (!dest_lstat_ok && (*__errno_location ()) != 2)
        {
          error (0, (*__errno_location ()), gettext ("failed to access %s"), quote (dest));
          return 0;
        }
    }
  ...
  if (dest_lstat_ok)
    {
      if (((((dest_stats.st_mode)) & 0170000) == (0040000)))
        {
          error (0, 0, gettext ("%s: cannot overwrite directory"), quote (dest));
          return 0;
        }
      if (interactive)
        {
          fprintf (stderr, gettext ("%s: replace %s? "), program_name, quote (dest));
          if (!yesno ())
            return 1;
          ...
          if (rename (dest, dest_backup) != 0)
            {
              ...
              if (rename_errno != 2)
                {
                  error (0, rename_errno, gettext ("cannot backup %s"), quote (dest));
                  return 0;
                }
            }
        }
  ...
  ok = ((symbolic_link ? symlink (source, dest)
         : linkat (-100, source, -100, dest,
                   logical ? 0x400 : 0))
        == 0);
# 326 "ln.c"
  if (!ok && (*__errno_location ()) == 17 && (remove_existing_files || dest_backup))
    {
      if (unlink (dest) != 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot remove %s"), quote (dest));
          free (dest_backup);
          free (rel_source);
          return 0;
        }
      ...
      if (dest_backup)
        {
          if (rename (dest_backup, dest) != 0)
            error (0, (*__errno_location ()), gettext ("cannot un-backup %s"), quote (dest));
        }
    }
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [OPTION]... [-T] TARGET LINK_NAME   (1st form)\n  or:  %s [OPTION]... TARGET                  (2nd form)\n  or:  %s [OPTION]... TARGET... DIRECTORY     (3rd form)\n  or:  %s [OPTION]... -t DIRECTORY TARGET...  (4th form)\n"),
...
main (int argc, char **argv)
{
  ...
  if (no_target_directory)
    {
      if (target_directory)
        error (1, 0,
               gettext ("cannot combine --target-directory " "and --no-target-directory"));
  ...
  if (relative && !symbolic_link)
    {
        error (1, 0,
               gettext ("cannot do --relative without --symbolic"));
    }
