...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (time_spec_string) / sizeof *(time_spec_string)) == (sizeof (time_spec) / sizeof *(time_spec)) + 1) ? 1 : -1; }))];
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -R, --rfc-2822             output date and time in RFC 2822 format.\n                               Example: Mon, 14 Aug 2006 02:34:56 -0600\n"),stdout);
...
batch_convert (const char *input_filename, const char *format)
{
  ...
  if ((strcmp (input_filename, "-") == 0))
  ...
  else
    {
      ...
      if (in_stream == ((void*)0))
        {
          error (1, (*__errno_location ()), "%s", quote (input_filename));
        }
    }
  ...
  while (1)
    {
      ...
      if (! parse_datetime (&when, line, ((void*)0)))
        {
        ...
        case RFC_3339_OPTION:
          {
            ...
            enum Time_spec i =
              ((time_spec + 2) [__xargmatch_internal ("--rfc-3339", optarg, time_spec_string + 2, (char const *) (time_spec + 2), sizeof *(time_spec + 2), argmatch_die)]);
        ...
        case 'I':
          {
            ...
            enum Time_spec i =
              (optarg
               ? ((time_spec) [__xargmatch_internal ("--iso-8601", optarg, time_spec_string, (char const *) (time_spec), sizeof *(time_spec), argmatch_die)])
               : TIME_SPEC_DATE);
          ...
          if (reference != ((void*)0))
            {
              if (stat (reference, &refstats) != 0)
                error (1, (*__errno_location ()), "%s", reference);
          ...
          else
            {
              if (set_datestr)
                datestr = set_datestr;
              valid_date = parse_datetime (&when, datestr, ((void*)0));
            }
        }

      if (! valid_date)
        error (1, 0, gettext ("invalid date %s"), quote (datestr));
  ...
  if (! tm)
    {
      ...
      error (0, 0, gettext ("time %s is out of range"), timetostr (when.tv_sec, buf));
      return 0;
    }
