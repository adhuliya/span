  ...
  if (status != 0)
  ...
  else
    {
      printf (gettext ("Usage: %s [OPTION]... [FILE]...\n"),
              ...
              program_name);
      fputs_unlocked (gettext ("Print the first 10 lines of each FILE to standard output.\nWith more than one FILE, precede each with a header giving the file name.\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -c, --bytes=[-]K         print the first K bytes of each file;\n                             with the leading '-', print all but the last\n                             K bytes of each file\n  -n, --lines=[-]K         print the first K lines instead of the first 10;\n                             with the leading '-', print all but the last\n                             K lines of each file\n"),stdout);
...
diagnose_copy_fd_failure (enum Copy_fd_status err, char const *filename)
{
  switch (err)
    ...
    case COPY_FD_READ_ERROR:
      error (0, (*__errno_location ()), gettext ("error reading %s"), quote (filename));
      break;
    case COPY_FD_UNEXPECTED_EOF:
      error (0, (*__errno_location ()), gettext ("%s: file has shrunk too much"), quote (filename));
      break;
    default:
      abort ();
    }
                       ...
                       struct stat const *st, off_t current_pos)
{
  ...
  if (presume_input_pipe || size <= ((0 < (*st).st_blksize && (*st).st_blksize <= ((size_t)-1) / 8 + 1) ? (*st).st_blksize : 512))
           ...
           _Bool elide_from_end)
{
  ...
  if (is_stdin)
  ...
  else
    {
      ...
      if (fd < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot open %s for reading"), quote (filename));
          return 0;
        }
    }
  ...
  if (!is_stdin && close (fd) != 0)
    {
      error (0, (*__errno_location ()), gettext ("failed to close %s"), quote (filename));
      return 0;
    }
  return ok;
}
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "c:n:qv0123456789", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        {
  ...
  if ( ! count_lines && elide_from_end && ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) < n_units)
    {
      ...
      error (1, 75, "%s: %s", gettext ("invalid number of bytes"),
