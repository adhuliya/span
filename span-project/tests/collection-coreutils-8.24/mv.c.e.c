      ...
      if (dir_to_remove != ((void*)0))
        {
          ...
          ((void) sizeof ((((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (status)", "mv.c", 235, __extension__ __PRETTY_FUNCTION__); }));
          if (status == RM_ERROR)
            ok = 0;
        }
    }
...
main (int argc, char **argv)
{
  ...
  if (no_target_directory)
    ...
    {
      ((void) sizeof ((2 <= n_files) ? 1 : 0), __extension__ ({ if (2 <= n_files) ; else __assert_fail ("2 <= n_files", "mv.c", 469, __extension__ __PRETTY_FUNCTION__); }));
