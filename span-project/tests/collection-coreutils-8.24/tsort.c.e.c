...
tsort (const char *file)
{
  ...
  if (!is_stdin && ! freopen_safer (file, "r", stdin))
    error (1, (*__errno_location ()), "%s", file);
  ...
  if (k != ((void*)0))
    error (1, 0, gettext ("%s: input contains an odd number of tokens"),
           file);
      ...
      if (n_strings > 0)
        {
          ...
          error (0, 0, gettext ("%s: input contains a loop:"), file);
          ok = 0;
  ...
  if (rpl_fclose (stdin) != 0)
    error (1, (*__errno_location ()), "%s",
           is_stdin ? gettext ("standard input") : quote (file));
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "tsort", "coreutils", Version,
                      usage, ("Mark Kettenis"), (char const *) ((void*)0));
