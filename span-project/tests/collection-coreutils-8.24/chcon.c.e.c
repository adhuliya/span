...
process_file (FTS *fts, FTSENT *ent)
{
  ...
  switch (ent->fts_info)
          ...
          if ((root_dev_ino && ((*ent->fts_statp).st_ino == (*root_dev_ino).st_ino && (*ent->fts_statp).st_dev == (*root_dev_ino).st_dev)))
            {
              ...
              do { if ((strcmp (file_full_name, "/") == 0)) error (0, 0, gettext ("it is dangerous to operate recursively on %s"), quote (file_full_name)); else error (0, 0, gettext ("it is dangerous to operate recursively on %s (same as %s)"), quote_n (0, file_full_name), quote_n (1, "/")); error (0, 0, gettext ("use --no-preserve-root to override this failsafe")); } while (0);

              fts_set (fts, ent, 4);

              (__extension__ ({ __typeof__ (fts_read (fts)) __x = (fts_read (fts)); (void) __x; }));
              return 0;
            }
          return 1;
        }
    ...
    case 10:
      ...
      error (0, ent->fts_errno, gettext ("cannot access %s"), quote (file_full_name));
      ok = 0;
      break;
  ...
  if (reference_file)
    {
      ...
      if (getfilecon (reference_file, &ref_context) < 0)
        error (1, (*__errno_location ()), gettext ("failed to get security context of %s"),
