...
append_normal_char (struct Spec_list *list, unsigned char c)
{
  ...
  new->u.normal_char = c;
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 652, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
}
...
append_range (struct Spec_list *list, unsigned char first, unsigned char last)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 684, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
                   ...
                   char const *char_class_str, size_t len)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 709, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
                      ...
                      count repeat_count)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 731, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
}
                    ...
                    char const *equiv_class_str, size_t len)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 754, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
                       ...
                       size_t *closing_bracket_idx)
{
  ...
  ((void) sizeof ((start_idx + 1 < es->len) ? 1 : 0), __extension__ ({ if (start_idx + 1 < es->len) ; else __assert_fail ("start_idx + 1 < es->len", "tr.c", 799, __extension__ __PRETTY_FUNCTION__); }));
      ...
      if (s->state == (((18446744073709551615UL) - 1) + 1))
        {
          for (i = 0; i < N_CHARS; i++)
            if (is_char_class_member (p->u.char_class, i))
              break;
          ((void) sizeof ((i < N_CHARS) ? 1 : 0), __extension__ ({ if (i < N_CHARS) ; else __assert_fail ("i < N_CHARS", "tr.c", 1098, __extension__ __PRETTY_FUNCTION__); }));
          s->state = i;
        }
      ((void) sizeof ((is_char_class_member (p->u.char_class, s->state)) ? 1 : 0), __extension__ ({ if (is_char_class_member (p->u.char_class, s->state)) ; else __assert_fail ("is_char_class_member (p->u.char_class, s->state)", "tr.c", 1101, __extension__ __PRETTY_FUNCTION__); }));
      return_val = s->state;
...
validate_case_classes (struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  ((void) sizeof ((old_s1_len >= s1->length && old_s2_len >= s2->length) ? 1 : 0), __extension__ ({ if (old_s1_len >= s1->length && old_s2_len >= s2->length) ; else __assert_fail ("old_s1_len >= s1->length && old_s2_len >= s2->length", "tr.c", 1245, __extension__ __PRETTY_FUNCTION__); }));
  ...
  for (p = s->head->next; p; p = p->next)
    {
      ...
      switch (p->type)
        ...
        case RE_RANGE:
          ((void) sizeof ((p->u.range.last_char >= p->u.range.first_char) ? 1 : 0), __extension__ ({ if (p->u.range.last_char >= p->u.range.first_char) ; else __assert_fail ("p->u.range.last_char >= p->u.range.first_char", "tr.c", 1287, __extension__ __PRETTY_FUNCTION__); }));
      ...
      if (! (length <= new_length && new_length <= ((18446744073709551615UL) - 1)))
        error (1, 0, gettext ("too many characters in set"));
      length = new_length;
    }
...
string2_extend (const struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  ((void) sizeof ((translating) ? 1 : 0), __extension__ ({ if (translating) ; else __assert_fail ("translating", "tr.c", 1401, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((s1->length > s2->length) ? 1 : 0), __extension__ ({ if (s1->length > s2->length) ; else __assert_fail ("s1->length > s2->length", "tr.c", 1402, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((s2->length > 0) ? 1 : 0), __extension__ ({ if (s2->length > 0) ; else __assert_fail ("s2->length > 0", "tr.c", 1403, __extension__ __PRETTY_FUNCTION__); }));
  ...
  switch (p->type)
    ...
    case RE_CHAR_CLASS:
      ...
      error (1, 0,
             gettext ("when translating with string1 longer than string2,\nthe latter string must not end with a character class"));
...
validate (struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  if (s1->n_indefinite_repeats > 0)
    {
      error (1, 0,
             gettext ("the [c*] repeat construct may not appear in string1"));
    }
  ...
  if (s2)
    {
      ...
      if (s2->n_indefinite_repeats > 1)
        {
          error (1, 0,
                 gettext ("only one [c*] repeat construct may appear in string2"));
        }
      ...
      if (translating)
        {
          if (s2->has_equiv_class)
            {
              error (1, 0,
                     gettext ("[=c=] expressions may not appear in string2 when translating"));
          ...
          if (s2->has_restricted_char_class)
            {
              error (1, 0,
                     gettext ("when translating, the only character classes that may appear in\nstring2 are 'upper' and 'lower'"));
              ...
              if (!truncate_set1)
                {
                  ...
                  if (s2->length == 0)
                    error (1, 0,
                     gettext ("when not truncating set1, string2 must be non-empty"));
          ...
          if (complement && s1->has_char_class
            ...
            {
              error (1, 0,
                     gettext ("when translating with complemented character classes,\nstring2 must map all characters in the domain to one"));
...

        {
          if (s2->n_indefinite_repeats > 0)
            error (1, 0,
                   gettext ("the [c*] construct may appear in string2 only when translating"));
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "+cCdst", long_options, ((void*)0))) != -1)
    {
      switch (c)
        {
        case 'c':
  ...
  else if (translating)
    {
      if (complement)
              ...
              if (!in_s1[i])
                {
                  ...
                  ((void) sizeof ((ch != -1 || truncate_set1) ? 1 : 0), __extension__ ({ if (ch != -1 || truncate_set1) ; else __assert_fail ("ch != -1 || truncate_set1", "tr.c", 1870, __extension__ __PRETTY_FUNCTION__); }));
                  if (ch == -1)
                    {
      ...
      else
        {
          ...
          ((void) sizeof ((c1 == -1 || truncate_set1) ? 1 : 0), __extension__ ({ if (c1 == -1 || truncate_set1) ; else __assert_fail ("c1 == -1 || truncate_set1", "tr.c", 1924, __extension__ __PRETTY_FUNCTION__); }));
        }
      if (squeeze_repeats)
        {
          set_initialize (s2, 0, in_squeeze_set);
