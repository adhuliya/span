...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
     ...
     fputs_unlocked (gettext ("  -c, --bytes=K            output the last K bytes; or use -c +K to output\n                             bytes starting with the Kth of each file\n"),stdout);
     ...
     printf (gettext ("  -n, --lines=K            output the last K lines, instead of the last %d;\n                             or use -n +K to output starting with the Kth\n      --max-unchanged-stats=N\n                           with --follow=name, reopen a FILE which has not\n                             changed size after N (default %d) iterations\n                             to see if it has been unlinked or renamed\n                             (this is the usual case of rotated log files);\n                             with inotify, this option is rarely useful\n"),
# 285 "tail.c"
             10,
             5
             );
...
close_fd (int fd, const char *filename)
{
  if (fd != -1 && fd != 0 && close (fd))
    {
      error (0, (*__errno_location ()), gettext ("closing %s (fd=%d)"), filename, fd);
    }
}
  ...
  if (n_bytes > 0 && fwrite_unlocked (buffer,1,n_bytes,stdout) < n_bytes)
    {
      ...
      error (1, (*__errno_location ()), gettext ("error writing %s"),
      ...
      if (bytes_read == ((size_t) -1))
        {
          if ((*__errno_location ()) != 11)
            error (1, (*__errno_location ()), gettext ("error reading %s"),
            ...
            off_t start_pos, off_t end_pos, uintmax_t *read_pos)
{
  ...
  if (bytes_read == ((size_t) -1))
    {
      error (0, (*__errno_location ()), gettext ("error reading %s"), quote (pretty_filename));
      return 0;
    }
    ...
    if (total_lines > n_lines)
      {
        ...
        for (j = total_lines - n_lines; j; --j)
          {
            beg = memchr (beg, '\n', buffer_end - beg);
            ((void) sizeof ((beg) ? 1 : 0), __extension__ ({ if (beg) ; else __assert_fail ("beg", "tail.c", 677, __extension__ __PRETTY_FUNCTION__); }));
            ++beg;
          }
      }
  ...
  if (err != 0)
    {
      ...
      if ((*__errno_location ()) != 38)
        error (0, (*__errno_location ()), gettext ("cannot determine location of %s. " "reverting to polling"), quote (name));
  ...
  else
    {
      switch (is_local_fs_type (buf.f_type))
        ...
        case -1:
          {
            ...
            error (0, 0, gettext ("unrecognized file system type 0x%08lx for %s. " "please report this to %s. reverting to polling"),

                   fs_type, quote (name), "bug-coreutils@gnu.org");
        ...
        case 1:
          ...
          break;
        default:
          ((void) sizeof ((!"unexpected return value from is_local_fs_type") ? 1 : 0), __extension__ ({ if (!"unexpected return value from is_local_fs_type") ; else __assert_fail ("!\"unexpected return value from is_local_fs_type\"", "tail.c", 915, __extension__ __PRETTY_FUNCTION__); }));
        }
    }
...
recheck (struct File_spec *f, _Bool blocking)
{
  ...
  ((void) sizeof ((valid_file_spec (f)) ? 1 : 0), __extension__ ({ if (valid_file_spec (f)) ; else __assert_fail ("valid_file_spec (f)", "tail.c", 942, __extension__ __PRETTY_FUNCTION__); }));
  ...
  if (! disable_inotify && ! lstat (f->name, &new_stats)
      && ((((new_stats.st_mode)) & 0170000) == (0120000)))
    {
      ...
      error (0, 0, gettext ("%s has been replaced with a symbolic link. " "giving up on this name"), quote (pretty_name (f)));
  ...
  else if (fd == -1 || fstat (fd, &new_stats) < 0)
    {
      ...
      if (!f->tailable)
        ...
        {
          error (0, (*__errno_location ()), "%s", pretty_name (f));
        }
    }
  else if (!(((((new_stats.st_mode)) & 0170000) == (0100000)) || ((((new_stats.st_mode)) & 0170000) == (0010000)) || ((((new_stats.st_mode)) & 0170000) == (0140000)) || ((((new_stats.st_mode)) & 0170000) == (0020000))))
    {
      ok = 0;
      f->errnum = -1;
      error (0, 0, gettext ("%s has been replaced with an untailable file; giving up on this name"),
  ...
  else if (!disable_inotify && fremote (fd, pretty_name (f)))
    {
      ok = 0;
      f->errnum = -1;
      error (0, 0, gettext ("%s has been replaced with a remote file. " "giving up on this name"), quote (pretty_name (f)));
  ...
  if (!ok)
    ...
    {
      new_file = 1;
      ((void) sizeof ((f->fd == -1) ? 1 : 0), __extension__ ({ if (f->fd == -1) ; else __assert_fail ("f->fd == -1", "tail.c", 1019, __extension__ __PRETTY_FUNCTION__); }));
      error (0, 0, gettext ("%s has become accessible"), quote (pretty_name (f)));
    }
  else if (f->fd == -1)
    {
...
tail_forever (struct File_spec *f, size_t n_files, double sleep_interval)
{
  ...
  _Bool blocking = (pid == 0 && follow_mode == Follow_descriptor
                   && n_files == 1 && ! ((((f[0].mode)) & 0170000) == (0100000)));
  size_t last;
      ...
      for (i = 0; i < n_files; i++)
        {
                      ...
                      && rpl_fcntl (fd, 4, new_flags) == -1))
                {
                  ...
                  if (((((f[i].mode)) & 0170000) == (0100000)) && (*__errno_location ()) == 1)
                    ...
                    error (1, (*__errno_location ()),
                           gettext ("%s: cannot change nonblocking mode"), name);
                }
              else
          ...
          if (!f[i].blocking)
            {
              ...
              if (((((mode)) & 0170000) == (0100000)) && stats.st_size < f[i].size)
                {
                  error (0, 0, gettext ("%s: file truncated"), name);
          ...
          bytes_read = dump_remainder (name, fd,
                                       (f[i].blocking
                                        ? ((18446744073709551615UL) - 1) : (18446744073709551615UL)));
...
check_fspec (struct File_spec *fspec, struct File_spec **prev_fspec)
{
  ...
  uintmax_t bytes_read = dump_remainder (name, fspec->fd, (18446744073709551615UL));
          ...
          if (f[i].wd < 0)
            {
              ...
              if ((*__errno_location ()) == 28 || (*__errno_location ()) == 12)
                ...
                error (0, (*__errno_location ()), gettext ("cannot watch %s"), quote (f[i].name));
              continue;
            }
  ...
  while (1)
    {
      ...
      if (pid)
        {
          ...
          if (writer_is_dead)
           ...
           int file_change = select (wd + 1, &rfd, ((void*)0), ((void*)0), &delay);

           if (file_change == 0)
             continue;
           else if (file_change == -1)
             error (1, (*__errno_location ()), gettext ("error monitoring inotify event"));
        }
          ...
          if (new_wd < 0)
            {
              if ((*__errno_location ()) == 28 || (*__errno_location ()) == 12)
              ...
              else
                {
                  ...
                  error (0, (*__errno_location ()), gettext ("cannot watch %s"), quote (f[j].name));
                }

            }


          _Bool new_watch = fspec->wd < 0 || new_wd != fspec->wd;
      ...
      if (ev->mask & (0x00000004 | 0x00000400 | 0x00000800))
        {
            ...
            uintmax_t *read_pos)
{
  ...
  if (fstat (fd, &stats))
    {
      error (0, (*__errno_location ()), gettext ("cannot fstat %s"), quote (pretty_filename));
      return 0;
    }
  ...
  if (from_start)
    {
      if ( ! presume_input_pipe
           && ((((stats.st_mode)) & 0170000) == (0100000)) && n_bytes <= ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))))
        {
          xlseek (fd, n_bytes, 1, pretty_filename);
          *read_pos += n_bytes;
        }
      else
        {
  ...
  else
    {
      off_t end_pos = ((! presume_input_pipe && usable_st_size (&stats)
                       ...
                       ? stats.st_size : -1);
      if (end_pos <= ((0 < (stats).st_blksize && (stats).st_blksize <= ((size_t)-1) / 8 + 1) ? (stats).st_blksize : 512))
        return pipe_bytes (pretty_filename, fd, n_bytes, read_pos);
      off_t current_pos = xlseek (fd, 0, 1, pretty_filename);
  ...
  if (from_start)
    {
      ...
      if (t)
        return t < 0;
      *read_pos += dump_remainder (pretty_filename, fd, (18446744073709551615UL));
    }
  else
    {
      off_t start_pos = -1;
          ...
          if (fstat (fd, &stats) < 0)
            {
              ok = 0;
              f->errnum = (*__errno_location ());
              error (0, (*__errno_location ()), gettext ("error reading %s"), quote (pretty_name (f)));
            }
            ...
            {
              error (0, 0, gettext ("%s: cannot follow end of this type of file; giving up on this name"),
...
parse_obsolete_option (int argc, char * const *argv, uintmax_t *n_units)
{
  ...
  switch (*p)
    {
    case 'b': default_count *= 512;
    case 'c': t_count_lines = 0;
    case 'l': p++; break;
    }
  ...
  if (n_string == n_string_end)
    *n_units = default_count;
           ...
           != LONGINT_OK)
    {
      error (1, (*__errno_location ()), "%s: %s", gettext ("invalid number"),
             quote (argv[1]));
    }
      ...
      switch (c)
        ...
        case LONG_FOLLOW_OPTION:
          ...
          if (optarg == ((void*)0))
            ...
            follow_mode = ((follow_mode_map) [__xargmatch_internal ("--follow", optarg, follow_mode_string, (char const *) (follow_mode_map), sizeof *(follow_mode_map), argmatch_die)]);
        ...
        case 's':
          {
            double s;
            if (! (xstrtod (optarg, ((void*)0), &s, c_strtod) && 0 <= s))
              error (1, 0,
                     gettext ("invalid number of seconds: %s"), quote (optarg));
            *sleep_interval = s;
          }
          break;
        ...
        case '5': case '6': case '7': case '8': case '9':
          error (1, 0,
                 gettext ("option used in invalid context -- %c"), c);
...
main (int argc, char **argv)
{
...

  {
    ...
    if (found_hyphen && follow_mode == Follow_name)
      error (1, 0, gettext ("cannot follow %s by name"), quote ("-"));




    if (forever && found_hyphen && isatty (0))
