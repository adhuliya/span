...
void __attribute__ ((__pure__)) __attribute__ ((__const__))
chopt_free (struct Chown_option *);

char *
gid_to_name (gid_t);

char * __attribute__ ((__pure__))
uid_to_name (uid_t);
...
chopt_init (struct Chown_option *chopt)
{
  ...
  chopt->affect_symlink_referent = 1;
  chopt->recurse = 0;
...
extern void
chopt_free (struct Chown_option *chopt __attribute__ ((__unused__)))
{
...
gid_to_name (gid_t gid)
{
  ...
  return xstrdup (grp ? grp->gr_name
                  : (! ((gid_t) 0 < (gid_t) -1)) ? imaxtostr (gid, buf)
                  : umaxtostr (gid, buf));
}
...
uid_to_name (uid_t uid)
{
  ...
  return xstrdup (pwd ? pwd->pw_name
                  : (! ((uid_t) 0 < (uid_t) -1)) ? imaxtostr (uid, buf)
                  : umaxtostr (uid, buf));
}
                 ...
                 char const *user, char const *group)
{
  ...
  old_spec = user_group_str (user ? old_user : ((void*)0), group ? old_group : ((void*)0));
                  ...
                  uid_t required_uid, gid_t required_gid)
{
  ...
  if (! (0 <= fd
         || ((*__errno_location ()) == 13 && ((((orig_st->st_mode)) & 0170000) == (0100000))
                   ...
                   struct Chown_option const *chopt)
{
  ...
  if (!ok)
  ...
  else
    {
      ...
      if (chopt->affect_symlink_referent && ((((file_stats->st_mode)) & 0170000) == (0120000)))
        {
          if (fstatat (fts->fts_cwd_fd, file, &stat_buf, 0) != 0)
            {
              if (! chopt->force_silent)
                error (0, (*__errno_location ()), gettext ("cannot dereference %s"),
                       quote (file_full_name));
              ok = 0;
            }
...
# 465 "chown-core.c"
      if (do_chown && !ok && ! chopt->force_silent)
        error (0, (*__errno_location ()), (uid != (uid_t) -1
      ...
      if (changed || chopt->verbosity == V_high)
        {
          enum Change_status ch_status =
            (!ok ? CH_FAILED
             ...
             : CH_SUCCEEDED);
          char *old_usr = file_stats ? uid_to_name (file_stats->st_uid) : ((void*)0);
          char *old_grp = file_stats ? gid_to_name (file_stats->st_gid) : ((void*)0);
             ...
             struct Chown_option const *chopt)
{
  ...
  FTS *fts = xfts_open (files, bit_flags | stat_flags, ((void*)0));
