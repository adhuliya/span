...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Print user and group information for the specified USER,\nor (when USER omitted) for the current user.\n\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "agnruzGZ", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case 'Z':
          ...
          if (!selinux_enabled)
            error (1, 0,
                   gettext ("--context (-Z) works only on an SELinux-enabled kernel"));
  ...
  _Bool default_format = (just_user + just_group + just_group_list
                         + just_context == 0);

  if (default_format && (use_real || use_name))
    error (1, 0,
           gettext ("cannot print only names or real IDs in default format"));
  ...
  if (n_ids == 1)
    {
      ...
      if (pwd == ((void*)0))
        error (1, 0, gettext ("%s: no such user"), spec);
