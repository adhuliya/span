...
# 30 "tee.c" 2
# 39 "tee.c"
static _Bool tee_files (int nfiles, const char **files);


static _Bool append;


static _Bool ignore_interrupts;
...
{
  {"append", 0, ((void*)0), 'a'},
  {"ignore-interrupts", 0, ((void*)0), 'i'},
  {"output-error", 2, ((void*)0), 'p'},
...
{
  "warn", "warn-nopipe", "exit", "exit-nopipe", ((void*)0)
};
...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (output_error_args) / sizeof *(output_error_args)) == (sizeof (output_error_types) / sizeof *(output_error_types)) + 1) ? 1 : -1; }))];
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Copy standard input to each FILE, and also to standard output.\n\n  -a, --append              append to the given FILEs, do not overwrite\n  -i, --ignore-interrupts   ignore interrupt signals\n"),stdout);





      fputs_unlocked (gettext ("  -p, --output-error[=MODE]  behavior on write error.  See MODE details below\n"),stdout);
  ...
  while ((optc = getopt_long (argc, argv, "aip", long_options, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case 'p':
          if (optarg)
            output_error = ((output_error_types) [__xargmatch_internal ("--output-error", optarg, output_error_args, (char const *) (output_error_types), sizeof *(output_error_types), argmatch_die)]);
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "tee", "GNU coreutils", Version, ("Mike Parker"), ("Richard M. Stallman"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
...
tee_files (int nfiles, const char **files)
{
  ...
  files[0] = gettext ("standard output");
  ...
  for (i = 1; i <= nfiles; i++)
    {
      ...
      descriptors[i] = fopen_safer (files[i], mode_string);
      if (descriptors[i] == ((void*)0))
        {
          error (output_error == output_error_exit
                 || output_error == output_error_exit_nopipe,
                 (*__errno_location ()), "%s", files[i]);
          ok = 0;
        }
      else
        {
          setvbuf (descriptors[i], ((void*)0), 2, 0);
          n_outputs++;
        }
    }
  ...
  while (n_outputs)
    {
      ...
      for (i = 0; i <= nfiles; i++)
        if (descriptors[i]
            && fwrite_unlocked (buffer,bytes_read,1,descriptors[i]) != 1)
          {
            int w_errno = (*__errno_location ());
            _Bool fail = (*__errno_location ()) != 32 || (output_error == output_error_exit
  ...
  for (i = 1; i <= nfiles; i++)
    if (descriptors[i] && rpl_fclose (descriptors[i]) != 0)
      {
        error (0, (*__errno_location ()), "%s", files[i]);
        ok = 0;
      }
