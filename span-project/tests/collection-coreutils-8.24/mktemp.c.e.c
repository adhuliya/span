...
main (int argc, char **argv)
{
  ...
  if (suffix)
    {
      ...
      if (!len || template[len - 1] != 'X')
        {
          error (1, 0,
                 gettext ("with --suffix, template %s must end in X"),
                 quote (template));
        }
  ...
  if (x_count < 3)
    error (1, 0, gettext ("too few X's in template %s"), quote (template));
      ...
      if (deprecated_t_option)
        {
          ...
          if (last_component (template) != template)
            error (1, 0,
                   gettext ("invalid template, %s, contains directory separator"),
                   quote (template));
        }
      else
        {
          ...
          if (((((template)[0]) == '/') || 0 != 0))
            error (1, 0,
                   gettext ("invalid template, %s; with --tmpdir," " it may not be absolute"),
