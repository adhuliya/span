...
create_hole (int fd, char const *name, _Bool punch_holes, off_t size)
{
  ...
  if (file_end < 0)
    {
      error (0, (*__errno_location ()), gettext ("cannot lseek %s"), quote (name));
      return 0;
    }
  ...
  if (punch_holes && punch_hole (fd, file_end - size, size) < 0)
    {
      error (0, (*__errno_location ()), gettext ("error deallocating %s"), quote (name));
      return 0;
    }
      ...
      if (n_read < 0)
        {
          if ((*__errno_location ()) == 4)
            continue;
          error (0, (*__errno_location ()), gettext ("error reading %s"), quote (src_name));
          return 0;
        }
      if (n_read == 0)
        break;
              ...
              if (! prev_hole)
                {
                  if (full_write (dest_fd, pbuf, psize) != psize)
                    {
                      error (0, (*__errno_location ()), gettext ("error writing %s"), quote (dst_name));
                      return 0;
                    }
                }
              else
          ...
          if (ext_hole_size)
            {
              if (lseek (src_fd, ext_start, 0) < 0)
                {
                  error (0, (*__errno_location ()), gettext ("cannot lseek %s"), quote (src_name));
                fail:
              ...
              if ((empty_extent && sparse_mode == SPARSE_ALWAYS)
              ...
              else
                {
                  ...
                  if (! write_zeros (dest_fd, nzeros))
                    {
                      error (0, (*__errno_location ()), gettext ("%s: write failed"), quote (dst_name));
                      goto fail;
                    }
...
# 542 "copy.c"
  if ((dest_pos < src_total_size || wrote_hole_at_eof)
    ...
    {
      error (0, (*__errno_location ()), gettext ("failed to extend %s"), quote (dst_name));
      return 0;
    }
  ...
  if (sparse_mode == SPARSE_ALWAYS && dest_pos < src_total_size
    ...
    {
      error (0, (*__errno_location ()), gettext ("error deallocating %s"), quote (dst_name));
      return 0;
    }
      ...
      if (! make_holes)
        {
          ...
          size_t blcm_max = ((((18446744073709551615UL))<(9223372036854775807L))?((18446744073709551615UL)):(9223372036854775807L)) - buf_alignment_slop;
...
close_src_and_dst_desc:
  if (close (dest_desc) < 0)
    {
      error (0, (*__errno_location ()), gettext ("failed to close %s"), quote (dst_name));
      return_val = 0;
    }
close_src_desc:
  if (close (source_desc) < 0)
    {
      error (0, (*__errno_location ()), gettext ("failed to close %s"), quote (src_name));
      return_val = 0;
    }
...
abandon_move (const struct cp_options *x,
              ...
              struct stat const *dst_sb)
{
  ((void) sizeof ((x->move_mode) ? 1 : 0), __extension__ ({ if (x->move_mode) ; else __assert_fail ("x->move_mode", "copy.c", 1730, __extension__ __PRETTY_FUNCTION__); }));
                  ...
                  _Bool replace, _Bool verbose, _Bool dereference)
{
  ...
  _Bool link_failed = (linkat (-100, src_name, -100, dst_name, flags)
                      != 0);



  if (link_failed && replace && (*__errno_location ()) == 17)
    {
      if (unlink (dst_name) != 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot remove %s"), quote (dst_name));
      ...
      if (verbose)
        printf (gettext ("removed %s\n"), quote (dst_name));
      link_failed = (linkat (-100, src_name, -100, dst_name, flags)
                     != 0);
    }
...
# 2596 "copy.c"
           ...
           && !(! 1 && ((((src_mode)) & 0170000) == (0120000))
                && x->dereference == DEREF_NEVER))
    {
      if (! create_hard_link (src_name, dst_name, 0, 0, dereference))
        goto un_backup;
    }
...
{
  ((void) sizeof ((co != ((void*)0)) ? 1 : 0), __extension__ ({ if (co != ((void*)0)) ; else __assert_fail ("co != NULL", "copy.c", 2885, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((((unsigned int) (co->backup_type) <= numbered_backups)) ? 1 : 0), __extension__ ({ if (((unsigned int) (co->backup_type) <= numbered_backups)) ; else __assert_fail ("VALID_BACKUP_TYPE (co->backup_type)", "copy.c", 2886, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((((co->sparse_mode) == SPARSE_NEVER || (co->sparse_mode) == SPARSE_AUTO || (co->sparse_mode) == SPARSE_ALWAYS)) ? 1 : 0), __extension__ ({ if (((co->sparse_mode) == SPARSE_NEVER || (co->sparse_mode) == SPARSE_AUTO || (co->sparse_mode) == SPARSE_ALWAYS)) ; else __assert_fail ("VALID_SPARSE_MODE (co->sparse_mode)", "copy.c", 2887, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((((co->reflink_mode) == REFLINK_NEVER || (co->reflink_mode) == REFLINK_AUTO || (co->reflink_mode) == REFLINK_ALWAYS)) ? 1 : 0), __extension__ ({ if (((co->reflink_mode) == REFLINK_NEVER || (co->reflink_mode) == REFLINK_AUTO || (co->reflink_mode) == REFLINK_ALWAYS)) ; else __assert_fail ("VALID_REFLINK_MODE (co->reflink_mode)", "copy.c", 2888, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((!(co->hard_link && co->symbolic_link)) ? 1 : 0), __extension__ ({ if (!(co->hard_link && co->symbolic_link)) ; else __assert_fail ("!(co->hard_link && co->symbolic_link)", "copy.c", 2889, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((! (co->reflink_mode == REFLINK_ALWAYS && co->sparse_mode != SPARSE_AUTO)) ? 1 : 0), __extension__ ({ if (! (co->reflink_mode == REFLINK_ALWAYS && co->sparse_mode != SPARSE_AUTO)) ; else __assert_fail ("! (co->reflink_mode == REFLINK_ALWAYS && co->sparse_mode != SPARSE_AUTO)", "copy.c", 2892, __extension__ __PRETTY_FUNCTION__); }));
...
copy (char const *src_name, char const *dst_name,
      ...
      _Bool *copy_into_self, _Bool *rename_succeeded)
{
  ((void) sizeof ((valid_options (options)) ? 1 : 0), __extension__ ({ if (valid_options (options)) ; else __assert_fail ("valid_options (options)", "copy.c", 2910, __extension__ __PRETTY_FUNCTION__); }));
# 2919 "copy.c"
