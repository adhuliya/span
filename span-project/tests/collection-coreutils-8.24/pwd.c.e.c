                ...
                size_t parent_height)
{
  ...
  dirp = opendir ("..");
  if (dirp == ((void*)0))
    error (1, (*__errno_location ()), gettext ("cannot open directory %s"),
           quote (nth_parent (parent_height)));

  fd = dirfd (dirp);
  if ((0 <= fd ? fchdir (fd) : chdir ("..")) < 0)
    error (1, (*__errno_location ()), gettext ("failed to chdir to %s"),
           quote (nth_parent (parent_height)));

  if ((0 <= fd ? fstat (fd, &parent_sb) : stat (".", &parent_sb)) < 0)
    error (1, (*__errno_location ()), gettext ("failed to stat %s"),
           quote (nth_parent (parent_height)));
  ...
  if (dirp == ((void*)0) || closedir (dirp) != 0)
    {
      ...
      error (1, (*__errno_location ()), gettext ("reading directory %s"),
             quote (nth_parent (parent_height)));
    }
...
robust_getcwd (struct file_name *file_name)
{
  ...
  if (stat (".", &dot_sb) < 0)
    error (1, (*__errno_location ()), gettext ("failed to stat %s"), quote ("."));
