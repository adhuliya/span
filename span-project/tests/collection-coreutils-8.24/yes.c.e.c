...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "yes", "GNU coreutils", Version,
                      usage, ("David MacKenzie"), (char const *) ((void*)0));
  ...
  while (1 )
    {
      int j;
      if ((pbuf - buf) && fwrite_unlocked (buf,pbuf - buf,1,stdout) != 1)
        {
          error (0, (*__errno_location ()), gettext ("standard output"));
          return 1;
        }
      for (j = i; j < argc; j++)
        if (fputs_unlocked (argv[j],stdout) == (-1)
            || putchar_unlocked (j == argc - 1 ? '\n' : ' ') == (-1))
          {
            error (0, (*__errno_location ()), gettext ("standard output"));
            return 1;
          }
