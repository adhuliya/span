...
enum
{
  IGNORE_FAIL_ON_NON_EMPTY_OPTION = 127 + 1
};
...
static struct option const longopts[] =
{
  ...
  {"ignore-fail-on-non-empty", 0, ((void*)0),
  ...
  while (1)
    {
      ...
      if (verbose)
        prog_fprintf (stdout, gettext ("removing directory, %s"), quote (dir));
      ...
      if (!ok)
        {
          ...
          if (ignorable_failure ((*__errno_location ()), dir))
            {
              ok = 1;
            }
          else
            {
              ...
              error (0, (*__errno_location ()), gettext ("failed to remove directory %s"),
                     quote (dir));
            }
          break;
        }
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -p, --parents   remove DIRECTORY and its ancestors; e.g., 'rmdir -p a/b/c' is\n                    similar to 'rmdir a/b/c a/b a'\n  -v, --verbose   output a diagnostic for every directory processed\n"),stdout);
  ...
  while ((optc = getopt_long (argc, argv, "pv", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "rmdir", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
      ...
      if (rmdir (dir) != 0)
        {
          ...
          error (0, (*__errno_location ()), gettext ("failed to remove %s"), quote (dir));
          ok = 0;
        }
