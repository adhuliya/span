...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Synchronize cached writes to persistent storage\n\nIf one or more files are specified, sync only them,\nor their containing file systems.\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -d, --data             sync only file data, no unneeded metadata\n"),stdout);


      fputs_unlocked (gettext ("  -f, --file-system      sync the file systems that contain the files\n"),stdout);
...
# 105 "sync.c"
  ...
  if (fd < 0)
    {
      ...
      if (fd < 0)
        error (0, rd_errno, gettext ("error opening %s"), quote (file));
      return 0;
    }
  ...
  if (fdflags == -1
      || rpl_fcntl (fd, 4, fdflags & ~04000) < 0)
    {
      error (0, (*__errno_location ()), gettext ("couldn't reset non-blocking mode %s"), quote (file));
      ret = 0;
    }
  ...
  if (ret == 1)
    {
      ...
      switch (mode)
        ...
        default:
          ((void) sizeof (("invalid sync_mode") ? 1 : 0), __extension__ ({ if ("invalid sync_mode") ; else __assert_fail ("\"invalid sync_mode\"", "sync.c", 149, __extension__ __PRETTY_FUNCTION__); }));
        }

      if (sync_status < 0)
        {
          error (0, (*__errno_location ()), gettext ("error syncing %s"), quote (file));
          ret = 0;
        }
    }
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "df", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "sync", "GNU coreutils", Version, ("Jim Meyering"), ("Giuseppe Scrivano"), (char *) ((void*)0)); exit (0); break;;
