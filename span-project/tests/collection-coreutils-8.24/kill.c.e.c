...
static int
list_signals (_Bool table, char *const *argv)
{
  int signum;
  int status = 0;
  ...
  if (table)
    {
      ...
      for (signum = 1; signum <= ((64 + 1) - 1) / 10; signum *= 10)
        num_width++;


      for (signum = 1; signum <= ((64 + 1) - 1); signum++)
...
send_signals (int signum, char *const *argv)
{
  ...
  do
    {
      char *endp;
      intmax_t n = ((*__errno_location ()) = 0, strtoimax (arg, &endp, 10));
      pid_t pid = n;

      if ((*__errno_location ()) == 34 || pid != n || arg == endp || *endp)
        {
          error (0, 0, gettext ("%s: invalid process id"), arg);
          status = 1;
        }
      else if (kill (pid, signum) != 0)
        {
          error (0, (*__errno_location ()), "%s", arg);
          status = 1;
        }
    }
  while ((arg = *++argv));
  ...
  while ((optc = getopt_long (argc, argv, short_options, long_options, ((void*)0)))
    ...
    switch (optc)
      ...
      case 's':
        if (0 <= signum)
          {
            error (0, 0, gettext ("%s: multiple signals specified"), optarg);
            usage (1);
          }
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "kill", "GNU coreutils", Version, ("Paul Eggert"), (char *) ((void*)0)); exit (0); break;;
      default:
        usage (1);
      }
 no_more_options:
  ...
  return (list
          ? list_signals (table, optind < argc ? argv + optind : ((void*)0))
