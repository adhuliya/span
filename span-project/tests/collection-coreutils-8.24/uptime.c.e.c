...
# 23 "/usr/include/x86_64-linux-gnu/sys/sysctl.h" 3 4
# 1 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 1 3 4
# 24 "/usr/include/x86_64-linux-gnu/sys/sysctl.h" 2 3 4
      ...
      if (b == buf)
        {
          ...
          if (buf != end_ptr)
            uptime = (0 <= upsecs && upsecs < ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1)))
                      ? upsecs : -1);
        }
...
# 107 "uptime.c"
  while (n--)
    {
      entries += (((this)->ut_user)[0] && (((this)->ut_type == (7)) || (0 && ((this)->ut_tv.tv_sec) != 0)));
      if (((this)->ut_type == (2)))
  ...
  if (uptime == 0)
    ...
    {
      if (boot_time == 0)
        error (1, (*__errno_location ()), gettext ("couldn't get boot time"));
  ...
  if (tmn)
    ...
    fprintftime (stdout, gettext (" %H:%M%P  "), tmn, 0, 0);
  else
    printf (gettext (" ??:????  "));
  if (uptime == (time_t) -1)
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      printf (gettext ("Print the current time, the length of time the system has been up,\nthe number of users on the system, and the average number of jobs\nin the run queue over the last 1, 5 and 15 minutes."));
# 208 "uptime.c"
      printf (gettext ("  Processes in\nan uninterruptible sleep state also contribute to the load average.\n"));





      printf (gettext ("If FILE is not specified, use %s.  %s as FILE is common.\n\n"),
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "uptime", "GNU coreutils", Version,
                      usage, ("Joseph Arceneaux"), ("David MacKenzie"), ("Kaveh Ghazi"), (char const *) ((void*)0));
  ...
  switch (argc - optind)
    {
    case 0:
      uptime ("/var/run/utmp", READ_UTMP_CHECK_PIDS);
      break;
