...
_Bool parse_datetime (struct timespec *, char const *, struct timespec const *);
# 31 "touch.c" 2
# 1 "../lib/posixtm.h" 1
# 25 "../lib/posixtm.h"
# 1 "../lib/time.h" 1
             ...
             char const *flex_date, struct timespec const *now)
{
  if (! parse_datetime (result, flex_date, now))
    error (1, 0, gettext ("invalid date format %s"), quote (flex_date));
}
  ...
  if (change_times != (1 | 2))
    {
      ...
      if (change_times == 2)
        ...
        {
          ((void) sizeof ((change_times == 1) ? 1 : 0), __extension__ ({ if (change_times == 1) ; else __assert_fail ("change_times == CH_ATIME", "touch.c", 152, __extension__ __PRETTY_FUNCTION__); }));
  ...
  if (fd == 0)
    {
      if (close (0) != 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to close %s"), quote (file));
          return 0;
        }
    }
  else if (fd == 1)
      ...
      if (open_errno)
        {
          ...
          error (0, open_errno, gettext ("cannot touch %s"), quote (file));
        }
      else
        {
          if (no_create && (*__errno_location ()) == 2)
            return 1;
          error (0, (*__errno_location ()), gettext ("setting times of %s"), quote (file));
        }
      return 0;
    }
      ...
      switch (c)
        ...
        case 't':
          if (! posixtime (&newtime[0].tv_sec, optarg,
                           1 | 4 | 8))
            error (1, 0, gettext ("invalid date format %s"),
                   quote (optarg));
          newtime[0].tv_nsec = 0;
          newtime[1] = newtime[0];
          date_set = 1;
        ...
        case TIME_OPTION:
          change_times |= ((time_masks) [__xargmatch_internal ("--time", optarg, time_args, (char const *) (time_masks), sizeof *(time_masks), argmatch_die)]);
  ...
  if (use_ref)
    {
      ...
      if (no_dereference ? lstat (ref_file, &ref_stats)
        ...
        error (1, (*__errno_location ()),
               gettext ("failed to get attributes of %s"), quote (ref_file));
