...
scan_arg (const char *arg)
{
  ...
  if (! xstrtold (arg, ((void*)0), &ret.value, c_strtold))
    {
      error (0, 0, gettext ("invalid floating point argument: %s"), arg);
      usage (1);
    }
...
long_double_format (char const *fmt, struct layout *layout)
{
  ...
  if (! strchr ("efgaEFGA", fmt[i]))
    error (1, 0,
           gettext ("format %s has unknown %%%c directive"), quote (fmt), fmt[i]);
  ...
  if (! out_of_range)
    {
          ...
          if (out_of_range)
            {
              ...
              if (xstrtold (x_str + layout.prefix_len, ((void*)0), &x_val, c_strtold)
                  && x_val == last)
                {
                  char *x0_str = ((void*)0);
                  if (rpl_asprintf (&x0_str, fmt, x0) < 0)
                    xalloc_die ();
      ...
      fputs_unlocked (terminator,stdout);
    }
}
      ...
      while (inf || cmp (p, p_len, q, q_len) < 0)
        {
          *bufp++ = *separator;
          ...
          if (buf_end - (p_len + 1) < bufp)
            {
              fwrite_unlocked (buf,bufp - buf,1,stdout);
              bufp = buf;
            }
        }
...
main (int argc, char **argv)
{
  ...
  if (all_digits_p (argv[optind])
      ...
      && (n_args < 3 || ((strcmp ("1", argv[optind + 1]) == 0)
  ...
  if ((((first.value) * 0 == 0) && first.precision == 0)
