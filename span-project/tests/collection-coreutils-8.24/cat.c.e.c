  ...
  while (1)
    {
      ...
      if (n_read == ((size_t) -1))
        {
          error (0, (*__errno_location ()), "%s", infile);
          return 0;
        }
  ...
  while ((c = getopt_long (argc, argv, "benstuvAET", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "cat", "GNU coreutils", Version, proper_name_utf8 ("Torbjorn Granlund", "Torbj\303\266rn Granlund"), ("Richard M. Stallman"), (char *) ((void*)0)); exit (0); break;;
