...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -f, --canonicalize            canonicalize by following every symlink in\n                                every component of the given name recursively;\n                                all but the last component must exist\n  -e, --canonicalize-existing   canonicalize by following every symlink in\n                                every component of the given name recursively,\n                                all components must exist\n"),stdout);
# 75 "readlink.c"
      fputs_unlocked (gettext ("  -m, --canonicalize-missing    canonicalize by following every symlink in\n                                every component of the given name recursively,\n                                without requirements on components existence\n  -n, --no-newline              do not output the trailing delimiter\n  -q, --quiet,\n  -s, --silent                  suppress most error messages\n  -v, --verbose                 report error messages\n  -z, --zero                    end each output line with NUL, not newline\n"),stdout);
# 86 "readlink.c"
  ...
  while ((optc = getopt_long (argc, argv, "efmnqsvz", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "readlink", "GNU coreutils", Version, ("Dmitry V. Levin"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  for (; optind < argc; ++optind)
    {
      ...
      if (value)
      ...
      else
        {
          status = 1;
          if (verbose)
            error (0, (*__errno_location ()), "%s", fname);
        }
    }
