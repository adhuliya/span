...
static _Bool test_unop (char const *s);
static _Bool unary_operator (void);
static _Bool binary_operator (_Bool);
static _Bool two_arguments (void);
static _Bool three_arguments (void);
static _Bool posixtest (int);

static _Bool expr (void);
static _Bool term (void);
static _Bool and (void);
static _Bool or (void);
...
find_int (char const *string)
{
  ...
  for (p = string; ((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISblank); p++)
    continue;
  ...
  if (((unsigned int) (*p++) - '0' <= 9))
    {
      while (((unsigned int) (*p) - '0' <= 9))
        p++;
      while (((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISblank))
        p++;
      if (!*p)
        return number_start;
    }
  ...
  if (argv[pos][0] == '(' && argv[pos][1] == '\0')
    {
      ...
      for (nargs = 1;
           pos + nargs < argc && ! (strcmp (argv[pos + nargs], ")") == 0);
           nargs++)
        if (nargs == 4)
          {
            nargs = argc - pos;
      ...
      if (argv[pos] == 0)
        test_syntax_error (gettext ("')' expected"), ((void*)0));
      else
        if (argv[pos][0] != ')' || argv[pos][1])
          test_syntax_error (gettext ("')' expected, found %s"), argv[pos]);
      advance (0);
    }


  else if (4 <= argc - pos && (strcmp (argv[pos], "-l") == 0) && binop (argv[pos + 2]))
  ...
  if (argv[op][0] == '-')
    {
          ...
          if (argv[op][2] == 't' && !argv[op][3])
            {
              ...
              pos += 3;
              if (l_is_l || r_is_l)
                test_syntax_error (gettext ("-nt does not accept -l"), ((void*)0));
          ...
          if (argv[op][2] == 'f' && !argv[op][3])
            {
              ...
              pos += 3;
              if (l_is_l || r_is_l)
                test_syntax_error (gettext ("-ef does not accept -l"), ((void*)0));
          ...
          if ('t' == argv[op][2] && '\000' == argv[op][3])
            {
              ...
              pos += 3;
              if (l_is_l || r_is_l)
                test_syntax_error (gettext ("-ot does not accept -l"), ((void*)0));
              le = get_mtime (argv[op - 1], &lt);
              re = get_mtime (argv[op + 1], &rt);
      ...
      test_syntax_error (gettext ("unknown binary operator"), argv[op]);
    }
...
unary_operator (void)
{
  ...
  switch (argv[pos][1])
    ...
    case 'r':
      unary_advance ();
      return euidaccess (argv[pos - 1], 4) == 0;

    case 'w':
      unary_advance ();
      return euidaccess (argv[pos - 1], 2) == 0;

    case 'x':
      unary_advance ();
      return euidaccess (argv[pos - 1], 1) == 0;
    ...
    case 'O':
      {
        ...
        uid_t euid = geteuid ();
        uid_t NO_UID = -1;
        return ! (euid == NO_UID && (*__errno_location ())) && euid == stat_buf.st_uid;
      }
    ...
    case 'G':
      {
        ...
        gid_t egid = getegid ();
        gid_t NO_GID = -1;
        return ! (egid == NO_GID && (*__errno_location ())) && egid == stat_buf.st_gid;
      }
    ...
    case 'f':
      ...
      return (stat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0100000)));
    ...
    case 'S':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0140000)));

    case 'c':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0020000)));

    case 'b':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0060000)));

    case 'p':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0010000)));
    ...
    case 'h':
      ...
      return (lstat (argv[pos - 1], &stat_buf) == 0
              && ((((stat_buf.st_mode)) & 0170000) == (0120000)));

    case 'u':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && (stat_buf.st_mode & 04000));

    case 'g':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && (stat_buf.st_mode & 02000));

    case 'k':
      unary_advance ();
      return (stat (argv[pos - 1], &stat_buf) == 0
              && (stat_buf.st_mode & 01000));
    ...
    case 't':
      {
        ...
        (*__errno_location ()) = 0;
        fd = strtol (arg, ((void*)0), 10);
        return ((*__errno_location ()) != 34 && 0 <= fd && fd <= 2147483647 && isatty (fd));
      }
...
or (void)
{
  ...
  while (1)
    {
      value |= and ();
      if (! (pos < argc && (strcmp (argv[pos], "-o") == 0)))
        return value;
      advance (0);
    }
}
...
three_arguments (void)
{
  ...
  if (binop (argv[pos + 1]))
    ...
    test_syntax_error (gettext ("%s: binary operator expected"), argv[pos+1]);
  return (value);
}
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Exit with the status determined by EXPRESSION.\n\n"),stdout);
...
# 748 "test.c"
      ...
      fputs_unlocked (gettext ("  -f FILE     FILE exists and is a regular file\n  -g FILE     FILE exists and is set-group-ID\n  -G FILE     FILE exists and is owned by the effective group ID\n  -h FILE     FILE exists and is a symbolic link (same as -L)\n  -k FILE     FILE exists and has its sticky bit set\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -L FILE     FILE exists and is a symbolic link (same as -h)\n  -O FILE     FILE exists and is owned by the effective user ID\n  -p FILE     FILE exists and is a named pipe\n  -r FILE     FILE exists and read permission is granted\n  -s FILE     FILE exists and has a size greater than zero\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -S FILE     FILE exists and is a socket\n  -t FD       file descriptor FD is opened on a terminal\n  -u FILE     FILE exists and its set-user-ID bit is set\n  -w FILE     FILE exists and write permission is granted\n  -x FILE     FILE exists and execute (or search) permission is granted\n"),stdout);
      ...
      if (margc == 2)
        {
          ...
          if ((strcmp (margv[1], "--version") == 0))
            {
              version_etc (stdout, "test", "GNU coreutils", Version, ("Kevin Braunsdorf"), ("Matthew Bradburn"),
                           (char *) ((void*)0));
              return 0;
            }
        }
      if (margc < 2 || !(strcmp (margv[margc - 1], "]") == 0))
        test_syntax_error (gettext ("missing ']'"), ((void*)0));
