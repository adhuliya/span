...
extern _Bool base64_decode_ctx (struct base64_decode_context *ctx,
                               const char *__restrict in, size_t inlen,
                               char *__restrict out, size_t *outlen);

extern _Bool base64_decode_alloc_ctx (struct base64_decode_context *ctx,
...
# 98 "base64.c"
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((1024*3*10) % 12 == 0) ? 1 : -1; }))];
extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((1024*3) % 12 == 0) ? 1 : -1; }))];
  ...
  if (wrap_column == 0)
    {
      ...
      if (fwrite_unlocked (buffer,1,len,stdout) < len)
    ...
    for (written = 0; written < len;)
      {
        ...
        size_t to_write = (((cols_remaining)<((18446744073709551615UL)))?(cols_remaining):((18446744073709551615UL)));
        to_write = (((to_write)<(len - written))?(to_write):(len - written));
        ...
        if (to_write == 0)
          ...
          {
            if (fwrite_unlocked (buffer + written,1,to_write,stdout) < to_write)
              error (1, (*__errno_location ()), gettext ("write error"));
...
do_encode (FILE *in, FILE *out, uintmax_t wrap_column)
{
  ...
  char inbuf[(1024*3*10)];
  char outbuf[(((((1024*3*10)) + 2) / 3) * 4)];
  size_t sum;
...
static void
do_decode (FILE *in, FILE *out, _Bool ignore_garbage)
{
  char inbuf[(((((1024*3)) + 2) / 3) * 4)];
  char outbuf[(1024*3)];
  size_t sum;
  ...
  do
    {
      ...
      for (k = 0; k < 1 + !!feof_unlocked (in); k++)
        {
          if (k == 1 && ctx.i == 0)
            break;
          n = (1024*3);
    ...
    switch (opt)
      ...
      case 'w':
        if (xstrtoumax (optarg, ((void*)0), 0, &wrap_column, ((void*)0)) != LONGINT_OK)
          error (1, 0, gettext ("invalid wrap size: %s"),
                 quotearg (optarg));
        break;
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "base64", "GNU coreutils", Version, ("Simon Josefsson"), (char *) ((void*)0)); exit (0); break;;
