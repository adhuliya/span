...
# 27 "../lib/c-ctype.h" 2
# 120 "../lib/c-ctype.h"
extern _Bool c_isascii (int c) __attribute__ ((__const__));

extern _Bool c_isalnum (int c) __attribute__ ((__const__));
extern _Bool c_isalpha (int c) __attribute__ ((__const__));
extern _Bool c_isblank (int c) __attribute__ ((__const__));
extern _Bool c_iscntrl (int c) __attribute__ ((__const__));
extern _Bool c_isdigit (int c) __attribute__ ((__const__));
extern _Bool c_islower (int c) __attribute__ ((__const__));
extern _Bool c_isgraph (int c) __attribute__ ((__const__));
extern _Bool c_isprint (int c) __attribute__ ((__const__));
extern _Bool c_ispunct (int c) __attribute__ ((__const__));
extern _Bool c_isspace (int c) __attribute__ ((__const__));
extern _Bool c_isupper (int c) __attribute__ ((__const__));
extern _Bool c_isxdigit (int c) __attribute__ ((__const__));
...
# 32 "numfmt.c" 2
# 1 "../lib/gl_linked_list.h" 1
# 21 "../lib/gl_linked_list.h"
# 1 "../lib/gl_list.h" 1
# 22 "../lib/gl_list.h"
# 1 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 1 3
# 23 "../lib/gl_list.h" 2
# 107 "../lib/gl_list.h"
typedef _Bool (*gl_listelement_equals_fn) (const void *elt1, const void *elt2);
...
struct gl_list_implementation
{
  ...
  _Bool (*remove_elt) (gl_list_t list, const void *elt);
  ...
  gl_list_iterator_t (*iterator_from_to) (gl_list_t list,
                                          size_t start_index,
                                          size_t end_index);
  _Bool (*iterator_next) (gl_list_iterator_t *iterator,
  ...
  gl_list_node_t (*sortedlist_nx_add) (gl_list_t list,
                                       gl_listelement_compar_fn compar,
                                    const void *elt);
  _Bool (*sortedlist_remove) (gl_list_t list,
                             gl_listelement_compar_fn compar,
                             const void *elt);
};
                 ...
                 enum scale_type scale, int group, enum round_type round)
{
  int num_size;
  char fmt[64];
  extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (fmt) > ((((((sizeof (zero_padding_width) * 8 - (! ((__typeof__ (zero_padding_width)) 0 < (__typeof__ (zero_padding_width)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (zero_padding_width)) 0 < (__typeof__ (zero_padding_width)) -1))) + 1) + (((((sizeof (precision) * 8 - (! ((__typeof__ (precision)) 0 < (__typeof__ (precision)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (precision)) 0 < (__typeof__ (precision)) -1))) + 1) + 10 )) ? 1 : -1; }))];
...
unit_to_umax (const char *n_string)
{
  ...
  if (s_err != LONGINT_OK || *end || n == 0)
    {
      free (t_string);
      error (1, 0, gettext ("invalid unit size: %s"), quote (n_string));
    }
  ...
  if (fmt[i] == '.')
    {
      ...
      if ((*__errno_location ()) == 34 || user_precision < 0 || (18446744073709551615UL) < user_precision
          || ((*__ctype_b_loc ())[(int) ((fmt[i]))] & (unsigned short int) _ISblank) || fmt[i] == '+')
        {
          ...
          error (1, 0,
                 gettext ("invalid precision in format %s"), quote (fmt));
        }
      i = endptr - fmt;
    }

  if (fmt[i] != 'f')
    error (1, 0, gettext ("invalid format %s," " directive must be %%[0]['][-][N][.][N]f"),
  ...
  if (*start == '-')
    {
      ...
      if (start == end || all_fields_before <=0)
        error (1, 0, gettext ("invalid field value %s"),
               quote (start));

      return;
    }

  field_list = gl_list_create_empty (&gl_linked_list_implementation,
                                     ((void*)0), ((void*)0), free_field, 0);
    ...
    if (! range_val)
    ...
    else
      {
        ...
        if (field_val < range_val)
          error (1, 0, gettext ("invalid decreasing range"));
        rp = xmalloc (sizeof (*rp));
        rp->lo = range_val + 1;
        rp->hi = field_val;
        gl_sortedlist_add (field_list, sort_field, rp);
...
next_field (char **line)
{
  ...
  if (delimiter != DELIMITER_DEFAULT)
  ...
  else
    {
      ...
      while (*field_end && ((*__ctype_b_loc ())[(int) ((to_uchar (*field_end)))] & (unsigned short int) _ISblank))
        ++field_end;
...
main (int argc, char **argv)
{
  ...
  while (1)
    {
      int c = getopt_long (argc, argv, "d:", longopts, ((void*)0));
      ...
      switch (c)
        ...
        case PADDING_OPTION:
          if (xstrtol (optarg, ((void*)0), 10, &padding_width, "") != LONGINT_OK
              || padding_width == 0)
            error (1, 0, gettext ("invalid padding value %s"),
                   quote (optarg));
          if (padding_width < 0)
            {
              padding_alignment = MBS_ALIGN_LEFT;
        ...
        case FIELD_OPTION:
          if (all_fields || all_fields_before || all_fields_after || field_list)
            {
              error (1, 0,
                     gettext ("multiple field specifications"));
            }
          ...
          if (optarg)
            {
              if (xstrtoumax (optarg, ((void*)0), 10, &header, "") != LONGINT_OK
                  || header == 0)
                error (1, 0, gettext ("invalid header value %s"),
                       quote (optarg));
            }
          else
            {
  ...
  if (format_str != ((void*)0) && grouping)
    error (1, 0, gettext ("--grouping cannot be combined with --format"));
  ...
  if (grouping)
    {
      if (scale_to != scale_none)
        error (1, 0, gettext ("grouping cannot be combined with --to"));
