  ...
  if (specified_mode)
    {
      ...
      if (newmode & ~((0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3)))
        error (1, 0,
               gettext ("mode must specify only file permission bits"));
    }
    ...
    block_or_character:
      {
        ...
        if (xstrtoumax (s_major, ((void*)0), 0, &i_major, ((void*)0)) != LONGINT_OK
            || i_major != (unsigned int) i_major)
          error (1, 0,
                 gettext ("invalid major device number %s"), quote (s_major));

        if (xstrtoumax (s_minor, ((void*)0), 0, &i_minor, ((void*)0)) != LONGINT_OK
            || i_minor != (unsigned int) i_minor)
          error (1, 0,
                 gettext ("invalid minor device number %s"), quote (s_minor));

        device =
# 242 "mknod.c"
#pragma GCC warning "In the GNU C Library, \042makedev\042 is defined\012 by <sys/sysmacros.h>. For historical compatibility, it is\012 currently defined by <sys/types.h> as well, but we plan to\012 remove this soon. To use \042makedev\042, include <sys/sysmacros.h>\012 directly. If you did not intend to use a system-defined macro\012 \042makedev\042, you should undefine it after including <sys/types.h>."
# 242 "mknod.c"
 gnu_dev_makedev (i_major, i_minor);

        if (device == ((dev_t) -1))
          error (1, 0, gettext ("invalid device %s %s"), s_major, s_minor);
        ...
        if (mknod (argv[optind], newmode | node_type, device) != 0)
          error (1, (*__errno_location ()), "%s", quote (argv[optind]));
      }
      break;
  ...
  if (specified_mode && chmod (argv[optind], newmode) != 0)
    error (1, (*__errno_location ()), gettext ("cannot set permissions of %s"),
           quote (argv[optind]));
