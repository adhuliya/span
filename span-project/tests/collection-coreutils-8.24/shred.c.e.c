...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (0 < SECTOR_SIZE && (SECTOR_SIZE & SECTOR_MASK) == 0) ? 1 : -1; }))];
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -u, --remove[=HOW]  truncate and remove file after overwriting; See below\n  -v, --verbose  show progress\n  -x, --exact    do not round file sizes up to the next full block;\n                   this is the default for non-regular files\n  -z, --zero     add a final overwrite with zeros to hide shredding\n"),stdout);
...
# 208 "shred.c"
      fputs_unlocked (gettext ("CAUTION: Note that shred relies on a very important assumption:\nthat the file system overwrites data in place.  This is the traditional\nway to do things, but many modern file system designs do not satisfy this\nassumption.  The following are examples of file systems on which shred is\nnot effective, or is not guaranteed to be effective in all file system modes:\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("* log-structured or journaled file systems, such as those supplied with\nAIX and Solaris (and JFS, ReiserFS, XFS, Ext3, etc.)\n\n* file systems that write redundant data and carry on even if some writes\nfail, such as RAID-based file systems\n\n* file systems that make snapshots, such as Network Appliance's NFS server\n\n"),stdout);
# 226 "shred.c"
      fputs_unlocked (gettext ("* file systems that cache in temporary locations, such as NFS\nversion 3 clients\n\n* compressed file systems\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("In the case of ext3 file systems, the above disclaimer applies\n(and shred is thus of limited effectiveness) only in data=journal mode,\nwhich journals file data in addition to just metadata.  In both the\ndata=ordered (default) and data=writeback modes, shred works as usual.\nExt3 journaling modes can be changed by adding the data=something option\nto the mount options for a particular file system in the /etc/fstab file,\nas documented in the mount man page (man mount).\n\n"),stdout);
# 243 "shred.c"
      fputs_unlocked (gettext ("In addition, file system backups and remote mirrors may contain copies\nof the file that cannot be removed, and that will allow a shredded file\nto be recovered later.\n"),stdout);
...
static void
direct_mode (int fd, _Bool enable)
{
  if (040000)
    {
      int fd_flags = rpl_fcntl (fd, 3);
      if (0 < fd_flags)
        {
  ...
  if (((((st->st_mode)) & 0170000) == (0020000)))
    {
      ...
      op.mt_op = 6;
      op.mt_count = 1;
      if (ioctl (fd, (((1U) << (((0 +8)+8)+14)) | ((('m')) << (0 +8)) | (((1)) << 0) | ((((sizeof(struct mtop)))) << ((0 +8)+8))), &op) == 0)
        return 1;
        ...
        unsigned long int k, unsigned long int n)
{
  ...
  extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((60 * 1024) % 3 == 0) ? 1 : -1; }))];
  size_t output_size = periodic_pattern (type)
                       ? (60 * 1024) : (64 * 1024);



  void *fill_pattern_mem = xmalloc (((page_size - 1) + (((output_size + 2) / 3) * 3)));
  ...
  char previous_offset_buf[((2 * sizeof (uintmax_t) * 8 * 146 / 485 + 1) * (16 + 1) - 16 + 1 + 3) + 1];
  ...
  if (type >= 0)
    {
      lim = (0 <= size && size < (((output_size + 2) / 3) * 3) ? size : (((output_size + 2) / 3) * 3));
  ...
  while (1)
    {
      ...
      for (soff = 0; soff < lim; soff += ssize)
        {
          ...
          if (ssize <= 0)
            {
              if (size < 0 && (ssize == 0 || (*__errno_location ()) == 28))
                {
              ...
              else
                {
                  ...
                  extern int (*_gl_verify_function4 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((60 * 1024) % SECTOR_SIZE == 0) ? 1 : -1; }))];
                  extern int (*_gl_verify_function5 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((64 * 1024) % SECTOR_SIZE == 0) ? 1 : -1; }))];
                  if (errnum == 5 && 0 <= size && (soff | SECTOR_MASK) < lim)
                    {
      ...
      if (offset > ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - (off_t) soff)
        {
      ...
      if (n && ((done && *previous_human_offset)
        ...
        {
          char offset_buf[((2 * sizeof (uintmax_t) * 8 * 146 / 485 + 1) * (16 + 1) - 16 + 1 + 3) + 1];
          char size_buf[((2 * sizeof (uintmax_t) * 8 * 146 / 485 + 1) * (16 + 1) - 16 + 1 + 3) + 1];
...
free_pattern_mem:
  memset (pbuf, 0, (((output_size + 2) / 3) * 3));
           ...
           struct Options const *flags)
{
  ...
  if ((((((st.st_mode)) & 0170000) == (0020000)) && isatty (fd))
      || ((((st.st_mode)) & 0170000) == (0010000))
      || ((((st.st_mode)) & 0170000) == (0140000)))
    {
      error (0, 0, gettext ("%s: invalid file type"), qname);
      return 0;
    }
  else if (((((st.st_mode)) & 0170000) == (0100000)) && st.st_size < 0)
    {
      ...
      if (((((st.st_mode)) & 0170000) == (0100000)))
          ...
          if (! flags->exact)
            {
              ...
              off_t remainder = size % ((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1) ? (st).st_blksize : 512);
              if (size && size < ((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1) ? (st).st_blksize : 512))
                i_size = size;
              if (remainder != 0)
                {
                  off_t size_incr = ((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1) ? (st).st_blksize : 512) - remainder;
                  size += (((size_incr)<(((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - size))?(size_incr):(((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - size));
                }
            }
        }
      else
        {
          ...
          if (size <= 0)
           ...
           && st.st_size < (((((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1) ? (st).st_blksize : 512))<(size))?(((0 < (st).st_blksize && (st).st_blksize <= ((size_t)-1) / 8 + 1) ? (st).st_blksize : 512)):(size)))
    i_size = st.st_size;
  ...
  while (len--)
    {
      ...
      ((void) sizeof ((p) ? 1 : 0), __extension__ ({ if (p) ; else __assert_fail ("p", "shred.c", 1022, __extension__ __PRETTY_FUNCTION__); }));
...
wipename (char *oldname, char const *qoldname, struct Options const *flags)
{
  ...
  if (flags->remove_file == remove_wipesync)
    dir_fd = open_safer (dir, 00 | 0200000 | 0400 | 04000);
          ...
          struct randint_source *s, struct Options const *flags)
{
  ...
  fd = open_safer (name, 01 | 0400 | 0);
  if (fd < 0
      && ((*__errno_location ()) == 13 && flags->force)
      && chmod (name, 0200) == 0)
    fd = open_safer (name, 01 | 0400 | 0);
  if (fd < 0)
    {
      error (0, (*__errno_location ()), gettext ("%s: failed to open for writing"), qname);
      return 0;
    }
...
main (int argc, char **argv)
{
  ...
  int c;
  int i;
  char const *random_source = ((void*)0);
      ...
      switch (c)
        ...
        case 'n':
          flags.n_iterations = xdectoumax (optarg, 0,
                                           ((((9223372036854775807L *2UL+1UL))<((18446744073709551615UL) / sizeof (int)))?((9223372036854775807L *2UL+1UL)):((18446744073709551615UL) / sizeof (int))), "",
        ...
        case RANDOM_SOURCE_OPTION:
          if (random_source && !(strcmp (random_source, optarg) == 0))
            error (1, 0, gettext ("multiple random sources specified"));
        ...
        case 'u':
          if (optarg == ((void*)0))
            ...
            flags.remove_file = ((remove_methods) [__xargmatch_internal ("--remove", optarg, remove_args, (char const *) (remove_methods), sizeof *(remove_methods), argmatch_die)]);
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "shred", "GNU coreutils", Version, ("Colin Plumb"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (! randint_source)
    error (1, (*__errno_location ()), "%s", quotearg_colon (random_source));
  ...
  for (i = 0; i < n_files; i++)
    {
      ...
      if ((strcmp (file[i], "-") == 0))
        {
          ok &= wipefd (1, qname, randint_source, &flags);
        }
      else
        {
