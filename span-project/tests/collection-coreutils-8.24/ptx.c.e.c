...
static int *file_line_count;

static BLOCK *text_buffers;
# 227 "ptx.c"
typedef short int DELTA;
...
static size_t number_of_occurs[1];
...
static int half_line_width;
static int before_max_width;
static int keyafter_max_width;
...
static BLOCK tail;
static int tail_truncation;

static BLOCK before;
static int before_truncation;

static BLOCK keyafter;
static int keyafter_truncation;

static BLOCK head;
static int head_truncation;
...
compile_regex (struct regex_data *regex)
{
  ...
  if (message)
    error (1, 0, gettext ("%s (for regexp %s)"), message, quote (string));
...
swallow_file_in_memory (const char *file_name, BLOCK *block)
{
  ...
  if (!block->start)
    error (1, (*__errno_location ()), "%s", quote (using_stdin ? "-" : file_name));
...
compare_words (const void *void_first, const void *void_second)
{
  ...
  return ((const WORD *) void_first)->size - ((const WORD *) void_second)->size;
...
compare_occurs (const void *void_first, const void *void_second)
{
  ...
  return value == 0 ? ((const OCCURS *) void_first)->key.start - ((const OCCURS *) void_second)->key.start : value;
      ...
      if (cursor > word_start)
        {
          if (table->length == table->alloc)
            {
              if (((18446744073709551615UL) / sizeof *table->start - 1) / 2 < table->alloc)
                xalloc_die ();
...
# 940 "ptx.c"
          ...
          if (number_of_occurs[0] == occurs_alloc[0])
            {
              if (((18446744073709551615UL) / sizeof *occurs_table[0] - 1) / 2
                  < occurs_alloc[0])
                xalloc_die ();
...
define_all_fields (OCCURS *occurs)
{
  ...
  while (cursor < right_context_end
    ...
    {
      keyafter.end = cursor;
      if (word_regex.string) { regoff_t count; count = re_match (&word_regex.pattern, cursor, right_context_end - cursor, 0, ((void*)0)); if (count == -2) matcher_error (); cursor += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*cursor)]) while (cursor < right_context_end && word_fastmap[to_uchar (*cursor)]) cursor++; else cursor++;
    }
  ...
  if (head_max_width > 0)
    {
      ...
      while (head.start + head_max_width < head.end)
        if (word_regex.string) { regoff_t count; count = re_match (&word_regex.pattern, head.start, head.end - head.start, 0, ((void*)0)); if (count == -2) matcher_error (); head.start += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*head.start)]) while (head.start < head.end && word_fastmap[to_uchar (*head.start)]) head.start++; else head.start++;
...
output_one_tex_line (void)
{
  ...
  if (word_regex.string) { regoff_t count; count = re_match (&word_regex.pattern, cursor, keyafter.end - cursor, 0, ((void*)0)); if (count == -2) matcher_error (); cursor += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*cursor)]) while (cursor < keyafter.end && word_fastmap[to_uchar (*cursor)]) cursor++; else cursor++;
  key.end = cursor;
  after.start = cursor;
  print_field (key);
  fputs_unlocked ("}{",stdout);
  ...
  while (optchar = getopt_long (argc, argv, "AF:GM:ORS:TW:b:i:fg:o:trw:",
         ...
         optchar != (-1))
    {
      switch (optchar)
        ...
        case 'g':
          {
            ...
            if (xstrtoul (optarg, ((void*)0), 0, &tmp_ulong, ((void*)0)) != LONGINT_OK
                || ! (0 < tmp_ulong && tmp_ulong <= 2147483647))
              error (1, 0, gettext ("invalid gap width: %s"),
                     quotearg (optarg));
            gap_size = tmp_ulong;
            break;
          }
        ...
        case 'w':
          {
            ...
            if (xstrtoul (optarg, ((void*)0), 0, &tmp_ulong, ((void*)0)) != LONGINT_OK
                || ! (0 < tmp_ulong && tmp_ulong <= 2147483647))
              error (1, 0, gettext ("invalid line width: %s"),
                     quotearg (optarg));
        ...
        case 10:
          output_format = ((format_vals) [__xargmatch_internal ("--format", optarg, format_args, (char const *) (format_vals), sizeof *(format_vals), argmatch_die)]);
      ...
      if (optind < argc)
        {
          if (! freopen_safer (argv[optind], "w", stdout))
            error (1, (*__errno_location ()), "%s", argv[optind]);
          optind++;
        }
