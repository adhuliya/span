...
hide_problematic_chars (char *cell)
{
  ...
  while (*p)
    {
      if (((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _IScntrl))
        *p = '?';
      p++;
    }
  return cell;
...
alloc_field (int f, const char *c)
{
  ...
  if (field_data[f].used)
    ((void) sizeof ((!"field used") ? 1 : 0), __extension__ ({ if (!"field used") ; else __assert_fail ("!\"field used\"", "df.c", 352, __extension__ __PRETTY_FUNCTION__); }));
  ...
  do
    {
      ...
      switch (field)
        ...
        default:
          ((void) sizeof ((!"invalid field") ? 1 : 0), __extension__ ({ if (!"invalid field") ; else __assert_fail ("!\"invalid field\"", "df.c", 423, __extension__ __PRETTY_FUNCTION__); }));
        }
      s = comma;
    }
  while (s);
...
get_field_list (void)
{
  switch (header_mode)
    ...
    default:
      ((void) sizeof ((!"invalid header_mode") ? 1 : 0), __extension__ ({ if (!"invalid header_mode") ; else __assert_fail ("!\"invalid header_mode\"", "df.c", 492, __extension__ __PRETTY_FUNCTION__); }));
    }
}
  ...
  for (col = 0; col < ncolumns; col++)
    {
      ...
      columns[col]->width = (((columns[col]->width)>(gnu_mbswidth (cell, 0)))?(columns[col]->width):(gnu_mbswidth (cell, 0)));
    }
}
          ...
          if (devlist)
            {
              if (! print_grand_total && me->me_remote && devlist->me->me_remote
                  && ! (strcmp (devlist->me->me_devname, me->me_devname) == 0))
                {
                       ...
                       || (! (strcmp (devlist->me->me_devname, me->me_devname) == 0)
                           ...
                           && (strcmp (me->me_mountdir, devlist->me->me_mountdir) == 0)))
  ...
  if (force_fsu)
  ...
  else if (get_fs_usage (stat_file, disk, &fsu))
    {
      ...
      if (process_all && ((*__errno_location ()) == 13 || (*__errno_location ()) == 2))
        ...
        {
          error (0, (*__errno_location ()), "%s", quote (stat_file));
          exit_status = 1;
          return;
        }
    }
  ...
  for (col = 0; col < ncolumns; col++)
    {
      ...
      switch (columns[col]->field_type)
        ...
        default:
          v = ((void*)0);
          ((void) sizeof ((!"bad field_type") ? 1 : 0), __extension__ ({ if (!"bad field_type") ; else __assert_fail ("!\"bad field_type\"", "df.c", 1023, __extension__ __PRETTY_FUNCTION__); }));
        }
...
# 1121 "df.c"
        ...
        default:
          ((void) sizeof ((!"unhandled field") ? 1 : 0), __extension__ ({ if (!"unhandled field") ; else __assert_fail ("!\"unhandled field\"", "df.c", 1125, __extension__ __PRETTY_FUNCTION__); }));
        }

      if (!cell)
        ((void) sizeof ((!"empty cell") ? 1 : 0), __extension__ ({ if (!"empty cell") ; else __assert_fail ("!\"empty cell\"", "df.c", 1129, __extension__ __PRETTY_FUNCTION__); }));
            ...
            if (stat (me->me_mountdir, &disk_stats) == 0)
            ...
            else
              {
                ...
                if ((*__errno_location ()) == 5)
                  {
                    error (0, (*__errno_location ()), "%s", quote (me->me_mountdir));
                    exit_status = 1;
                  }
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "df", "GNU coreutils", Version, proper_name_utf8 ("Torbjorn Granlund", "Torbj\303\266rn Granlund"), ("David MacKenzie"), ("Paul Eggert"), (char *) ((void*)0)); exit (0); break;;
