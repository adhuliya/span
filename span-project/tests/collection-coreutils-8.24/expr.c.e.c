...
syntax_error (void)
{
  error (EXPR_INVALID, 0, gettext ("syntax error"));
}
# 290 "expr.c"
int
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "expr", "GNU coreutils", "8.24",
...
docolon (VALUE *sv, VALUE *pv)
{
  ...
  if (0 <= matchlen)
    ...
    error (EXPR_FAILURE,
           (matchlen == -2 ? (*__errno_location ()) : 75),
              ...
              if ((*__errno_location ()))
                {
                  ...
                  error (EXPR_INVALID, 0,
                         gettext ("the strings compared were %s and %s"),
