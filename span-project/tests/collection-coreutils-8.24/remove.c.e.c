...
excise (FTS *fts, FTSENT *ent, struct rm_options const *x, _Bool is_dir)
{
  ...
  error (0, (*__errno_location ()), gettext ("cannot remove %s"), quote (ent->fts_path));
    ...
    case 1:
      if (! x->recursive
               ...
               && is_empty_dir (fts->fts_cwd_fd, ent->fts_accpath)))
        {
          ...
          error (0, err, gettext ("cannot remove %s"), quote (ent->fts_path));
          mark_ancestor_dirs (ent);
      ...
      if (ent->fts_level == 0)
        {
          ...
          if ((x->root_dev_ino && ((*ent->fts_statp).st_ino == (*x->root_dev_ino).st_ino && (*ent->fts_statp).st_dev == (*x->root_dev_ino).st_dev)))
            {
              do { if ((strcmp (ent->fts_path, "/") == 0)) error (0, 0, gettext ("it is dangerous to operate recursively on %s"), quote (ent->fts_path)); else error (0, 0, gettext ("it is dangerous to operate recursively on %s (same as %s)"), quote_n (0, ent->fts_path), quote_n (1, "/")); error (0, 0, gettext ("use --no-preserve-root to override this failsafe")); } while (0);
              fts_skip_tree (fts, ent);
              return RM_ERROR;
            }
        }
      ...
      while (1)
        {
          ...
          ((void) sizeof ((((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (s)", "remove.c", 575, __extension__ __PRETTY_FUNCTION__); }));
