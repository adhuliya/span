...
enum
{
      FOREGROUND_OPTION = 127 + 1,
      PRESERVE_STATUS_OPTION
};

static struct option const long_options[] =
{
  {"kill-after", 1, ((void*)0), 'k'},
  {"signal", 1, ((void*)0), 's'},
  {"foreground", 0, ((void*)0), FOREGROUND_OPTION},
...
static void
settimeout (double duration, _Bool warn)
{
...
# 136 "timeout.c"
  ...
  if ((2147483647 *2U +1U) <= duration)
    timeint = (2147483647 *2U +1U);
  else
    {
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Start COMMAND, and kill it if still running after DURATION.\n"),stdout);



      emit_mandatory_arg_note ();

      fputs_unlocked (gettext ("      --preserve-status\n                 exit with the same status as COMMAND, even when the\n                   command times out\n      --foreground\n                 when not running timeout directly from a shell prompt,\n                   allow COMMAND to read from the TTY and get TTY signals;\n                   in this mode, children of COMMAND will not be timed out\n  -k, --kill-after=DURATION\n                 also send a KILL signal if COMMAND is still running\n                   this long after the initial signal was sent\n  -s, --signal=SIGNAL\n                 specify the signal to be sent on timeout;\n                   SIGNAL may be a name like 'HUP' or a number;\n                   see 'kill -l' for a list of signals\n"),stdout);
# 256 "timeout.c"
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "+k:s:", long_options, ((void*)0))) != -1)
    {
      switch (c)
        {
        case 'k':
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "timeout", "GNU coreutils", Version, proper_name_utf8 ("Padraig Brady", "P\303\241draig Brady"), (char *) ((void*)0)); exit (0); break;;
