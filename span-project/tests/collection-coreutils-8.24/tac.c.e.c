...
tac_seekable (int input_fd, const char *file, off_t file_pos)
{
  ...
  if (remainder != 0)
    {
      ...
      if (lseek (input_fd, file_pos, 0) < 0)
        error (0, (*__errno_location ()), gettext ("%s: seek failed"), quotearg_colon (file));
    }
  ...
  if (saved_record_size == ((size_t) -1))
    {
      error (0, (*__errno_location ()), gettext ("%s: read error"), quotearg_colon (file));
      return 0;
    }
      ...
      if (sentinel_length == 0)
        {
          ...
          if (1 < range)
            error (1, 0, gettext ("record too large"));
  ...
  while (1)
    {
      ...
      if (fwrite_unlocked (G_buffer,1,bytes_read,fp) != bytes_read)
        {
          error (0, (*__errno_location ()), gettext ("%s: write error"), quotearg_colon (file_name));
          goto Fail;
        }
...
tac_file (const char *filename)
{
  ...
  if (!is_stdin && close (fd) != 0)
    {
      error (0, (*__errno_location ()), gettext ("%s: read error"), quotearg_colon (filename));
      ok = 0;
    }
  return ok;
}
