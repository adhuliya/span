...
# 22 "extent-scan.c" 2
# 1 "../lib/sys/utsname.h" 1
# 22 "../lib/sys/utsname.h" 3
# 33 "../lib/sys/utsname.h" 3
...
extent_scan_init (int src_fd, struct extent_scan *scan)
{
  ...
  scan->ext_info = ((void*)0);
  scan->scan_start = 0;
  scan->initial_scan_failed = 0;
  scan->hit_final_extent = 0;
  scan->fm_flags = extent_need_sync () ? 0x00000001 : 0;
}
  ...
  while (1)
    {
      ...
      fiemap->fm_length = (~0ULL) - scan->scan_start;



      if (ioctl (scan->fd, (((2U|1U) << (((0 +8)+8)+14)) | ((('f')) << (0 +8)) | (((11)) << 0) | ((((sizeof(struct fiemap)))) << ((0 +8)+8))), fiemap) < 0)
        {
          if (scan->scan_start == 0)
            scan->initial_scan_failed = 1;
          return 0;
        }
      ...
      ((void) sizeof ((scan->ei_count <= (18446744073709551615UL) - fiemap->fm_mapped_extents) ? 1 : 0), __extension__ ({ if (scan->ei_count <= (18446744073709551615UL) - fiemap->fm_mapped_extents) ; else __assert_fail ("scan->ei_count <= SIZE_MAX - fiemap->fm_mapped_extents", "extent-scan.c", 126, __extension__ __PRETTY_FUNCTION__); }));
      ...
      for (i = 0; i < fiemap->fm_mapped_extents; i++)
        {
          ((void) sizeof ((fm_extents[i].fe_logical <= ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - fm_extents[i].fe_length) ? 1 : 0), __extension__ ({ if (fm_extents[i].fe_logical <= ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) - fm_extents[i].fe_length) ; else __assert_fail ("fm_extents[i].fe_logical <= OFF_T_MAX - fm_extents[i].fe_length", "extent-scan.c", 141, __extension__ __PRETTY_FUNCTION__); }));


          extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof last_ei->ext_flags >= sizeof fm_extents->fe_flags) ? 1 : -1; }))];

          if (si && last_ei->ext_flags
              == (fm_extents[i].fe_flags & ~0x00000001)
      ...
      if (last_ei->ext_flags & 0x00000001)
        scan->hit_final_extent = 1;
