...
# 35 "cut.c" 2
# 1 "../lib/getndelim2.h" 1
# 22 "../lib/getndelim2.h"
# 1 "../lib/stdio.h" 1
# 21 "../lib/stdio.h" 3
# 23 "../lib/getndelim2.h" 2
# 1 "../lib/sys/types.h" 1
# 21 "../lib/sys/types.h" 3
# 24 "../lib/getndelim2.h" 2
# 38 "../lib/getndelim2.h"
...
add_range_pair (size_t lo, size_t hi)
{
  if (n_rp == n_rp_allocated)
    rp = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(rp) != 1) ? 1 : -1; })), x2nrealloc (rp, &n_rp_allocated, sizeof *(rp)));
  rp[n_rp].lo = lo;
  rp[n_rp].hi = hi;
  ++n_rp;
}
  ...
  if (complement)
    {
      ...
      if (c[n-1].hi < (18446744073709551615UL))
        add_range_pair (c[n-1].hi + 1, (18446744073709551615UL));
...
set_fields (const char *fieldstr)
{
  size_t initial = 1;
  size_t value = 0;
  _Bool lhs_specified = 0;
  _Bool rhs_specified = 0;
  _Bool dash_found = 0;
  _Bool field_found = 0;
      ...
      if (*fieldstr == '-')
        {
          ...
          if (dash_found)
            do { error (0, 0, (gettext ("invalid byte, character or field list"))); usage (1); } while (0);
          dash_found = 1;
          fieldstr++;

          if (lhs_specified && !value)
            do { error (0, 0, (gettext ("fields and positions are numbered from 1"))); usage (1); } while (0);

          initial = (lhs_specified ? value : 1);
          value = 0;
        }
      else if (*fieldstr == ','
               || ((*__ctype_b_loc ())[(int) ((to_uchar (*fieldstr)))] & (unsigned short int) _ISblank) || *fieldstr == '\0')
        {
          in_digits = 0;
          ...
          if (dash_found)
            {
              ...
              if (!lhs_specified && !rhs_specified)
                do { error (0, 0, (gettext ("invalid range with no endpoint: -"))); usage (1); } while (0);
              ...
              if (!rhs_specified)
              ...
              else
                {
                  ...
                  if (value < initial)
                    do { error (0, 0, (gettext ("invalid decreasing range"))); usage (1); } while (0);
      ...
      else if (((unsigned int) (*fieldstr) - '0' <= 9))
        {
          ...
          if (!( (void) (&(value) == (size_t *) ((void*)0)), (void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (! ((size_t) 0 < (size_t) -1))) ? 1 : -1; })), (void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof (value) == sizeof (size_t)) ? 1 : -1; })), (((size_t) -1 / 10 < (value) || (size_t) ((value) * 10 + (*fieldstr - '0')) < (value)) ? 0 : (((value) = (value) * 10 + (*fieldstr - '0')), 1)) )
              || value == (18446744073709551615UL))
            {
              ...
              if (operating_mode == byte_mode)
                error (0, 0,
                       gettext ("byte offset %s is too large"), quote (bad_num));
              else
                error (0, 0,
                       gettext ("field number %s is too large"), quote (bad_num));
              free (bad_num);
              exit (1);
            }
      ...
      for (size_t j = i + 1; j < n_rp; ++j)
        {
          if (rp[j].lo <= rp[i].hi)
            {
              rp[i].hi = (((rp[j].hi)>(rp[i].hi))?(rp[j].hi):(rp[i].hi));
  ...
  rp[n_rp - 1].lo = rp[n_rp - 1].hi = (18446744073709551615UL);
      ...
      if (field_idx == 1 && buffer_first_field)
        {
          ...
          n_bytes = len;
          ((void) sizeof ((n_bytes != 0) ? 1 : 0), __extension__ ({ if (n_bytes != 0) ; else __assert_fail ("n_bytes != 0", "cut.c", 553, __extension__ __PRETTY_FUNCTION__); }));
      ...
      if (print_kth (field_idx))
        {
          ...
          while ((c = getc_unlocked (stream)) != delim && c != '\n' && c != (-1))
            {
              putchar_unlocked (c);
              prev_c = c;
            }
  ...
  if (! set_fields (spec_list_string))
    {
      if (operating_mode == field_mode)
        do { error (0, 0, (gettext ("missing list of fields"))); usage (1); } while (0);
      else
        do { error (0, 0, (gettext ("missing list of positions"))); usage (1); } while (0);
    }
