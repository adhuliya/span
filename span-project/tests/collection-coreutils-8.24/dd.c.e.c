...
extern int (*_gl_verify_function2 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (O_FULLBLOCK != 0) ? 1 : -1; }))];
extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (O_NOCACHE != 0) ? 1 : -1; }))];
extern int (*_gl_verify_function4 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (O_COUNT_BYTES != 0) ? 1 : -1; }))];
extern int (*_gl_verify_function5 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (O_SKIP_BYTES != 0) ? 1 : -1; }))];
extern int (*_gl_verify_function6 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (O_SEEK_BYTES != 0) ? 1 : -1; }))];




extern int (*_gl_verify_function7 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (((O_FULLBLOCK) & ((O_FULLBLOCK) - 1)) != 0)) ? 1 : -1; }))];
extern int (*_gl_verify_function8 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (((O_NOCACHE) & ((O_NOCACHE) - 1)) != 0)) ? 1 : -1; }))];
extern int (*_gl_verify_function9 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (((O_COUNT_BYTES) & ((O_COUNT_BYTES) - 1)) != 0)) ? 1 : -1; }))];
extern int (*_gl_verify_function10 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (((O_SKIP_BYTES) & ((O_SKIP_BYTES) - 1)) != 0)) ? 1 : -1; }))];
extern int (*_gl_verify_function11 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (! (((O_SEEK_BYTES) & ((O_SEEK_BYTES) - 1)) != 0)) ? 1 : -1; }))];
...
# 605 "dd.c"
      ...
      if (O_NOCACHE)
        fputs_unlocked (gettext ("  nocache   discard cached data\n"),stdout);
...
human_size (size_t n)
{
  static char hbuf[((2 * sizeof (uintmax_t) * 8 * 146 / 485 + 1) * (16 + 1) - 16 + 1 + 3) + 1];
  int human_opts =
...
alloc_ibuf (void)
{
  ...
  if (!real_buf)
    nl_error (1, 0,
           gettext ("memory exhausted by input buffer of size %""l" "u"" bytes (%s)"),
...
alloc_obuf (void)
{
  ...
  if (conversions_mask & C_TWOBUFS)
    {
      ...
      if (!real_obuf)
        nl_error (1, 0,
               gettext ("memory exhausted by output buffer of size %""l" "u" " bytes (%s)"),
  ...
  ((void) sizeof ((sizeof (uintptr_t) <= (page_size - 1)) ? 1 : 0), __extension__ ({ if (sizeof (uintptr_t) <= (page_size - 1)) ; else __assert_fail ("sizeof (uintptr_t) <= OUTPUT_BLOCK_SLOP", "dd.c", 734, __extension__ __PRETTY_FUNCTION__); }));
...
# 807 "dd.c"
  char const *time_fmt = gettext (", %g s, %s/s\n");
  if (progress_time)
    time_fmt = gettext (", %.6f s, %s/s");
...
write_output (void)
{
  ...
  if (nwritten != output_blocksize)
    {
      nl_error (0, (*__errno_location ()), gettext ("writing to %s"), quote (output_file));
      if (nwritten != 0)
        w_partial++;
      quit (1);
    }
...
scanargs (int argc, char *const *argv)
{
  ...
  for (i = optind; i < argc; i++)
    {
      ...
      if (val == ((void*)0))
        {
          nl_error (0, 0, gettext ("unrecognized operand %s"), quote (name));
          usage (1);
        }
      val++;
      ...
      if (operand_is (name, "if"))
      ...
      else
        {
          ...
          if (invalid != LONGINT_OK)
            nl_error (1, invalid == LONGINT_OVERFLOW ? 75 : 0,
                   "%s: %s", gettext ("invalid number"), quote (val));
        }
    }
  ...
  if (output_flags & O_FULLBLOCK)
    {
      nl_error (0, 0, "%s: %s", gettext ("invalid output flag"), "'fullblock'");
      usage (1);
    }

  if (input_flags & O_SEEK_BYTES)
    {
      nl_error (0, 0, "%s: %s", gettext ("invalid input flag"), "'seek_bytes'");
      usage (1);
    }
  ...
  if (multiple_bits_set (conversions_mask & (C_ASCII | C_EBCDIC | C_IBM)))
    nl_error (1, 0, gettext ("cannot combine any two of {ascii,ebcdic,ibm}"));
  if (multiple_bits_set (conversions_mask & (C_BLOCK | C_UNBLOCK)))
    nl_error (1, 0, gettext ("cannot combine block and unblock"));
  if (multiple_bits_set (conversions_mask & (C_LCASE | C_UCASE)))
    nl_error (1, 0, gettext ("cannot combine lcase and ucase"));
  if (multiple_bits_set (conversions_mask & (C_EXCL | C_NOCREAT)))
    nl_error (1, 0, gettext ("cannot combine excl and nocreat"));
  if (multiple_bits_set (input_flags & (040000 | O_NOCACHE))
      || multiple_bits_set (output_flags & (040000 | O_NOCACHE)))
    nl_error (1, 0, gettext ("cannot combine direct and nocache"));
      ...
      if (fdesc == 0)
        {
           struct stat st;
           if (fstat (0, &st) != 0)
             nl_error (1, (*__errno_location ()), gettext ("cannot fstat %s"), quote (file));
      ...
      if ((skip_via_lseek (file, fdesc, 0, 2)) >= 0)
        {
          ...
          if (fdesc == 0)
            nl_error (0, lseek_errno, gettext ("%s: cannot skip"), quote (file));
          else
            nl_error (0, lseek_errno, gettext ("%s: cannot seek"), quote (file));
          ...
          if (nread < 0)
            {
              if (fdesc == 0)
                {
                  nl_error (0, (*__errno_location ()), gettext ("error reading %s"), quote (file));
...
advance_input_after_read_error (size_t nbytes)
{
  ...
  nl_error (0, (*__errno_location ()), gettext ("%s: cannot seek"), quote (input_file));
  return 0;
}
  ...
  if (add_flags)
    {
      ...
      if (!ok)
        nl_error (1, (*__errno_location ()), gettext ("setting flags for %s"), quote (name));
    }
}
...
# 2042 "dd.c"
  if (skip_records != 0 || skip_bytes != 0)
    {
      ...
      if ((us_blocks || (!input_offset_overflow && us_bytes))
        ...
        {
          nl_error (0, 0,
                 gettext ("%s: cannot skip to specified offset"), quote (input_file));
        }
    }
      ...
      if (nread < 0)
        {
          if (!(conversions_mask & C_NOERROR) || status_level != STATUS_NONE)
            nl_error (0, (*__errno_location ()), gettext ("error reading %s"), quote (input_file));
      ...
      if (ibuf == obuf)
        {
          ...
          if (nwritten != n_bytes_read)
            {
              nl_error (0, (*__errno_location ()), gettext ("error writing %s"), quote (output_file));
              return 1;
            }
  ...
  if (final_op_was_seek)
    {
      ...
      if (fstat (1, &stdout_stat) != 0)
        {
          nl_error (0, (*__errno_location ()), gettext ("cannot fstat %s"), quote (output_file));
          return 1;
        }
  ...
  if ((conversions_mask & C_FDATASYNC) && fdatasync (1) != 0)
    {
      if ((*__errno_location ()) != 38 && (*__errno_location ()) != 22)
        {
          nl_error (0, (*__errno_location ()), gettext ("fdatasync failed for %s"), quote (output_file));
          exit_status = 1;
        }
      conversions_mask |= C_FSYNC;
    }
  ...
  if (conversions_mask & C_FSYNC)
    while (fsync (1) != 0)
      if ((*__errno_location ()) != 4)
        {
          nl_error (0, (*__errno_location ()), gettext ("fsync failed for %s"), quote (output_file));
          return 1;
        }
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "dd", "coreutils", Version,
  ...
  if (input_file == ((void*)0))
  ...
  else
    {
      if (ifd_reopen (0, input_file, 00 | input_flags, 0) < 0)
        nl_error (1, (*__errno_location ()), gettext ("failed to open %s"), quote (input_file));
    }
  ...
  else
    {
      ...
      if ((! seek_records
          ...
          && (ifd_reopen (1, output_file, 01 | opts, perms)
              < 0))
        nl_error (1, (*__errno_location ()), gettext ("failed to open %s"),
               quote (output_file));
      ...
      if (seek_records != 0 && !(conversions_mask & C_NOTRUNC))
        {
          ...
          if (((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << (sizeof (off_t) * 8 - 2)) - 1) * 2 + 1))) / output_blocksize < seek_records)
            nl_error (1, 0,
                   gettext ("offset too large: " "cannot truncate to a length of seek=%""l" "u""" " (%lu-byte) blocks"),
          ...
          if (iftruncate (1, size) != 0)
            {
              ...
              struct stat stdout_stat;
              if (fstat (1, &stdout_stat) != 0)
                nl_error (1, (*__errno_location ()), gettext ("cannot fstat %s"),
                       quote (output_file));
              if (((((stdout_stat.st_mode)) & 0170000) == (0100000))
                  || ((((stdout_stat.st_mode)) & 0170000) == (0040000))
                  || ((&stdout_stat)->st_mode - (&stdout_stat)->st_mode))
                nl_error (1, ftruncate_errno,
                       gettext ("failed to truncate to %""l" "u"" bytes" " in output file %s"),
