...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s --coreutils-prog=PROGRAM_NAME [PARAMETERS]... \n"),

              program_name);
      fputs_unlocked (gettext ("Execute the PROGRAM_NAME built-in program with the given PARAMETERS.\n\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  if (argc >= 2)
    {
      ...
      else if ((strncmp (argv[1], "--coreutils-prog-shebang=", strlen ("--coreutils-prog-shebang=")) == 0))
        {
          nskip = 2;
      ...
      if (nskip)
        {
          ...
          error (1, 0, gettext ("unknown program %s"), quote (prog_name));
        }
    }
  ...
  if ((optc = getopt_long (argc, argv, "", long_options, ((void*)0))) != -1)
    switch (optc)
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "coreutils", "GNU coreutils", Version, ("Alex Deymo"), (char *) ((void*)0)); exit (0); break;;
      }



  if (optind == 1 && prog_name && !(strcmp (prog_name, "coreutils") == 0))
    error (0, 0, gettext ("unknown program %s"), quote (prog_name));
