...
duinfo_init (struct duinfo *a)
{
  a->size = 0;
  a->inodes = 0;
  a->tmax.tv_sec = ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) 0 : ((time_t) ~ (time_t) 0 < (time_t) -1) ? ~ (time_t) 0 : ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << (sizeof (time_t) * 8 - 2)) - 1) * 2 + 1)))));
  a->tmax.tv_nsec = -1;
}
...
extern int (*_gl_verify_function3 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (time_args) / sizeof *(time_args)) == (sizeof (time_types) / sizeof *(time_types)) + 1) ? 1 : -1; }))];
...
extern int (*_gl_verify_function4 (void)) [(!!sizeof (struct { unsigned int _gl_verify_error_if_negative: ((sizeof (time_style_args) / sizeof *(time_style_args)) == (sizeof (time_style_types) / sizeof *(time_style_types)) + 1) ? 1 : -1; }))];
  ...
  if (! tm)
    {
      ...
      error (0, 0, gettext ("time %s is out of range"), when_str);
  ...
  if (info == 4)
    {
      ...
      error (0, ent->fts_errno, gettext ("cannot read directory %s"), quote (file));
      ok = 0;
    }
  else if (info != 6)
    {
          ...
          if (info == 11)
            {
              ...
              ((void) sizeof ((e == ent) ? 1 : 0), __extension__ ({ if (e == ent) ; else __assert_fail ("e == ent", "du.c", 487, __extension__ __PRETTY_FUNCTION__); }));
              info = ent->fts_info;
            }

          if (info == 10 || info == 13)
            {
              error (0, ent->fts_errno, gettext ("cannot access %s"), quote (file));
              return 0;
            }
          ...
          if (info == 1)
            {
              fts_set (fts, ent, 4);
              FTSENT const *e = fts_read (fts);
              ((void) sizeof ((e == ent) ? 1 : 0), __extension__ ({ if (e == ent) ; else __assert_fail ("e == ent", "du.c", 520, __extension__ __PRETTY_FUNCTION__); }));
            }
  ...
  else
    {
      if (level == prev_level)
      ...
      else
        {
          ...
          ((void) sizeof ((level == prev_level - 1) ? 1 : 0), __extension__ ({ if (level == prev_level - 1) ; else __assert_fail ("level == prev_level - 1", "du.c", 599, __extension__ __PRETTY_FUNCTION__); }));
...
main (int argc, char **argv)
{
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case TIME_OPTION:
          ...
          time_type =
            (optarg
             ? ((time_types) [__xargmatch_internal ("--time", optarg, time_args, (char const *) (time_types), sizeof *(time_types), argmatch_die)])
             : time_mtime);
          break;
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "du", "GNU coreutils", Version, proper_name_utf8 ("Torbjorn Granlund", "Torbj\303\266rn Granlund"), ("David MacKenzie"), ("Paul Eggert"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (opt_time)
    {
      ...
      if (*time_style == '+')
        ...
        {
          switch (((time_style_types) [__xargmatch_internal ("time style", time_style, time_style_args, (char const *) (time_style_types), sizeof *(time_style_types), argmatch_die)]))
      ...
      if (!file_name)
        {
          switch (ai_err)
            ...
            case AI_ERR_MEM:
              xalloc_die ();
            default:
              ((void) sizeof ((!"unexpected error code from argv_iter") ? 1 : 0), __extension__ ({ if (!"unexpected error code from argv_iter") ; else __assert_fail ("!\"unexpected error code from argv_iter\"", "du.c", 1075, __extension__ __PRETTY_FUNCTION__); }));
            }
        }
 ...
 argv_iter_done:
  ...
  if (files_from && (ferror_unlocked (stdin) || rpl_fclose (stdin) != 0) && ok)
    error (1, 0, gettext ("error reading %s"), quote (files_from));
