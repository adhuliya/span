...
# 29 "fold.c" 2
# 38 "fold.c"
static _Bool break_spaces;


static _Bool count_bytes;
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -b, --bytes         count bytes rather than columns\n  -s, --spaces        break at spaces\n  -w, --width=WIDTH   use WIDTH columns instead of 80\n"),stdout);
...
fold_file (char const *filename, size_t width)
{
  ...
  while ((c = getc_unlocked (istream)) != (-1))
    {
      if (offset_out + 1 >= allocated_out)
        line_out = ((void) (!!sizeof (struct { unsigned int _gl_verify_error_if_negative: (sizeof *(line_out) == 1) ? 1 : -1; })), x2realloc (line_out, &allocated_out));
          ...
          if (break_spaces)
            {
              ...
              while (logical_end)
                {
                  --logical_end;
                  if (((*__ctype_b_loc ())[(int) ((to_uchar (line_out[logical_end])))] & (unsigned short int) _ISblank))
                    {
                      found_blank = 1;
                      break;
                    }
  ...
  if (!(strcmp (filename, "-") == 0) && rpl_fclose (istream) == (-1))
    {
      error (0, (*__errno_location ()), "%s", filename);
      return 0;
    }
  ...
  while ((optc = getopt_long (argc, argv, shortopts, longopts, ((void*)0))) != -1)
    {
      ...
      switch (optc)
        ...
        case 'w':
          width = xdectoumax (optarg, 1, (18446744073709551615UL) - 8 - 1, "",
                              gettext ("invalid number of columns"), 0);
          break;

        case GETOPT_HELP_CHAR: usage (0); break;;

        case GETOPT_VERSION_CHAR: version_etc (stdout, "fold", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
