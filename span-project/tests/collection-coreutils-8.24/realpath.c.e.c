...
isdir (const char *path)
{
  struct stat sb;
  if (stat (path, &sb) != 0)
    error (1, (*__errno_location ()), gettext ("cannot stat %s"), quote (path));
...
process_path (const char *fname, int can_mode)
{
  ...
  if (!can_fname)
    {
      if (verbose)
        error (0, (*__errno_location ()), "%s", quote (fname));
      return 0;
    }
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "realpath", "GNU coreutils", Version, proper_name_utf8 ("Padraig Brady", "P\303\241draig Brady"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (relative_to)
    {
      ...
      if (!can_relative_to)
        error (1, (*__errno_location ()), "%s", quote (relative_to));
      if (need_dir && !isdir (can_relative_to))
        error (1, 20, "%s", quote (relative_to));
    }
  if (relative_base == relative_to)
  ...
  else if (relative_base)
    {
      ...
      if (!base)
        error (1, (*__errno_location ()), "%s", quote (relative_base));
      if (need_dir && !isdir (base))
        error (1, 20, "%s", quote (relative_base));
