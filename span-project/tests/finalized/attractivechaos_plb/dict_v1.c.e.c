...
# 1 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 1 3 4
# 74 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 3 4
typedef int wchar_t;
...
# 3 "dict_v1.c" 2
typedef struct { khint_t n_buckets, size, n_occupied, upper_bound; khint32_t *flags; kh_cstr_t *keys; int *vals; } kh_str_t; static inline kh_str_t *kh_init_str() { return (kh_str_t*)calloc(1, sizeof(kh_str_t)); } static inline void kh_destroy_str(kh_str_t *h) { if (h) { free(h->keys); free(h->flags); free(h->vals); free(h); } } static inline void kh_clear_str(kh_str_t *h) { if (h && h->flags) { memset(h->flags, 0xaa, ((h->n_buckets>>4) + 1) * sizeof(khint32_t)); h->size = h->n_occupied = 0; } } static inline khint_t kh_get_str(const kh_str_t *h, kh_cstr_t key) { if (h->n_buckets) { khint_t inc, k, i, last; k = __ac_X31_hash_string(key); i = k % h->n_buckets; inc = 1 + k % (h->n_buckets - 1); last = i; while (!((h->flags[i>>4]>>((i&0xfU)<<1))&2) && (((h->flags[i>>4]>>((i&0xfU)<<1))&1) || !(strcmp(h->keys[i], key) == 0))) { if (i + inc >= h->n_buckets) i = i + inc - h->n_buckets; else i += inc; if (i == last) return h->n_buckets; } return ((h->flags[i>>4]>>((i&0xfU)<<1))&3)? h->n_buckets : i; } else return 0; } static inline void kh_resize_str(kh_str_t *h, khint_t new_n_buckets) { khint32_t *new_flags = 0; khint_t j = 1; { khint_t t = 32 - 1; while (__ac_prime_list[t] > new_n_buckets) --t; new_n_buckets = __ac_prime_list[t+1]; if (h->size >= (khint_t)(new_n_buckets * __ac_HASH_UPPER + 0.5)) j = 0; else { new_flags = (khint32_t*)malloc(((new_n_buckets>>4) + 1) * sizeof(khint32_t)); memset(new_flags, 0xaa, ((new_n_buckets>>4) + 1) * sizeof(khint32_t)); if (h->n_buckets < new_n_buckets) { h->keys = (kh_cstr_t*)realloc(h->keys, new_n_buckets * sizeof(kh_cstr_t)); if (1) h->vals = (int*)realloc(h->vals, new_n_buckets * sizeof(int)); } } } if (j) { for (j = 0; j != h->n_buckets; ++j) { if (((h->flags[j>>4]>>((j&0xfU)<<1))&3) == 0) { kh_cstr_t key = h->keys[j]; int val; if (1) val = h->vals[j]; (h->flags[j>>4]|=1ul<<((j&0xfU)<<1)); while (1) { khint_t inc, k, i; k = __ac_X31_hash_string(key); i = k % new_n_buckets; inc = 1 + k % (new_n_buckets - 1); while (!((new_flags[i>>4]>>((i&0xfU)<<1))&2)) { if (i + inc >= new_n_buckets) i = i + inc - new_n_buckets; else i += inc; } (new_flags[i>>4]&=~(2ul<<((i&0xfU)<<1))); if (i < h->n_buckets && ((h->flags[i>>4]>>((i&0xfU)<<1))&3) == 0) { { kh_cstr_t tmp = h->keys[i]; h->keys[i] = key; key = tmp; } if (1) { int tmp = h->vals[i]; h->vals[i] = val; val = tmp; } (h->flags[i>>4]|=1ul<<((i&0xfU)<<1)); } else { h->keys[i] = key; if (1) h->vals[i] = val; break; } } } } if (h->n_buckets > new_n_buckets) { h->keys = (kh_cstr_t*)realloc(h->keys, new_n_buckets * sizeof(kh_cstr_t)); if (1) h->vals = (int*)realloc(h->vals, new_n_buckets * sizeof(int)); } free(h->flags); h->flags = new_flags; h->n_buckets = new_n_buckets; h->n_occupied = h->size; h->upper_bound = (khint_t)(h->n_buckets * __ac_HASH_UPPER + 0.5); } } static inline khint_t kh_put_str(kh_str_t *h, kh_cstr_t key, int *ret) { khint_t x; if (h->n_occupied >= h->upper_bound) { if (h->n_buckets > (h->size<<1)) kh_resize_str(h, h->n_buckets - 1); else kh_resize_str(h, h->n_buckets + 1); } { khint_t inc, k, i, site, last; x = site = h->n_buckets; k = __ac_X31_hash_string(key); i = k % h->n_buckets; if (((h->flags[i>>4]>>((i&0xfU)<<1))&2)) x = i; else { inc = 1 + k % (h->n_buckets - 1); last = i; while (!((h->flags[i>>4]>>((i&0xfU)<<1))&2) && (((h->flags[i>>4]>>((i&0xfU)<<1))&1) || !(strcmp(h->keys[i], key) == 0))) { if (((h->flags[i>>4]>>((i&0xfU)<<1))&1)) site = i; if (i + inc >= h->n_buckets) i = i + inc - h->n_buckets; else i += inc; if (i == last) { x = site; break; } } if (x == h->n_buckets) { if (((h->flags[i>>4]>>((i&0xfU)<<1))&2) && site != h->n_buckets) x = site; else x = i; } } } if (((h->flags[x>>4]>>((x&0xfU)<<1))&2)) { h->keys[x] = key; (h->flags[x>>4]&=~(3ul<<((x&0xfU)<<1))); ++h->size; ++h->n_occupied; *ret = 1; } else if (((h->flags[x>>4]>>((x&0xfU)<<1))&1)) { h->keys[x] = key; (h->flags[x>>4]&=~(3ul<<((x&0xfU)<<1))); ++h->size; *ret = 2; } else *ret = 0; return x; } static inline void kh_del_str(kh_str_t *h, khint_t x) { if (x != h->n_buckets && !((h->flags[x>>4]>>((x&0xfU)<<1))&3)) { (h->flags[x>>4]|=1ul<<((x&0xfU)<<1)); --h->size; } }
...
int main(int argc, char *argv[])
{
 ...
 mem[0] = malloc(0x100000);
 curr = block_end = 0;
 while (!feof(stdin)) {
  fgets(buf, 0x10000, stdin);
  k = kh_put_str(h, buf, &ret);
  if (ret) {
   int l = strlen(buf) + 1;
   if (block_end + l > 0x100000) {
    ++curr; block_end = 0;
    mem = realloc(mem, (curr + 1) * sizeof(void*));
    mem[curr] = malloc(0x100000);
   }
   memcpy(mem[curr] + block_end, buf, l);
   ((h)->keys[k]) = mem[curr] + block_end;
   block_end += l;
   ((h)->vals[k]) = 1;
  } else {
   ++((h)->vals[k]);
   if (((h)->vals[k]) > max) max = ((h)->vals[k]);
  }
 }
 printf("%u\t%d\n", ((h)->size), max);
