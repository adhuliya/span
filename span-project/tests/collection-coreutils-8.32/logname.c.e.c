...
main (int argc, char **argv)
{
  ...
  parse_gnu_standard_options_only (argc, argv, "logname", "GNU coreutils",
                                   Version, 1, usage, ("FIXME: unknown"),
                                   (char const *) ((void*)0));
  ...
  cp = getlogin ();
  if (! cp)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"no login name\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("no login name")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("no login name")), ((0) ? (void) 0 : __builtin_unreachable ()))));
