...
# 23 "../lib/canon-host.h"
char *canon_host (char const *host) __attribute__ ((__malloc__));
char *canon_host_r (char const *host, int *cherror) __attribute__ ((__malloc__));
...
{
  static time_t now = ((time_t) ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << ((sizeof (time_t) * 8) - 2)) - 1) * 2 + 1))));

  if (now == ((time_t) ~ ((time_t) (! (! ((time_t) 0 < (time_t) -1)) ? (time_t) -1 : ((((time_t) 1 << ((sizeof (time_t) * 8) - 2)) - 1) * 2 + 1)))))
    time (&now);
  ...
  if (boottime < when && now - 24 * 60 * 60 < when && when <= now)
    {
      ...
      if (seconds_idle < 60)
      ...
      else
        {
          ...
          ((void) sizeof ((seconds_idle / (60 * 60) < 24) ? 1 : 0), __extension__ ({ if (seconds_idle / (60 * 60) < 24) ; else __assert_fail ("seconds_idle / (60 * 60) < 24", "who.c", 205, __extension__ __PRETTY_FUNCTION__); }));
...
make_id_equals_comment (STRUCT_UTMP const *utmp_ent)
{
  ...
  strncat (comment, ((utmp_ent)->ut_id), utmpsize);
  return comment;
}
...
who (const char *filename, int options)
{
  ...
  if (read_utmp (filename, &n_users, &utmp_buf, options) != 0)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon (0, shell_escape_quoting_style, filename)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, filename)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, filename)), ((0) ? (void) 0 : __builtin_unreachable ()))));
