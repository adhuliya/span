...
static _Bool separator_ends_record;
...
static char compiled_separator_fastmap[(127*2 +1) + 1];
...
output (const char *start, const char *past_end)
{
  ...
  size_t bytes_available = 8192 - bytes_in_buffer;

  if (start == 0)
    {
      fwrite_unlocked (buffer,1,bytes_in_buffer,stdout);
      bytes_in_buffer = 0;
      return;
    }
  ...
  while (bytes_to_add >= bytes_available)
    {
      ...
      fwrite_unlocked (buffer,1,8192,stdout);
      bytes_in_buffer = 0;
...
tac_seekable (int input_fd, const char *file, off_t file_pos)
{
  ...
  _Bool first_time = 1;
  char first_char = *separator;
  char const *separator1 = separator + 1;
  size_t match_length1 = match_length - 1;
  ...
  if (remainder != 0)
    {
      file_pos -= remainder;
      if (lseek (input_fd, file_pos, 0) < 0)
        error (0, (*__errno_location ()), gettext ("%s: seek failed"), quotearg_n_style_colon (0, shell_escape_quoting_style, file));
    }
  ...
  if (saved_record_size == ((size_t) -1))
    {
      error (0, (*__errno_location ()), gettext ("%s: read error"), quotearg_n_style_colon (0, shell_escape_quoting_style, file));
      return 0;
    }
      ...
      if (sentinel_length == 0)
        {
          ...
          if (1 < range)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"record too large\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("record too large")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("record too large")), ((0) ? (void) 0 : __builtin_unreachable ()))));

          if (range == 1
              || ((ret = rpl_re_search (&compiled_separator, G_buffer,
                                    i, i - 1, range, &regs))
                  == -1))
            match_start = G_buffer - 1;
          else if (ret == -2)
            {
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"error in regular expression search\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("error in regular expression search")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("error in regular expression search")), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      else
        {
          ...
          while (*--match_start != first_char
                 || (match_length1 && !(strncmp (match_start + 1, separator1, match_length1) == 0)))
...
# 419 "tac.c"
static void
record_or_unlink_tempfile (char const *fn, FILE *fp __attribute__ ((__unused__)))
{
  unlink (fn);
}
  ...
  if (tempfile == ((void*)0))
    {
      ...
      char const *tempdir = t ? t : "/tmp";
      tempfile = mfile_name_concat (tempdir, "tacXXXXXX", ((void*)0));
      if (tempdir == ((void*)0))
        {
          error (0, 0, gettext ("memory exhausted"));
          return 0;
        }
# 455 "tac.c"
      ...
      if (fd < 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to create temporary file in %s"),
      ...
      if (! tmp_fp)
        {
          error (0, (*__errno_location ()), gettext ("failed to open %s for writing"),
  ...
  else
    {
      ...
      if (rpl_fseeko (tmp_fp, 0, 0) < 0
          || ftruncate (fileno (tmp_fp), 0) < 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to rewind stream for %s"),
                 quotearg_style (shell_escape_always_quoting_style, tempfile));
          return 0;
        }
    }
  ...
  while (1)
    {
      ...
      if (fwrite_unlocked (G_buffer,1,bytes_read,fp) != bytes_read)
        {
          error (0, (*__errno_location ()), gettext ("%s: write error"), quotearg_n_style_colon (0, shell_escape_quoting_style, file_name));
          return -1;
        }
...
tac_file (const char *filename)
{
  ...
  if (is_stdin)
  ...
  else
    {
      ...
      if (fd < 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to open %s for reading"),
  ...
  if (!is_stdin && close (fd) != 0)
    {
      error (0, (*__errno_location ()), gettext ("%s: read error"), quotearg_n_style_colon (0, shell_escape_quoting_style, filename));
      ok = 0;
    }
  return ok;
}
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "brs:", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "tac", "GNU coreutils", Version, ("Jay Lepreau"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (sentinel_length == 0)
    {
      if (*separator == 0)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"separator cannot be empty\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("separator cannot be empty")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("separator cannot be empty")), ((0) ? (void) 0 : __builtin_unreachable ()))));

      compiled_separator.buffer = ((void*)0);
      compiled_separator.allocated = 0;
      compiled_separator.fastmap = compiled_separator_fastmap;
      compiled_separator.translate = ((void*)0);
      error_message = rpl_re_compile_pattern (separator, strlen (separator),
                                          &compiled_separator);
      if (error_message)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, \"%s\", (error_message)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, "%s", (error_message)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, "%s", (error_message)), ((0) ? (void) 0 : __builtin_unreachable ()))));
    }
  else
  ...
  while (sentinel_length >= read_size / 2)
    {
      if ((18446744073709551615UL) / 2 < read_size)
        xalloc_die ();
      read_size *= 2;
    }
  ...
  output ((char *) ((void*)0), (char *) ((void*)0));
