...
# 21 "../lib/unicodeio.h"
# 1 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 1 3
# 22 "../lib/unicodeio.h" 2
# 1 "../lib/stdio.h" 1
# 21 "../lib/stdio.h" 3
# 23 "../lib/unicodeio.h" 2
...
# 189 "printf.c"
static intmax_t vstrtoimax (char const *s) { char *end; intmax_t val; if ((*s == '\"' || *s == '\'') && *(s + 1)) { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (strtoimax (s, &end, 0)); verify_numeric (s, end); } return val; }
static uintmax_t vstrtoumax (char const *s) { char *end; uintmax_t val; if ((*s == '\"' || *s == '\'') && *(s + 1)) { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (strtoumax (s, &end, 0)); verify_numeric (s, end); } return val; }
static long double vstrtold (char const *s) { char *end; long double val; if ((*s == '\"' || *s == '\'') && *(s + 1)) { unsigned char ch = *++s; val = ch; if (*++s != 0 && !posixly_correct) error (0, 0, gettext (cfcc_msg), s); } else { (*__errno_location ()) = 0; val = (cl_strtold (s, &end)); verify_numeric (s, end); } return val; }
...
static int
print_esc (const char *escstart, _Bool octal_0)
{
  ...
  if (*p == 'x')
    {
      ...
      for (esc_length = 0, ++p;
           esc_length < 2 && ((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISxdigit);
           ++esc_length, ++p)
        esc_value = esc_value * 16 + ((*p) >= 'a' && (*p) <= 'f' ? (*p) - 'a' + 10 : (*p) >= 'A' && (*p) <= 'F' ? (*p) - 'A' + 10 : (*p) - '0');
      if (esc_length == 0)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"missing hexadecimal number in escape\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("missing hexadecimal number in escape")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("missing hexadecimal number in escape")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  else if (((*p) >= '0' && (*p) <= '7'))
    {
      ...
      for (esc_length = 0, p += octal_0 && *p == '0';
           esc_length < 3 && ((*p) >= '0' && (*p) <= '7');
           ++esc_length, ++p)
        esc_value = esc_value * 8 + ((*p) - '0');
      putchar_unlocked (esc_value);
    }
  ...
  else if (*p == 'u' || *p == 'U')
    {
      ...
      for (esc_length = (esc_char == 'u' ? 4 : 8), ++p;
           ...
           --esc_length, ++p)
        {
          if (! ((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISxdigit))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"missing hexadecimal number in escape\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("missing hexadecimal number in escape")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("missing hexadecimal number in escape")), ((0) ? (void) 0 : __builtin_unreachable ()))));
          uni_value = uni_value * 16 + ((*p) >= 'a' && (*p) <= 'f' ? (*p) - 'a' + 10 : (*p) >= 'A' && (*p) <= 'F' ? (*p) - 'A' + 10 : (*p) - '0');
        }
      ...
      if ((uni_value <= 0x9f
        ...
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid universal character name \\\\%c%0*x\"), esc_char, (esc_char == 'u' ? 4 : 8), uni_value), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid universal character name \\%c%0*x"), esc_char, (esc_char == 'u' ? 4 : 8), uni_value), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid universal character name \\%c%0*x"), esc_char, (esc_char == 'u' ? 4 : 8), uni_value), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
print_direc (const char *start, size_t length, char conversion,
             _Bool have_field_width, int field_width,
             _Bool have_precision, int precision,
             char const *argument)
{
  char *p;
...

  {
    ...
    switch (conversion)
      ...
      case 'd': case 'i': case 'o': case 'u': case 'x': case 'X':
        length_modifier = "l" "d";
        length_modifier_len = sizeof "l" "d" - 2;
        break;
  ...
  for (f = format; *f; ++f)
    {
      switch (*f)
        ...
        case '%':
          direc_start = f++;
          direc_length = 1;
          have_field_width = have_precision = 0;
          if (*f == '%')
            {
              putchar_unlocked ('%');
              break;
          ...
          if (*f == 'q')
            {
              if (argc > 0)
                {
                  fputs_unlocked (quotearg_style (shell_escape_quoting_style, *argv),stdout);
        ...
        no_more_flag_characters:
              ...
              if (argc > 0)
                {
                  ...
                  if ((-2147483647 -1) <= width && width <= 2147483647)
                    field_width = width;
                  else
                    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid field width: %s\"), quote (*argv)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid field width: %s"), quote (*argv)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid field width: %s"), quote (*argv)), ((0) ? (void) 0 : __builtin_unreachable ()))));
                  ...
                  if (argc > 0)
                    {
                      ...
                      else if (2147483647 < prec)
                        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid precision: %s\"), quote (*argv)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid precision: %s"), quote (*argv)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid precision: %s"), quote (*argv)), ((0) ? (void) 0 : __builtin_unreachable ()))));
...

          {
            ...
            if (! ok[conversion])
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"%.*s: invalid conversion specification\"), (int) (f + 1 - direc_start), direc_start), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("%.*s: invalid conversion specification"), (int) (f + 1 - direc_start), direc_start), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("%.*s: invalid conversion specification"), (int) (f + 1 - direc_start), direc_start), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (argc == 2)
    {
      ...
      if ((strcmp (argv[1], "--version") == 0))
        {
          version_etc (stdout, "printf", "GNU coreutils", Version, ("David MacKenzie"),
                       (char *) ((void*)0));
          return 0;
        }
    }
