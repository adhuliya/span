...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      printf (gettext ("Run COMMAND with an adjusted niceness, which affects process scheduling.\nWith no COMMAND, print the current niceness.  Niceness values range from\n%d (most favorable to the process) to %d (least favorable to the process).\n"),
...
main (int argc, char **argv)
{
  ...
  int adjustment = 10;
  char const *adjustment_given = ((void*)0);
  _Bool ok;
  int i;
  ...
  for (i = 1; i < argc; )
    {
      ...
      if (s[0] == '-' && ((unsigned int) (s[1 + (s[1] == '-' || s[1] == '+')]) - '0' <= 9))
        {
      ...
      else
        {
          ...
          switch (c)
            ...
            case GETOPT_VERSION_CHAR: version_etc (stdout, "nice", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (adjustment_given)
    {
      ...
      enum { MIN_ADJUSTMENT = 1 - 2 * 20, MAX_ADJUSTMENT = 2 * 20 - 1 };
      long int tmp;
      if (LONGINT_OVERFLOW < xstrtol (adjustment_given, ((void*)0), 10, &tmp, ""))
        ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, 0, gettext (\"invalid adjustment %s\"), quote (adjustment_given)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, 0, gettext ("invalid adjustment %s"), quote (adjustment_given)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, 0, gettext ("invalid adjustment %s"), quote (adjustment_given)), ((0) ? (void) 0 : __builtin_unreachable ()))));

      adjustment = (((MIN_ADJUSTMENT)>((((tmp)<(MAX_ADJUSTMENT))?(tmp):(MAX_ADJUSTMENT))))?(MIN_ADJUSTMENT):((((tmp)<(MAX_ADJUSTMENT))?(tmp):(MAX_ADJUSTMENT))));
    }
  ...
  if (i == argc)
    {
      ...
      if (current_niceness == -1 && (*__errno_location ()) != 0)
        ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, (*__errno_location ()), gettext (\"cannot get niceness\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("cannot get niceness")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("cannot get niceness")), ((0) ? (void) 0 : __builtin_unreachable ()))));
      printf ("%d\n", current_niceness);
      return 0;
    }
  ...
  if (!ok)
    {
      error (perm_related_errno ((*__errno_location ())) ? 0
             : EXIT_CANCELED, (*__errno_location ()), gettext ("cannot set niceness"));
