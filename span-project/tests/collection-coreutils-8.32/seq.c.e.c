...
{
  { "equal-width", 0, ((void*)0), 'w'},
  { "format", 1, ((void*)0), 'f'},
  { "separator", 1, ((void*)0), 's'},
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [OPTION]... LAST\n  or:  %s [OPTION]... FIRST LAST\n  or:  %s [OPTION]... FIRST INCREMENT LAST\n"), program_name, program_name, program_name);




      fputs_unlocked (gettext ("Print numbers from FIRST to LAST, in steps of INCREMENT.\n"),stdout);



      emit_mandatory_arg_note ();

      fputs_unlocked (gettext ("  -f, --format=FORMAT      use printf style floating-point FORMAT\n  -s, --separator=STRING   use STRING to separate numbers (default: \\n)\n  -w, --equal-width        equalize width by padding with leading zeroes\n"),stdout);
...
# 109 "seq.c"
      fputs_unlocked (gettext ("FORMAT must be suitable for printing one argument of type 'double';\nit defaults to %.PRECf if FIRST, INCREMENT, and LAST are all fixed point\ndecimal numbers with maximum precision PREC, and to %g otherwise.\n"),stdout);
...
scan_arg (const char *arg)
{
  ...
  while (((*__ctype_b_loc ())[(int) ((to_uchar (*arg)))] & (unsigned short int) _ISspace) || *arg == '+')
    arg++;
  ...
  if (! arg[strcspn (arg, "xX")] && ((ret.value) * 0 == 0))
    {
      ...
      if (e)
        {
          long exponent = strtol (e + 1, ((void*)0), 10);
          ret.precision += exponent < 0 ? -exponent
                                        : - (((ret.precision)<(exponent))?(ret.precision):(exponent));
          ...
          if (exponent < 0)
          ...
          else
            {
              if (decimal_point && ret.precision == 0 && fraction_len)
                ret.width--;
              exponent -= (((fraction_len)<(exponent))?(fraction_len):(exponent));
            }
          ret.width += exponent;
        }
    }
...
long_double_format (char const *fmt, struct layout *layout)
{
  ...
  if (! strchr ("efgaEFGA", fmt[i]))
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"format %s has unknown %%%c directive\"), quote (fmt), fmt[i]), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("format %s has unknown %%%c directive"), quote (fmt), fmt[i]), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("format %s has unknown %%%c directive"), quote (fmt), fmt[i]), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
{
  _Bool out_of_range = (step < 0 ? first < last : last < first);
  ...
  if (! out_of_range)
    {
          ...
          if (out_of_range)
            {
              ...
              if (xstrtold (x_str + layout.prefix_len, ((void*)0), &x_val, cl_strtold)
                  && x_val == last)
                {
                  char *x0_str = ((void*)0);
                  int x0_strlen = rpl_asprintf (&x0_str, fmt, x0);
                  if (x0_strlen < 0)
                    xalloc_die ();
      ...
      if (fputs_unlocked (terminator,stdout) == (-1))
        io_error ();
    }
}
...
{
  static char format_buf[sizeof "%0.Lf" + 2 * (((((sizeof (int) * 8) - (! ((__typeof__ (int)) 0 < (__typeof__ (int)) -1))) * 146 + 484) / 485) + (! ((__typeof__ (int)) 0 < (__typeof__ (int)) -1)))];

  int prec = (((first.precision)>(step.precision))?(first.precision):(step.precision));

  if (prec != 2147483647 && last.precision != 2147483647)
    {
      if (equal_width)
        {
          ...
          if (first.precision == 0 && prec)
            first_width++;
          size_t width = (((first_width)>(last_width))?(first_width):(last_width));
          if (width <= 2147483647)
            {
              int w = width;
...
seq_fast (char const *a, char const *b, uintmax_t step)
{
  ...
  size_t inc_size = ((((((p_len + 1)>(q_len))?(p_len + 1):(q_len)))>(31))?((((p_len + 1)>(q_len))?(p_len + 1):(q_len))):(31));
  ...
  _Bool ok = inf || cmp (p, p_len, q, q_len) <= 0;
  if (ok)
    {



      size_t buf_size = (((8192)>((inc_size + 1) * 2))?(8192):((inc_size + 1) * 2));
      ...
      while (1)
        {
          ...
          *bufp++ = *separator;
          ...
          if (buf_end - (p_len + 1) < bufp)
            {
              if (fwrite_unlocked (buf,bufp - buf,1,stdout) != 1)
                io_error ();
              bufp = buf;
            }
        }
...
{
  size_t n = strlen (s);
  return ((unsigned int) (s[0]) - '0' <= 9) && n == strspn (s, "0123456789");
}
...
main (int argc, char **argv)
{
  ...
  while (optind < argc)
    {
      ...
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "seq", "GNU coreutils", Version, ("Ulrich Drepper"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (format_str != ((void*)0) && equal_width)
    {
...
# 660 "seq.c"
  _Bool fast_step_ok = 0;
  if (n_args != 3
      || (all_digits_p (argv[optind + 1])
          && xstrtold (argv[optind + 1], ((void*)0), &step.value, cl_strtold)
  ...
  if (first.precision == 0 && step.precision == 0 && last.precision == 0
      ...
      && !equal_width && !format_str && strlen (separator) == 1)
    {
      char *s1;
      char *s2;
      if (rpl_asprintf (&s1, "%0.Lf", first.value) < 0)
        xalloc_die ();
      if (! ((last.value) * 0 == 0))
        s2 = xstrdup ("inf");
      else if (rpl_asprintf (&s2, "%0.Lf", last.value) < 0)
        xalloc_die ();
