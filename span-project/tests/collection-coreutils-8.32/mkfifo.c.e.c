  ...
  while ((optc = getopt_long (argc, argv, "m:Z", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "mkfifo", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  for (; optind < argc; ++optind)
    {
      ...
      if (mkfifo (argv[optind], newmode) != 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot create fifo %s"), quotearg_style (shell_escape_always_quoting_style, argv[optind]));
          exit_status = 1;
        }
      else if (specified_mode && lchmod (argv[optind], newmode) != 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot set permissions of %s"),
