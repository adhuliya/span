...
{
  {"parenb", control, 4, 0000400, 0},
  {"parodd", control, 4, 0001000, 0},

  {"cmspar", control, 4, 010000000000, 0},

  {"cs5", control, 0, 0000000, 0000060},
  {"cs6", control, 0, 0000020, 0000060},
  {"cs7", control, 0, 0000040, 0000060},
  {"cs8", control, 0, 0000060, 0000060},
  {"hupcl", control, 4, 0002000, 0},
  {"hup", control, 4 | 8, 0002000, 0},
  {"cstopb", control, 4, 0000100, 0},
  {"cread", control, 1 | 4, 0000200, 0},
  {"clocal", control, 4, 0004000, 0},

  {"crtscts", control, 4, 020000000000, 0},





  {"ignbrk", input, 2 | 4, 0000001, 0},
  {"brkint", input, 1 | 4, 0000002, 0},
  {"ignpar", input, 4, 0000004, 0},
  {"parmrk", input, 4, 0000010, 0},
  {"inpck", input, 4, 0000020, 0},
  {"istrip", input, 4, 0000040, 0},
  {"inlcr", input, 2 | 4, 0000100, 0},
  {"igncr", input, 2 | 4, 0000200, 0},
  {"icrnl", input, 1 | 4, 0000400, 0},
  {"ixon", input, 4, 0002000, 0},
  {"ixoff", input, 2 | 4, 0010000, 0},
  {"tandem", input, 4 | 8, 0010000, 0},

  {"iuclc", input, 2 | 4, 0001000, 0},


  {"ixany", input, 2 | 4, 0004000, 0},


  {"imaxbel", input, 1 | 4, 0020000, 0},


  {"iutf8", input, 2 | 4, 0040000, 0},


  {"opost", output, 1 | 4, 0000001, 0},

  {"olcuc", output, 2 | 4, 0000002, 0},


  {"ocrnl", output, 2 | 4, 0000010, 0},


  {"onlcr", output, 1 | 4, 0000004, 0},


  {"onocr", output, 2 | 4, 0000020, 0},


  {"onlret", output, 2 | 4, 0000040, 0},


  {"ofill", output, 2 | 4, 0000100, 0},


  {"ofdel", output, 2 | 4, 0000200, 0},


  {"nl1", output, 2, 0000400, 0000400},
  {"nl0", output, 1, 0000000, 0000400},


  {"cr3", output, 2, 0003000, 0003000},
  {"cr2", output, 2, 0002000, 0003000},
  {"cr1", output, 2, 0001000, 0003000},
  {"cr0", output, 1, 0000000, 0003000},



  {"tab3", output, 2, 0014000, 0014000},


  {"tab2", output, 2, 0010000, 0014000},


  {"tab1", output, 2, 0004000, 0014000},


  {"tab0", output, 1, 0000000, 0014000},
  ...
  {"bs1", output, 2, 0020000, 0020000},
  {"bs0", output, 1, 0000000, 0020000},


  {"vt1", output, 2, 0040000, 0040000},
  {"vt0", output, 1, 0000000, 0040000},


  {"ff1", output, 2, 0100000, 0100000},
  {"ff0", output, 1, 0000000, 0100000},


  {"isig", local, 1 | 4, 0000001, 0},
  {"icanon", local, 1 | 4, 0000002, 0},

  {"iexten", local, 1 | 4, 0100000, 0},

  {"echo", local, 1 | 4, 0000010, 0},
  {"echoe", local, 1 | 4, 0000020, 0},
  {"crterase", local, 4 | 8, 0000020, 0},
  {"echok", local, 1 | 4, 0000040, 0},
  {"echonl", local, 2 | 4, 0000100, 0},
  {"noflsh", local, 2 | 4, 0000200, 0},

  {"xcase", local, 2 | 4, 0000004, 0},


  {"tostop", local, 2 | 4, 0000400, 0},


  {"echoprt", local, 2 | 4, 0002000, 0},
  {"prterase", local, 4 | 8, 0002000, 0},


  {"echoctl", local, 1 | 4, 0001000, 0},
  {"ctlecho", local, 4 | 8, 0001000, 0},


  {"echoke", local, 1 | 4, 0004000, 0},
  {"crtkill", local, 4 | 8, 0004000, 0},


  {"flusho", local, 2 | 4, 0010000, 0},




  {"extproc", local, 2 | 4, 0200000, 0},


  {"evenp", combination, 4 | 8, 0, 0},
  {"parity", combination, 4 | 8, 0, 0},
  {"oddp", combination, 4 | 8, 0, 0},
  {"nl", combination, 4 | 8, 0, 0},
  {"ek", combination, 8, 0, 0},
  {"sane", combination, 8, 0, 0},
  {"cooked", combination, 4 | 8, 0, 0},
  {"raw", combination, 4 | 8, 0, 0},
  {"pass8", combination, 4 | 8, 0, 0},
  {"litout", combination, 4 | 8, 0, 0},
  {"cbreak", combination, 4 | 8, 0, 0},

  {"decctlq", combination, 4 | 8, 0, 0},


  {"tabs", combination, 4 | 8, 0, 0},


  {"lcase", combination, 4 | 8, 0, 0},
  {"LCASE", combination, 4 | 8, 0, 0},

  {"crt", combination, 8, 0, 0},
  {"dec", combination, 8, 0, 0},

  {((void*)0), control, 0, 0, 0}
};
...
static _Bool recover_mode (char const *arg, struct termios *mode);
static int screen_columns (void);
static _Bool set_mode (struct mode_info const *info, _Bool reversed,
                      struct termios *mode);
...
static int tcsetattr_options = 1;
...
wrapf (const char *message,...)
{
  ...
  buflen = rpl_vasprintf (&buf, message, args);
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [-F DEVICE | --file=DEVICE] [SETTING]...\n  or:  %s [-F DEVICE | --file=DEVICE] [-a|--all]\n  or:  %s [-F DEVICE | --file=DEVICE] [-g|--save]\n"),




              program_name, program_name, program_name);
      fputs_unlocked (gettext ("Print or change terminal characteristics.\n"),stdout);



      emit_mandatory_arg_note ();

      fputs_unlocked (gettext ("  -a, --all          print all current settings in human-readable form\n  -g, --save         print all current settings in a stty-readable form\n  -F, --file=DEVICE  open and use the specified DEVICE instead of stdin\n"),stdout);
      ...
      fputs_unlocked (gettext (" * discard CHAR  CHAR will toggle discarding of output\n"),stdout);
# 568 "stty.c"
      fputs_unlocked (gettext ("   eof CHAR      CHAR will send an end of file (terminate the input)\n   eol CHAR      CHAR will end the line\n"),stdout);




      fputs_unlocked (gettext (" * eol2 CHAR     alternate CHAR for ending the line\n"),stdout);



      fputs_unlocked (gettext ("   erase CHAR    CHAR will erase the last character typed\n   intr CHAR     CHAR will send an interrupt signal\n   kill CHAR     CHAR will erase the current line\n"),stdout);





      fputs_unlocked (gettext (" * lnext CHAR    CHAR will enter the next character quoted\n"),stdout);
# 592 "stty.c"
      fputs_unlocked (gettext ("   quit CHAR     CHAR will send a quit signal\n"),stdout);



      fputs_unlocked (gettext (" * rprnt CHAR    CHAR will redraw the current line\n"),stdout);



      fputs_unlocked (gettext ("   start CHAR    CHAR will restart the output after stopping it\n   stop CHAR     CHAR will stop the output\n   susp CHAR     CHAR will send a terminal stop signal\n"),stdout);





      fputs_unlocked (gettext (" * swtch CHAR    CHAR will switch to a different shell layer\n"),stdout);




      fputs_unlocked (gettext (" * werase CHAR   CHAR will erase the last word typed\n"),stdout);
      ...
      fputs_unlocked (gettext (" * cols N        tell the kernel that the terminal has N columns\n * columns N     same as cols N\n"),stdout);




      printf (gettext (" * [-]drain      wait for transmission before applying settings (%s by default)\n"), tcsetattr_options == 1 ? gettext ("on") : gettext ("off"));


      fputs_unlocked (gettext ("   ispeed N      set the input speed to N\n"),stdout);



      fputs_unlocked (gettext (" * line N        use line discipline N\n"),stdout);



      fputs_unlocked (gettext ("   min N         with -icanon, set N characters minimum for a completed read\n   ospeed N      set the output speed to N\n"),stdout);




      fputs_unlocked (gettext (" * rows N        tell the kernel that the terminal has N rows\n * size          print the number of rows and columns according to the kernel\n"),stdout);




      fputs_unlocked (gettext ("   speed         print the terminal speed\n   time N        with -icanon, set read timeout of N tenths of a second\n"),stdout);
      ...
      fputs_unlocked (gettext (" * [-]crtscts    enable RTS/CTS handshaking\n"),stdout);
# 667 "stty.c"
      fputs_unlocked (gettext ("   csN           set character size to N bits, N in [5..8]\n"),stdout);


      fputs_unlocked (gettext ("   [-]cstopb     use two stop bits per character (one with '-')\n   [-]hup        send a hangup signal when the last process closes the tty\n   [-]hupcl      same as [-]hup\n   [-]parenb     generate parity bit in output and expect parity bit in input\n   [-]parodd     set odd parity (or even parity with '-')\n"),stdout);
      ...
      fputs_unlocked (gettext (" * [-]cmspar     use \"stick\" (mark/space) parity\n"),stdout);



      fputs_unlocked (gettext ("\nInput settings:\n   [-]brkint     breaks cause an interrupt signal\n   [-]icrnl      translate carriage return to newline\n   [-]ignbrk     ignore break characters\n   [-]igncr      ignore carriage return\n   [-]ignpar     ignore characters with parity errors\n"),stdout);
# 692 "stty.c"
      fputs_unlocked (gettext (" * [-]imaxbel    beep and do not flush a full input buffer on a character\n"),stdout);



      fputs_unlocked (gettext ("   [-]inlcr      translate newline to carriage return\n   [-]inpck      enable input parity checking\n   [-]istrip     clear high (8th) bit of input characters\n"),stdout);





      fputs_unlocked (gettext (" * [-]iutf8      assume input characters are UTF-8 encoded\n"),stdout);




      fputs_unlocked (gettext (" * [-]iuclc      translate uppercase characters to lowercase\n"),stdout);




      fputs_unlocked (gettext (" * [-]ixany      let any character restart output, not only start character\n"),stdout);



      fputs_unlocked (gettext ("   [-]ixoff      enable sending of start/stop characters\n   [-]ixon       enable XON/XOFF flow control\n   [-]parmrk     mark parity errors (with a 255-0-character sequence)\n   [-]tandem     same as [-]ixoff\n"),stdout);
      ...
      fputs_unlocked (gettext (" * bsN           backspace delay style, N in [0..1]\n"),stdout);




      fputs_unlocked (gettext (" * crN           carriage return delay style, N in [0..3]\n"),stdout);




      fputs_unlocked (gettext (" * ffN           form feed delay style, N in [0..1]\n"),stdout);




      fputs_unlocked (gettext (" * nlN           newline delay style, N in [0..1]\n"),stdout);




      fputs_unlocked (gettext (" * [-]ocrnl      translate carriage return to newline\n"),stdout);




      fputs_unlocked (gettext (" * [-]ofdel      use delete characters for fill instead of NUL characters\n"),stdout);




      fputs_unlocked (gettext (" * [-]ofill      use fill (padding) characters instead of timing for delays\n"),stdout);




      fputs_unlocked (gettext (" * [-]olcuc      translate lowercase characters to uppercase\n"),stdout);




      fputs_unlocked (gettext (" * [-]onlcr      translate newline to carriage return-newline\n"),stdout);




      fputs_unlocked (gettext (" * [-]onlret     newline performs a carriage return\n"),stdout);




      fputs_unlocked (gettext (" * [-]onocr      do not print carriage returns in the first column\n"),stdout);



      fputs_unlocked (gettext ("   [-]opost      postprocess output\n"),stdout);



      fputs_unlocked (gettext (" * tabN          horizontal tab delay style, N in [0..3]\n * tabs          same as tab0\n * -tabs         same as tab3\n"),stdout);
      ...
      fputs_unlocked (gettext (" * vtN           vertical tab delay style, N in [0..1]\n"),stdout);
      ...
      fputs_unlocked (gettext (" * crtkill       kill all line by obeying the echoprt and echoe settings\n * -crtkill      kill all line by obeying the echoctl and echok settings\n"),stdout);





      fputs_unlocked (gettext (" * [-]ctlecho    echo control characters in hat notation ('^c')\n"),stdout);



      fputs_unlocked (gettext ("   [-]echo       echo input characters\n"),stdout);



      fputs_unlocked (gettext (" * [-]echoctl    same as [-]ctlecho\n"),stdout);



      fputs_unlocked (gettext ("   [-]echoe      same as [-]crterase\n   [-]echok      echo a newline after a kill character\n"),stdout);




      fputs_unlocked (gettext (" * [-]echoke     same as [-]crtkill\n"),stdout);



      fputs_unlocked (gettext ("   [-]echonl     echo newline even if not echoing other characters\n"),stdout);



      fputs_unlocked (gettext (" * [-]echoprt    echo erased characters backward, between '\\' and '/'\n"),stdout);




      fputs_unlocked (gettext (" * [-]extproc    enable \"LINEMODE\"; useful with high latency links\n"),stdout);




      fputs_unlocked (gettext (" * [-]flusho     discard output\n"),stdout);



      printf (gettext ("   [-]icanon     enable special characters: %s\n   [-]iexten     enable non-POSIX special characters\n"), "erase, kill"
    ...
    ", rprnt"
      ...
      fputs_unlocked (gettext ("   [-]isig       enable interrupt, quit, and suspend special characters\n   [-]noflsh     disable flushing after interrupt and quit special characters\n"),stdout);




      fputs_unlocked (gettext (" * [-]prterase   same as [-]echoprt\n"),stdout);




      fputs_unlocked (gettext (" * [-]tostop     stop background jobs that try to write to the terminal\n"),stdout);




      fputs_unlocked (gettext (" * [-]xcase      with icanon, escape with '\\' for uppercase characters\n"),stdout);
      ...
      fputs_unlocked (gettext (" * [-]LCASE      same as [-]lcase\n"),stdout);



      fputs_unlocked (gettext ("   cbreak        same as -icanon\n   -cbreak       same as icanon\n"),stdout);



      fputs_unlocked (gettext ("   cooked        same as brkint ignpar istrip icrnl ixon opost isig\n                 icanon, eof and eol characters to their default values\n   -cooked       same as raw\n"),stdout);




      printf (gettext ("   crt           same as %s\n"), "echoe"
    ...
    " echoke"
      ...
      printf (gettext ("   dec           same as %s intr ^c erase 0177\n                 kill ^u\n"), "echoe"
    ...
    " -ixany"
      ...
      fputs_unlocked (gettext (" * [-]decctlq    same as [-]ixany\n"),stdout);



      fputs_unlocked (gettext ("   ek            erase and kill characters to their default values\n   evenp         same as parenb -parodd cs7\n   -evenp        same as -parenb cs8\n"),stdout);





      fputs_unlocked (gettext (" * [-]lcase      same as xcase iuclc olcuc\n"),stdout);



      fputs_unlocked (gettext ("   litout        same as -parenb -istrip -opost cs8\n   -litout       same as parenb istrip opost cs7\n"),stdout);



      printf (gettext ("   nl            same as %s\n   -nl           same as %s\n"), "-icrnl"
  ...
  , "icrnl -inlcr -igncr"
   ...
   " -onlret"
      ...
      fputs_unlocked (gettext ("   oddp          same as parenb parodd cs7\n   -oddp         same as -parenb cs8\n   [-]parity     same as [-]evenp\n   pass8         same as -parenb -istrip cs8\n   -pass8        same as parenb istrip cs7\n"),stdout);
      ...
      printf (gettext ("   raw           same as -ignbrk -brkint -ignpar -parmrk -inpck -istrip\n                 -inlcr -igncr -icrnl -ixon -ixoff -icanon -opost\n                 -isig%s min 1 time 0\n   -raw          same as cooked\n"),
   ...
   " -xcase"
      ...
      printf (gettext ("   sane          same as cread -ignbrk brkint -inlcr -igncr icrnl\n                 icanon iexten echo echoe echok -echonl -noflsh\n                 %s\n                 %s\n                 %s,\n                 all special characters to their default values\n"),
...
static void
apply_settings (_Bool checking, const char *device_name,
                char * const *settings, int n_settings,
                struct termios *mode, _Bool *speed_was_set,
  ...
  for (int k = 1; k < n_settings; k++)
    {
      ...
      if ((strcmp (arg, "drain") == 0))
        {
          tcsetattr_options = reversed ? 0 : 1;
          continue;
        }
      for (i = 0; mode_info[i].name != ((void*)0); ++i)
        {
          if ((strcmp (arg, mode_info[i].name) == 0))
            {
      ...
      if (!match_found)
        {
          for (i = 0; control_info[i].name != ((void*)0); ++i)
            {
              if ((strcmp (arg, control_info[i].name) == 0))
                {
                  do { if (k == n_settings - 1 || ! settings[k+1]) { error (0, 0, gettext ("missing argument to %s"), quote (arg)); usage (1); } } while (0);
                  match_found = 1;
                  ++k;
...
# 1198 "stty.c"
          else if ((strcmp (arg, "rows") == 0))
            {
              ...
              if (checking)
                continue;
              set_window_size (integer_arg (settings[k], 2147483647), -1,
                               device_name);
            }
          else if ((strcmp (arg, "cols") == 0)
                   || (strcmp (arg, "columns") == 0))
            {
              do { if (k == n_settings - 1 || ! settings[k+1]) { error (0, 0, gettext ("missing argument to %s"), quote (arg)); usage (1); } } while (0);
              ++k;
              if (checking)
                continue;
              set_window_size (-1, integer_arg (settings[k], 2147483647),
                               device_name);
            }
          else if ((strcmp (arg, "size") == 0))
            {
              ...
              current_col = 0;
              display_window_size (0, device_name);
            }
          ...
          else if ((strcmp (arg, "line") == 0))
            {
              ...
              do { if (k == n_settings - 1 || ! settings[k+1]) { error (0, 0, gettext ("missing argument to %s"), quote (arg)); usage (1); } } while (0);
              ++k;
              mode->c_line = value = integer_arg (settings[k], (9223372036854775807L *2UL+1UL));
              if (mode->c_line != value)
                error (0, 0, gettext ("invalid line discipline %s"),
                       quote (settings[k]));
              *require_set_attr = 1;
            }
...
main (int argc, char **argv)
{
  ...
  int opti = 1;
  _Bool require_set_attr;
  _Bool speed_was_set __attribute__ ((__unused__));
...
# 1308 "stty.c"
  while ((optc = getopt_long (argc - argi, argv + argi, "-agF:",
         ...
         != -1)
    {
      switch (optc)
        ...
        case 'F':
          if (file_name)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"only one device may be specified\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("only one device may be specified")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("only one device may be specified")), ((0) ? (void) 0 : __builtin_unreachable ()))));
          file_name = optarg;
          break;

        case GETOPT_HELP_CHAR: usage (0); break;;

        case GETOPT_VERSION_CHAR: version_etc (stdout, "stty", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;

        default:


          if (! (strcmp (argv[argi + opti], "-drain") == 0)
              && ! (strcmp (argv[argi + opti], "drain") == 0))
            noargs = 0;
  ...
  if (verbose_output && recoverable_output)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"the options for verbose and stty-readable output styles are\\n\" \"mutually exclusive\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("the options for verbose and stty-readable output styles are\n" "mutually exclusive")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("the options for verbose and stty-readable output styles are\n" "mutually exclusive")), ((0) ? (void) 0 : __builtin_unreachable ()))));




  if (!noargs && (verbose_output || recoverable_output))
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"when specifying an output style, modes may not be set\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("when specifying an output style, modes may not be set")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("when specifying an output style, modes may not be set")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (!noargs && !verbose_output && !recoverable_output)
    {
      ...
      apply_settings ( 1, device_name, argv, argc,
  ...
  if (file_name)
    {
      int fdflags;
      if (fd_reopen (0, device_name, 00 | 04000, 0) < 0)
      ...
      if ((fdflags = rpl_fcntl (0, 3)) == -1
          || rpl_fcntl (0, 4, fdflags & ~04000) < 0)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"%s: couldn't reset non-blocking mode\"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("%s: couldn't reset non-blocking mode"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("%s: couldn't reset non-blocking mode"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  speed_was_set = 0;
  require_set_attr = 0;
  apply_settings ( 0, device_name, argv, argc,
  ...
  if (require_set_attr)
    {
      ...
      if (tcsetattr (0, tcsetattr_options, &mode))
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
# 1420 "stty.c"
...
# 1430 "stty.c"
      if (memcmp (&mode, &new_mode, sizeof (mode)) != 0)
        {
# 1442 "stty.c"
          new_mode.c_cflag &= (~002003600000);
          if (speed_was_set || memcmp (&mode, &new_mode, sizeof (mode)) != 0)

            {
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"%s: unable to perform all requested operations\"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("%s: unable to perform all requested operations"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("%s: unable to perform all requested operations"), quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
# 1458 "stty.c"
            }
        }
    }
...
static _Bool
set_mode (struct mode_info const *info, _Bool reversed, struct termios *mode)
{
  tcflag_t *bitsp;

  if (reversed && (info->flags & 4) == 0)
    return 0;
  ...
  if (bitsp == ((void*)0))
    {
      ...
      if ((strcmp (info->name, "evenp") == 0) || (strcmp (info->name, "parity") == 0))
        {
          if (reversed)
            mode->c_cflag = (mode->c_cflag & ~0000400 & ~0000060) | 0000060;
          else
            mode->c_cflag = (mode->c_cflag & ~0001000 & ~0000060) | 0000400 | 0000040;
        }
      else if ((strcmp (info->name, "oddp") == 0))
        {
          if (reversed)
            mode->c_cflag = (mode->c_cflag & ~0000400 & ~0000060) | 0000060;
          else
            mode->c_cflag = (mode->c_cflag & ~0000060) | 0000040 | 0001000 | 0000400;
        }
      else if ((strcmp (info->name, "nl") == 0))
        {
          if (reversed)
            {
              mode->c_iflag = (mode->c_iflag | 0000400) & ~0000100 & ~0000200;
            ...
            {
              mode->c_iflag = mode->c_iflag & ~0000400;

              mode->c_oflag = mode->c_oflag & ~0000004;

            }
        }
      else if ((strcmp (info->name, "ek") == 0))
        {
          mode->c_cc[2] = 0177;
          mode->c_cc[3] = ('u'&037);
        }
      else if ((strcmp (info->name, "sane") == 0))
        sane_mode (mode);
      else if ((strcmp (info->name, "cbreak") == 0))
        {
          if (reversed)
            mode->c_lflag |= 0000002;
          else
            mode->c_lflag &= ~0000002;
        }
      else if ((strcmp (info->name, "pass8") == 0))
        {
          if (reversed)
            {
              mode->c_cflag = (mode->c_cflag & ~0000060) | 0000040 | 0000400;
              mode->c_iflag |= 0000040;
            }
          else
            {
              mode->c_cflag = (mode->c_cflag & ~0000400 & ~0000060) | 0000060;
      ...
      else if ((strcmp (info->name, "litout") == 0))
        {
          if (reversed)
            {
              mode->c_cflag = (mode->c_cflag & ~0000060) | 0000040 | 0000400;
              mode->c_iflag |= 0000040;
              mode->c_oflag |= 0000001;
            }
          else
            {
              mode->c_cflag = (mode->c_cflag & ~0000400 & ~0000060) | 0000060;
      ...
      else if ((strcmp (info->name, "raw") == 0) || (strcmp (info->name, "cooked") == 0))
        {
          if ((info->name[0] == 'r' && reversed)
              || (info->name[0] == 'c' && !reversed))
            {

              mode->c_iflag |= 0000002 | 0000004 | 0000040 | 0000400 | 0002000;
              mode->c_oflag |= 0000001;
              mode->c_lflag |= 0000001 | 0000002;
      ...
      else if ((strcmp (info->name, "decctlq") == 0))
        {
          if (reversed)
            mode->c_iflag |= 0004000;
          else
            mode->c_iflag &= ~0004000;
        }


      else if ((strcmp (info->name, "tabs") == 0))
        {
          if (reversed)
            mode->c_oflag = (mode->c_oflag & ~0014000) | 0014000;
          else
            mode->c_oflag = (mode->c_oflag & ~0014000) | 0000000;
        }
# 1622 "stty.c"
      else if ((strcmp (info->name, "lcase") == 0)
               || (strcmp (info->name, "LCASE") == 0))
        {
          if (reversed)
            {
              mode->c_lflag &= ~0000004;
              mode->c_iflag &= ~0001000;
              mode->c_oflag &= ~0000002;
            }
          else
            {
              mode->c_lflag |= 0000004;
      ...
      else if ((strcmp (info->name, "crt") == 0))
        mode->c_lflag |= 0000020
          ...
          ;
      else if ((strcmp (info->name, "dec") == 0))
        {
          mode->c_cc[0] = 3;
          mode->c_cc[2] = 127;
          mode->c_cc[3] = 21;
                  ...
                  struct termios *mode)
{
  ...
  if ((strcmp (info->name, "min") == 0) || (strcmp (info->name, "time") == 0))
    value = integer_arg (arg, ((cc_t) (! (! ((cc_t) 0 < (cc_t) -1)) ? (cc_t) -1 : ((((cc_t) 1 << ((sizeof (cc_t) * 8) - 2)) - 1) * 2 + 1))));
...
{
  int err = ioctl (fd, 0x5413, (char *) win);
  return err;
}
...
set_window_size (int rows, int cols, char const *device_name)
{
  ...
  if (cols >= 0)
    win.ws_col = cols;
# 1777 "stty.c"
  if (ioctl (0, 0x5414, (char *) &win))
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, device_name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
}

static void
display_window_size (_Bool fancy, char const *device_name)
{
  struct winsize win;
...
screen_columns (void)
{
...

  {
    ...
    if (!(col_string != ((void*)0)
          ...
          && 0 < n_columns
          && n_columns <= 2147483647))
      n_columns = 80;
    return n_columns;
  }
}

static tcflag_t * __attribute__ ((__pure__))
...
# 1918 "stty.c"
  ...
  if ((mode->c_lflag & 0000002) == 0)
    {
      wrapf ("min = %lu; time = %lu;\n",
             (unsigned long int) mode->c_cc[6],
  ...
  for (i = 0; mode_info[i].name != ((void*)0); ++i)
    {
      ...
      ((void) sizeof ((bitsp) ? 1 : 0), __extension__ ({ if (bitsp) ; else __assert_fail ("bitsp", "stty.c", 1954, __extension__ __PRETTY_FUNCTION__); }));
  ...
  for (i = 0; mode_info[i].name != ((void*)0); ++i)
    {
      ...
      bitsp = mode_type_flag (mode_info[i].type, mode);
      mask = mode_info[i].mask ? mode_info[i].mask : mode_info[i].bits;
      ((void) sizeof ((bitsp) ? 1 : 0), __extension__ ({ if (bitsp) ; else __assert_fail ("bitsp", "stty.c", 2038, __extension__ __PRETTY_FUNCTION__); }));
      if ((*bitsp & mask) == mode_info[i].bits)
        wrapf ("%s", mode_info[i].name);
...
static void
display_speed (struct termios *mode, _Bool fancy)
{
                  ...
                  char delim)
{
  ...
  (*__errno_location ()) = 0;
  ul = strtoul (s, p, base);
  if ((*__errno_location ()) || **p != delim || *p == s || (tcflag_t) ul != ul)
    return -1;
  *result = ul;
  return 0;
}
...
strtoul_cc_t (char const *s, int base, char **p, cc_t *result, char delim)
{
  ...
  (*__errno_location ()) = 0;
  ul = strtoul (s, p, base);
  if ((*__errno_location ()) || **p != delim || *p == s || (cc_t) ul != ul)
    return -1;
  *result = ul;
  return 0;
}
...
static struct speed_map const speeds[] =
{
  ...
  {"115200", 0010002, 115200},


  {"230400", 0010003, 230400},


  {"460800", 0010004, 460800},


  {"500000", 0010005, 500000},


  {"576000", 0010006, 576000},


  {"921600", 0010007, 921600},


  {"1000000", 0010010, 1000000},


  {"1152000", 0010011, 1152000},


  {"1500000", 0010012, 1500000},


  {"2000000", 0010013, 2000000},


  {"2500000", 0010014, 2500000},


  {"3000000", 0010015, 3000000},


  {"3500000", 0010016, 3500000},


  {"4000000", 0010017, 4000000},

  {((void*)0), 0, 0}
};

static speed_t __attribute__ ((__pure__))
...
static unsigned long int __attribute__ ((__pure__))
  ...
  for (i = 0; mode_info[i].name != ((void*)0); ++i)
    {
      ...
      if (mode_info[i].flags & 1)
        {
          bitsp = mode_type_flag (mode_info[i].type, mode);
          ((void) sizeof ((bitsp) ? 1 : 0), __extension__ ({ if (bitsp) ; else __assert_fail ("bitsp", "stty.c", 2251, __extension__ __PRETTY_FUNCTION__); }));
          *bitsp = (*bitsp & ~mode_info[i].mask) | mode_info[i].bits;
        }
      else if (mode_info[i].flags & 2)
        {
          bitsp = mode_type_flag (mode_info[i].type, mode);
          ((void) sizeof ((bitsp) ? 1 : 0), __extension__ ({ if (bitsp) ; else __assert_fail ("bitsp", "stty.c", 2257, __extension__ __PRETTY_FUNCTION__); }));
          *bitsp = *bitsp & ~mode_info[i].mask & ~mode_info[i].bits;
        }
    }
}
