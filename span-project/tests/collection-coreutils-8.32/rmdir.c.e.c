  ...
  while (1)
    {
      ...
      if (verbose)
        prog_fprintf (stdout, gettext ("removing directory, %s"), quotearg_style (shell_escape_always_quoting_style, dir));
      ...
      if (! ok)
        {
          ...
          if (ignorable_failure (rmdir_errno, dir))
          ...
          else
            {
              ...
              error (0, rmdir_errno, gettext ("failed to remove directory %s"),
