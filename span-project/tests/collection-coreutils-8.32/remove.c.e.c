...
# 154 "remove.c"
  {
    if (faccessat (fd_cwd, file, 2, 0x200) == 0)
      return 0;

    return (*__errno_location ()) == 13 ? 1 : -1;
  }
}
# 175 "remove.c"
static enum RM_status
prompt (FTS const *fts, FTSENT const *ent, _Bool is_dir,
  ...
  if (write_protected || x->interactive == RMI_ALWAYS)
    {
      ...
      char const *quoted_name = quotearg_style (shell_escape_always_quoting_style, full_name);
      ...
      if (dirent_type == DT_DIR
        ...
        fprintf (stderr,
                 (write_protected
                  ...
                  : gettext ("%s: descend into directory %s? ")),
      ...
      else
        {
          ...
          fprintf (stderr,
                   (write_protected
                    ...
                    ? gettext ("%s: remove write-protected %s %s? ")
                    : gettext ("%s: remove %s %s? ")),
...
static enum RM_status
excise (FTS *fts, FTSENT *ent, struct rm_options const *x, _Bool is_dir)
{
  ...
  if (ent->fts_info == 4
      && ((*__errno_location ()) == 39 || (*__errno_location ()) == 21 || (*__errno_location ()) == 20
          || (*__errno_location ()) == 17)
      && (ent->fts_errno == 1 || ent->fts_errno == 13))
    (*__errno_location ()) = ent->fts_errno;
  error (0, (*__errno_location ()), gettext ("cannot remove %s"), quotearg_style (shell_escape_always_quoting_style, ent->fts_path));
...
rm_fts (FTS *fts, FTSENT *ent, struct rm_options const *x)
{
  switch (ent->fts_info)
    ...
    case 1:
      if (! x->recursive
               ...
               && is_empty_dir (fts->fts_cwd_fd, ent->fts_accpath)))
        {
          ...
          int err = x->remove_empty_directories ? 39 : 21;
          error (0, err, gettext ("cannot remove %s"), quotearg_style (shell_escape_always_quoting_style, ent->fts_path));
          mark_ancestor_dirs (ent);
      ...
      if (ent->fts_level == 0)
        {
          ...
          if (dot_or_dotdot (last_component (ent->fts_accpath)))
            {
              error (0, 0,
                     gettext ("refusing to remove %s or %s directory: skipping %s"),
                     quotearg_n_style (0, shell_escape_always_quoting_style, "."), quotearg_n_style (1, shell_escape_always_quoting_style, ".."),
                     quotearg_n_style (2, shell_escape_always_quoting_style, ent->fts_path));
              fts_skip_tree (fts, ent);
              return RM_ERROR;
            }
          ...
          if ((x->root_dev_ino && ((*ent->fts_statp).st_ino == (*x->root_dev_ino).st_ino && (*ent->fts_statp).st_dev == (*x->root_dev_ino).st_dev)))
            {
              do { if ((strcmp (ent->fts_path, "/") == 0)) error (0, 0, gettext ("it is dangerous to operate recursively on %s"), quotearg_style (shell_escape_always_quoting_style, ent->fts_path)); else error (0, 0, gettext ("it is dangerous to operate recursively on %s (same as %s)"), quotearg_n_style (0, shell_escape_always_quoting_style, ent->fts_path), quotearg_n_style (1, shell_escape_always_quoting_style, "/")); error (0, 0, gettext ("use --no-preserve-root to override this failsafe")); } while (0);
              fts_skip_tree (fts, ent);
              return RM_ERROR;
            }
          ...
          if (x->preserve_all_root)
            {
              _Bool failed = 0;
              char *parent = file_name_concat (ent->fts_accpath, "..", ((void*)0));
              struct stat statbuf;

              if (!parent || lstat (parent, &statbuf))
                {
                  error (0, 0,
                         gettext ("failed to stat %s: skipping %s"),
                         quotearg_n_style (0, shell_escape_always_quoting_style, parent),
                         quotearg_n_style (1, shell_escape_always_quoting_style, ent->fts_accpath));
                  failed = 1;
                }
...

      {
        ...
        enum RM_status s = prompt (fts, ent, 1 , x,
    ...
    case 3:
      {
        ...
        if (ent->fts_info == 6
          ...
          {
            mark_ancestor_dirs (ent);
            error (0, 0, gettext ("skipping %s, since it's on a different device"),
                   quotearg_style (shell_escape_always_quoting_style, ent->fts_path));
            return RM_ERROR;
          }

        _Bool is_dir = ent->fts_info == 6 || ent->fts_info == 4;
        enum RM_status s = prompt (fts, ent, is_dir, x, PA_REMOVE_DIR, ((void*)0));
        if (s != RM_OK)
          return s;
    ...
    case 7:
      ...
      error (0, ent->fts_errno, gettext ("traversal failed: %s"),
             quotearg_n_style_colon (0, shell_escape_quoting_style, ent->fts_path));
      fts_skip_tree (fts, ent);
      return RM_ERROR;

    default:
      error (0, 0, gettext ("unexpected failure: fts_info=%d: %s\n" "please report to %s"),
      ...
      while (1)
        {
          ...
          ((void) sizeof ((((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((s) == RM_OK || (s) == RM_USER_DECLINED || (s) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (s)", "remove.c", 609, __extension__ __PRETTY_FUNCTION__); }));
          do { if ((s) == RM_ERROR || ((s) == RM_USER_DECLINED && (rm_status) == RM_OK)) (rm_status) = (s); } while (0);
        }
