...
set_fields (const char *fieldstr, unsigned int options)
{
  ...
  if ((options & SETFLD_ALLOW_DASH) && (strcmp (fieldstr, "-") == 0))
    {
      value = 1;
      lhs_specified = 1;
      dash_found = 1;
      ...
      if (*fieldstr == '-')
        {
          ...
          if (dash_found)
            do { error (0, 0, ((options & SETFLD_ERRMSG_USE_POS) ?gettext ("invalid byte or character range") :gettext ("invalid field range"))); usage (1); } while (0);
          ...
          if (lhs_specified && !value)
            do { error (0, 0, ((options & SETFLD_ERRMSG_USE_POS) ?gettext ("byte/character positions are numbered from 1") :gettext ("fields are numbered from 1"))); usage (1); } while (0);
      ...
      else if (((unsigned int) (*fieldstr) - '0' <= 9))
        {
          ...
          if (!( (void) (&(value) == (uintmax_t *) ((void*)0)), ((!!sizeof (struct { _Static_assert (! (! ((uintmax_t) 0 < (uintmax_t) -1)), "verify_expr (" "! TYPE_SIGNED (uintmax_t)" ", " "(((uintmax_t) -1 / 10 < (value) || (uintmax_t) ((value) * 10 + (*fieldstr - '0')) < (value)) ? false : (((value) = (value) * 10 + (*fieldstr - '0')), true))" ")"); int _gl_dummy; })) ? ((((uintmax_t) -1 / 10 < (value) || (uintmax_t) ((value) * 10 + (*fieldstr - '0')) < (value)) ? 0 : (((value) = (value) * 10 + (*fieldstr - '0')), 1))) : ((((uintmax_t) -1 / 10 < (value) || (uintmax_t) ((value) * 10 + (*fieldstr - '0')) < (value)) ? 0 : (((value) = (value) * 10 + (*fieldstr - '0')), 1)))) )
              || value == (18446744073709551615UL))
            {
              ...
              error (0, 0, (options & SETFLD_ERRMSG_USE_POS)
                           ?gettext ("byte/character offset %s is too large")
                           :gettext ("field number %s is too large"),
                           quote (bad_num));
              free (bad_num);
              usage (1);
            }
        ...
        {
          error (0, 0, (options & SETFLD_ERRMSG_USE_POS)
                       ?gettext ("invalid byte/character position %s")
                       :gettext ("invalid field value %s"),
                       quote (fieldstr));
          usage (1);
        }
    }
  ...
  if (!n_frp)
    do { error (0, 0, ((options&SETFLD_ERRMSG_USE_POS) ?gettext ("missing list of byte/character positions") :gettext ("missing list of fields"))); usage (1); } while (0);
      ...
      for (size_t j = i + 1; j < n_frp; ++j)
        {
          if (frp[j].lo <= frp[i].hi)
            {
              frp[i].hi = (((frp[j].hi)>(frp[i].hi))?(frp[j].hi):(frp[i].hi));
  ...
  frp[n_frp - 1].lo = frp[n_frp - 1].hi = (18446744073709551615UL);
}
