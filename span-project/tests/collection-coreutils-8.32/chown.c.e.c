...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "HLPRcfhv", long_options, ((void*)0)))
         != -1)
    {
      switch (optc)
        ...
        case FROM_OPTION:
          {
            const char *e = parse_user_spec (optarg,
                                             ...
                                             ((void*)0), ((void*)0));
            if (e)
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, \"%s: %s\", e, quote (optarg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, "%s: %s", e, quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, "%s: %s", e, quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))));
            break;
          }
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "chown", "GNU coreutils", Version, ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (reference_file)
  ...
  else
    {
      ...
      if (e)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, \"%s: %s\", e, quote (argv[optind])), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, "%s: %s", e, quote (argv[optind])), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, "%s: %s", e, quote (argv[optind])), ((0) ? (void) 0 : __builtin_unreachable ()))));
