...
main (int argc, char **argv)
{
  ...
  parse_gnu_standard_options_only (argc, argv, "nohup", "GNU coreutils",
                                   Version, 0, usage, ("Jim Meyering"),
                                   (char const *) ((void*)0));
          ...
          if (out_fd < 0)
            {
              ...
              error (0, saved_errno, gettext ("failed to open %s"), quotearg_style (shell_escape_always_quoting_style, file));
              if (in_home)
  ...
  if (redirecting_stderr)
    {
      ...
      saved_stderr_fd = rpl_fcntl (2, 1030,
                               2 + 1);
  ...
  if (dup2 (saved_stderr_fd, 2) == 2)
    error (0, saved_errno, gettext ("failed to run command %s"), quotearg_style (shell_escape_always_quoting_style, *cmd));
