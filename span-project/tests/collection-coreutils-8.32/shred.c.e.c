           ...
           struct Options const *flags)
{
  ...
  if (flags->remove_file && ftruncate (fd, 0) != 0
      && (((((st.st_mode)) & 0170000) == (0100000)) || ((&st)->st_mode - (&st)->st_mode)))
    {
  ...
  while (len--)
    {
      ...
      ((void) sizeof ((p) ? 1 : 0), __extension__ ({ if (p) ; else __assert_fail ("p", "shred.c", 1002, __extension__ __PRETTY_FUNCTION__); }));
    ...
    for (size_t len = base_len (base); len != 0; len--)
      {
        ...
        base[len] = 0;
        _Bool rename_ok;
        while (! (rename_ok = (renameatu (-100, oldname, -100, newname,
                                          (1 << 0))
                               == 0))
               && (*__errno_location ()) == 17 && incname (base, len))
          continue;
        if (rename_ok)
          {
