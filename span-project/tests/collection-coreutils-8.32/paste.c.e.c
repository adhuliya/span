...
static void write_error (void) __attribute__ ((__noreturn__));
static void
write_error (void)
{
...
paste_parallel (size_t nfiles, char **fnamptr)
{
      ...
      if ((strcmp (fnamptr[files_open], "-") == 0))
      ...
      else
        {
          ...
          if (fileptr[files_open] == ((void*)0))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon (0, shell_escape_quoting_style, fnamptr[files_open])), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, fnamptr[files_open])), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, fnamptr[files_open])), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (opened_stdin && have_read_stdin)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"standard input is closed\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("standard input is closed")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("standard input is closed")), ((0) ? (void) 0 : __builtin_unreachable ()))));
              ...
              if (fileptr[i])
                {
                  ...
                  else if (rpl_fclose (fileptr[i]) == (-1))
                    {
                      error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, fnamptr[i]));
                      ok = 0;
                    }
  ...
  for (; nfiles; nfiles--, fnamptr++)
    {
      ...
      if (is_stdin)
      ...
      else
        {
          ...
          if (fileptr == ((void*)0))
            {
              error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, *fnamptr));
              ok = 0;
              continue;
            }
      ...
      if (ferror_unlocked (fileptr))
        {
          error (0, saved_errno, "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, *fnamptr));
          ok = 0;
        }
      if (is_stdin)
        clearerr_unlocked (fileptr);
      else if (rpl_fclose (fileptr) == (-1))
        {
          error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, *fnamptr));
          ok = 0;
        }
  ...
  while ((optc = getopt_long (argc, argv, "d:sz", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "paste", "GNU coreutils", Version, ("David M. Ihnat"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (collapse_escapes (delim_arg))
    {
      ...
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"delimiter list ends with an unescaped backslash: %s\"), quotearg_n_style_colon (0, c_maybe_quoting_style, delim_arg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("delimiter list ends with an unescaped backslash: %s"), quotearg_n_style_colon (0, c_maybe_quoting_style, delim_arg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("delimiter list ends with an unescaped backslash: %s"), quotearg_n_style_colon (0, c_maybe_quoting_style, delim_arg)), ((0) ? (void) 0 : __builtin_unreachable ()))));


    }

  _Bool ok = ((serial_merge ? paste_serial : paste_parallel)
             (nfiles, &argv[optind]));
