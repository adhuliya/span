...
main (int argc, char **argv)
{
  ...
  if (toprint
          ...
          | 8 | 16))
    {
      ...
      if (uname (&name) == -1)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"cannot get system name\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("cannot get system name")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("cannot get system name")), ((0) ? (void) 0 : __builtin_unreachable ()))));
