...
enum
{
  STRIP_TRAILING_SLASHES_OPTION = 127 + 1
};
...
rm_option_init (struct rm_options *x)
{
  ...
  x->require_restore_cwd = 1;

  {
    static struct dev_ino dev_ino_buf;
    x->root_dev_ino = get_root_dev_ino (&dev_ino_buf);
    if (x->root_dev_ino == ((void*)0))
...
do_move (const char *source, const char *dest, const struct cp_options *x)
{
  ...
  _Bool ok = copy (source, dest, 0, x, &copy_into_self, &rename_succeeded);
      ...
      if (dir_to_remove != ((void*)0))
        {
          ...
          ((void) sizeof ((((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (status)", "mv.c", 239, __extension__ __PRETTY_FUNCTION__); }));
          if (status == RM_ERROR)
            ok = 0;
        }
    }
...
static _Bool
movefile (char *source, char *dest, _Bool dest_is_dir,
          const struct cp_options *x)
{
  _Bool ok;
# 265 "mv.c"
  ...
  if (dest_is_dir)
    {
      ...
      char *new_dest = file_name_concat (dest, src_basename, ((void*)0));
...
# 313 "mv.c"
      fputs_unlocked (gettext ("      --strip-trailing-slashes  remove any trailing slashes from each SOURCE\n                                 argument\n  -S, --suffix=SUFFIX          override the usual backup suffix\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "bfint:uvS:TZ", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "mv", "GNU coreutils", Version, ("Mike Parker"), ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (no_target_directory)
    ...
    {
      ((void) sizeof ((2 <= n_files) ? 1 : 0), __extension__ ({ if (2 <= n_files) ; else __assert_fail ("2 <= n_files", "mv.c", 457, __extension__ __PRETTY_FUNCTION__); }));
      if (n_files == 2)
        x.rename_errno = (renameatu (-100, file[0], -100, file[1],
                                     (1 << 0))
                          ? (*__errno_location ()) : 0);
      ...
      for (int i = 0; i < n_files; ++i)
        {
          ...
          ok &= movefile (file[i], target_directory, 1, &x);
        }
    }
  else
    {
      x.last_file = 1;
      ok = movefile (file[0], file[1], 0, &x);
    }
