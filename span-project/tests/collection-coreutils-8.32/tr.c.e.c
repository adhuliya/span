...
static _Bool squeeze_repeats = 0;


static _Bool delete = 0;


static _Bool complement = 0;
# 238 "tr.c"
static _Bool truncate_set1 = 0;
...
static _Bool in_squeeze_set[N_CHARS];





static _Bool in_delete_set[N_CHARS];
...
{
  {"complement", 0, ((void*)0), 'c'},
  {"delete", 0, ((void*)0), 'd'},
  {"squeeze-repeats", 0, ((void*)0), 's'},
  {"truncate-set1", 0, ((void*)0), 't'},
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [OPTION]... SET1 [SET2]\n"),
...
is_char_class_member (enum Char_class char_class, unsigned char c)
{
  ...
  switch (char_class)
    {
    case CC_ALNUM:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISalnum);
      break;
    case CC_ALPHA:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISalpha);
      break;
    case CC_BLANK:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISblank);
      break;
    case CC_CNTRL:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _IScntrl);
      break;
    case CC_DIGIT:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISdigit);
      break;
    case CC_GRAPH:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISgraph);
      break;
    case CC_LOWER:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISlower);
      break;
    case CC_PRINT:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISprint);
      break;
    case CC_PUNCT:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISpunct);
      break;
    case CC_SPACE:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISspace);
      break;
    case CC_UPPER:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISupper);
      break;
    case CC_XDIGIT:
      result = ((*__ctype_b_loc ())[(int) ((c))] & (unsigned short int) _ISxdigit);
      break;
    default:
      abort ();
    }
...
static enum Char_class __attribute__ ((__pure__))
look_up_char_class (char const *class_str, size_t len)
{
  enum Char_class i;

  for (i = 0; i < (sizeof (char_class_name) / sizeof *(char_class_name)); i++)
...
append_normal_char (struct Spec_list *list, unsigned char c)
{
  ...
  new->u.normal_char = c;
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 647, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
}
...
append_range (struct Spec_list *list, unsigned char first, unsigned char last)
{
  if (last < first)
    {
      ...
      error (0, 0,
       gettext ("range-endpoints of '%s-%s' are in reverse collating sequence order"),
             tmp1, tmp2);
      free (tmp1);
      free (tmp2);
      return 0;
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 677, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
                   ...
                   char const *char_class_str, size_t len)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 699, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
                      ...
                      count repeat_count)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 719, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
}
                    ...
                    char const *equiv_class_str, size_t len)
{
  ...
  ((void) sizeof ((list->tail) ? 1 : 0), __extension__ ({ if (list->tail) ; else __assert_fail ("list->tail", "tr.c", 741, __extension__ __PRETTY_FUNCTION__); }));
  list->tail->next = new;
  list->tail = new;
  return 1;
}
...
{
  ((void) sizeof ((start_idx + 1 < es->len) ? 1 : 0), __extension__ ({ if (start_idx + 1 < es->len) ; else __assert_fail ("start_idx + 1 < es->len", "tr.c", 782, __extension__ __PRETTY_FUNCTION__); }));
          ...
          if (digit_str_len == 0)
          ...
          else
            {
              ...
              if ((xstrtoumax (digit_str, &d_end, *digit_str == '0' ? 8 : 10,
                               repeat_count, ((void*)0))
                   != LONGINT_OK)
                  || ((18446744073709551615UL) - 1) < *repeat_count
                  || digit_str + digit_str_len != d_end)
                {
                  char *tmp = make_printable_str (digit_str, digit_str_len);
                  error (0, 0,
                         gettext ("invalid repeat count %s in [c*n] construct"),
                         quote (tmp));
                  free (tmp);
                  return -2;
                }
                          ...
                          if (star_digits_closebracket (es, i + 2))
                          ...
                          else
                            {
                              char *tmp = make_printable_str (opnd_str,
                                                              opnd_str_len);
                              error (0, 0, gettext ("invalid character class %s"),
                                     quote (tmp));
                              free (tmp);
                              return 0;
                            }
                          ...
                          if (star_digits_closebracket (es, i + 2))
                          ...
                          else
                            {
                              char *tmp = make_printable_str (opnd_str,
                                                              opnd_str_len);
                              error (0, 0,
               gettext ("%s: equivalence class operand must be a single character"),
                                     tmp);
                              free (tmp);
                              return 0;
                            }
...
{
  s->tail = s->tail->next;
  s->state = (((18446744073709551615UL) - 1) + 1);
}
# 1014 "tr.c"
static int
get_next (struct Spec_list *s, enum Upper_Lower_class *class)
{
  ...
  if (s->state == ((18446744073709551615UL) - 1))
    {
      s->tail = s->head->next;
      s->state = (((18446744073709551615UL) - 1) + 1);
    }
  ...
  switch (p->type)
    ...
    case RE_RANGE:
      if (s->state == (((18446744073709551615UL) - 1) + 1))
      ...
      if (s->state == (((18446744073709551615UL) - 1) + 1))
        {
          for (i = 0; i < N_CHARS; i++)
            if (is_char_class_member (p->u.char_class, i))
              break;
          ((void) sizeof ((i < N_CHARS) ? 1 : 0), __extension__ ({ if (i < N_CHARS) ; else __assert_fail ("i < N_CHARS", "tr.c", 1076, __extension__ __PRETTY_FUNCTION__); }));
          s->state = i;
        }
      ((void) sizeof ((is_char_class_member (p->u.char_class, s->state)) ? 1 : 0), __extension__ ({ if (is_char_class_member (p->u.char_class, s->state)) ; else __assert_fail ("is_char_class_member (p->u.char_class, s->state)", "tr.c", 1079, __extension__ __PRETTY_FUNCTION__); }));
      return_val = s->state;
...
validate_case_classes (struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  while (c1 != -1 && c2 != -1)
    {
      ...
      if (s2_new_element && class_s2 != UL_NONE
        ...
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"misaligned [:upper:] and/or [:lower:] construct\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("misaligned [:upper:] and/or [:lower:] construct")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("misaligned [:upper:] and/or [:lower:] construct")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  ((void) sizeof ((old_s1_len >= s1->length && old_s2_len >= s2->length) ? 1 : 0), __extension__ ({ if (old_s1_len >= s1->length && old_s2_len >= s2->length) ; else __assert_fail ("old_s1_len >= s1->length && old_s2_len >= s2->length", "tr.c", 1221, __extension__ __PRETTY_FUNCTION__); }));
  ...
  for (p = s->head->next; p; p = p->next)
    {
      ...
      switch (p->type)
        ...
        case RE_RANGE:
          ((void) sizeof ((p->u.range.last_char >= p->u.range.first_char) ? 1 : 0), __extension__ ({ if (p->u.range.last_char >= p->u.range.first_char) ; else __assert_fail ("p->u.range.last_char >= p->u.range.first_char", "tr.c", 1262, __extension__ __PRETTY_FUNCTION__); }));
      ...
      if (! (length <= new_length && new_length <= ((18446744073709551615UL) - 1)))
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"too many characters in set\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("too many characters in set")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("too many characters in set")), ((0) ? (void) 0 : __builtin_unreachable ()))));
      length = new_length;
    }
...
{
  struct E_string es;
  _Bool ok = unquote (s, &es) && build_spec_list (&es, spec_list);
  es_free (&es);
  return ok;
}
# 1368 "tr.c"
...
string2_extend (const struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  ((void) sizeof ((translating) ? 1 : 0), __extension__ ({ if (translating) ; else __assert_fail ("translating", "tr.c", 1374, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((s1->length > s2->length) ? 1 : 0), __extension__ ({ if (s1->length > s2->length) ; else __assert_fail ("s1->length > s2->length", "tr.c", 1375, __extension__ __PRETTY_FUNCTION__); }));
  ((void) sizeof ((s2->length > 0) ? 1 : 0), __extension__ ({ if (s2->length > 0) ; else __assert_fail ("s2->length > 0", "tr.c", 1376, __extension__ __PRETTY_FUNCTION__); }));
  ...
  switch (p->type)
    ...
    case RE_CHAR_CLASS:
      ...
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"when translating with string1 longer than string2,\\nthe latter string must not end with a character class\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("when translating with string1 longer than string2,\nthe latter string must not end with a character class")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("when translating with string1 longer than string2,\nthe latter string must not end with a character class")), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
validate (struct Spec_list *s1, struct Spec_list *s2)
{
  ...
  if (s1->n_indefinite_repeats > 0)
    {
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"the [c*] repeat construct may not appear in string1\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("the [c*] repeat construct may not appear in string1")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("the [c*] repeat construct may not appear in string1")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (s2)
    {
      ...
      if (s2->n_indefinite_repeats > 1)
        {
          ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"only one [c*] repeat construct may appear in string2\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("only one [c*] repeat construct may appear in string2")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("only one [c*] repeat construct may appear in string2")), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (translating)
        {
          if (s2->has_equiv_class)
            {
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"[=c=] expressions may not appear in string2 when translating\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("[=c=] expressions may not appear in string2 when translating")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("[=c=] expressions may not appear in string2 when translating")), ((0) ? (void) 0 : __builtin_unreachable ()))));
              ...
              if (!truncate_set1)
                {
                  ...
                  if (s2->length == 0)
                    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"when not truncating set1, string2 must be non-empty\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("when not truncating set1, string2 must be non-empty")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("when not truncating set1, string2 must be non-empty")), ((0) ? (void) 0 : __builtin_unreachable ()))));
          ...
          if (complement && s1->has_char_class
            ...
            {
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"when translating with complemented character classes,\\nstring2 must map all characters in the domain to one\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("when translating with complemented character classes,\nstring2 must map all characters in the domain to one")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("when translating with complemented character classes,\nstring2 must map all characters in the domain to one")), ((0) ? (void) 0 : __builtin_unreachable ()))));
...

        {
          if (s2->n_indefinite_repeats > 0)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"the [c*] construct may appear in string2 only when translating\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("the [c*] construct may appear in string2 only when translating")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("the [c*] construct may appear in string2 only when translating")), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
squeeze_filter (char *buf, size_t size, size_t (*reader) (char *, size_t))
{
  ...
  const int NOT_A_CHAR = 2147483647;
...
# 1558 "tr.c"
          ...
          if (out_len > 0
              && fwrite_unlocked (&buf[begin],1,out_len,stdout) != out_len)
...
{
  size_t nr = safe_read (0, buf, size);
  if (nr == ((size_t) -1))
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"read error\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("read error")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("read error")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  return nr;
}
...
static void
set_initialize (struct Spec_list *s, _Bool complement_this_set, _Bool *in_set)
{
  int c;
  ...
  while ((c = getopt_long (argc, argv, "+AcCdst", long_options, ((void*)0))) != -1)
    {
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "tr", "GNU coreutils", Version, ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (non_option_args < min_operands)
    {
      if (non_option_args == 0)
      ...
      else
        {
          ...
          fprintf (stderr, "%s\n",
                   gettext (squeeze_repeats ? "Two strings must be given when " "both deleting and squeezing repeats." : "Two strings must be given when translating."));
  ...
  if (max_operands < non_option_args)
    {
      ...
      if (non_option_args == 2)
        fprintf (stderr, "%s\n",
                 gettext ("Only one string may be given when " "deleting without squeezing repeats."));
      ...
      while (1)
        {
          ...
          if (nr == 0)
            break;
          if (fwrite_unlocked (io_buf,1,nr,stdout) != nr)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"write error\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("write error")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("write error")), ((0) ? (void) 0 : __builtin_unreachable ()))));
        }
    }
  ...
  else if (translating)
    {
      if (complement)
              ...
              if (!in_s1[i])
                {
                  ...
                  ((void) sizeof ((ch != -1 || truncate_set1) ? 1 : 0), __extension__ ({ if (ch != -1 || truncate_set1) ; else __assert_fail ("ch != -1 || truncate_set1", "tr.c", 1837, __extension__ __PRETTY_FUNCTION__); }));
                  if (ch == -1)
                    {
      ...
      else
        {
          ...
          ((void) sizeof ((c1 == -1 || truncate_set1) ? 1 : 0), __extension__ ({ if (c1 == -1 || truncate_set1) ; else __assert_fail ("c1 == -1 || truncate_set1", "tr.c", 1890, __extension__ __PRETTY_FUNCTION__); }));
        }
      if (squeeze_repeats)
        {
          set_initialize (s2, 0, in_squeeze_set);
          ...
          while (1)
            {
              ...
              if (bytes_read == 0)
                break;
              if (fwrite_unlocked (io_buf,1,bytes_read,stdout) != bytes_read)
                ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"write error\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("write error")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("write error")), ((0) ? (void) 0 : __builtin_unreachable ()))));
            }
        }
    }
