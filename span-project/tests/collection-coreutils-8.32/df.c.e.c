...
extern int _gl_cxxalias_dummy;
# 717 "../lib/wchar.h" 3
extern size_t rpl_mbrtowc (wchar_t *__restrict pwc, const char *__restrict s, size_t n, mbstate_t *__restrict ps);
...
extern int _gl_cxxalias_dummy;
# 752 "../lib/wchar.h" 3
extern size_t rpl_mbrlen (const char *__restrict s, size_t n, mbstate_t *__restrict ps);
...
static struct field_data_t field_data[] = {
  [SOURCE_FIELD] = { SOURCE_FIELD,
    "source", OTHER_FLD, "Filesystem", 14, MBS_ALIGN_LEFT, 0 },

  [FSTYPE_FIELD] = { FSTYPE_FIELD,
    "fstype", OTHER_FLD, "Type", 4, MBS_ALIGN_LEFT, 0 },

  [SIZE_FIELD] = { SIZE_FIELD,
    "size", BLOCK_FLD, "blocks", 5, MBS_ALIGN_RIGHT, 0 },

  [USED_FIELD] = { USED_FIELD,
    "used", BLOCK_FLD, "Used", 5, MBS_ALIGN_RIGHT, 0 },

  [AVAIL_FIELD] = { AVAIL_FIELD,
    "avail", BLOCK_FLD, "Available", 5, MBS_ALIGN_RIGHT, 0 },

  [PCENT_FIELD] = { PCENT_FIELD,
    "pcent", BLOCK_FLD, "Use%", 4, MBS_ALIGN_RIGHT, 0 },

  [ITOTAL_FIELD] = { ITOTAL_FIELD,
    "itotal", INODE_FLD, "Inodes", 5, MBS_ALIGN_RIGHT, 0 },

  [IUSED_FIELD] = { IUSED_FIELD,
    "iused", INODE_FLD, "IUsed", 5, MBS_ALIGN_RIGHT, 0 },

  [IAVAIL_FIELD] = { IAVAIL_FIELD,
    "iavail", INODE_FLD, "IFree", 5, MBS_ALIGN_RIGHT, 0 },

  [IPCENT_FIELD] = { IPCENT_FIELD,
    "ipcent", INODE_FLD, "IUse%", 4, MBS_ALIGN_RIGHT, 0 },

  [TARGET_FIELD] = { TARGET_FIELD,
    "target", OTHER_FLD, "Mounted on", 0, MBS_ALIGN_LEFT, 0 },

  [FILE_FIELD] = { FILE_FIELD,
    "file", OTHER_FLD, "File", 0, MBS_ALIGN_LEFT, 0 }
};
...
{
  {"all", 0, ((void*)0), 'a'},
  {"block-size", 1, ((void*)0), 'B'},
  {"inodes", 0, ((void*)0), 'i'},
  {"human-readable", 0, ((void*)0), 'h'},
  {"si", 0, ((void*)0), 'H'},
  {"local", 0, ((void*)0), 'l'},
  {"output", 2, ((void*)0), OUTPUT_OPTION},
  {"portability", 0, ((void*)0), 'P'},
  {"print-type", 0, ((void*)0), 'T'},
  {"sync", 0, ((void*)0), SYNC_OPTION},
  {"no-sync", 0, ((void*)0), NO_SYNC_OPTION},
  {"total", 0, ((void*)0), TOTAL_OPTION},
  {"type", 1, ((void*)0), 't'},
  {"exclude-type", 1, ((void*)0), 'x'},
  ...
  for (char *src = cell; src != srcend; src += n)
    {
      ...
      n = rpl_mbrtowc (&wc, src, srcbytes, &mbstate);
      _Bool ok = n <= srcbytes;
...
alloc_field (int f, const char *c)
{
  ...
  if (field_data[f].used)
    ((void) sizeof ((!"field used") ? 1 : 0), __extension__ ({ if (!"field used") ; else __assert_fail ("!\"field used\"", "df.c", 407, __extension__ __PRETTY_FUNCTION__); }));
  ...
  do
    {
      ...
      for (unsigned int i = 0; i < (sizeof (field_data) / sizeof *(field_data)); i++)
        {
          if ((strcmp (field_data[i].arg, s) == 0))
            {
              field = i;
              break;
            }
      ...
      if (field_data[field].used)
        {
          ...
          error (0, 0, gettext ("option --output: field %s used more than once"),
      ...
      switch (field)
        ...
        default:
          ((void) sizeof ((!"invalid field") ? 1 : 0), __extension__ ({ if (!"invalid field") ; else __assert_fail ("!\"invalid field\"", "df.c", 478, __extension__ __PRETTY_FUNCTION__); }));
        }
      s = comma;
    }
  while (s);
...
get_field_list (void)
{
  switch (header_mode)
    ...
    default:
      ((void) sizeof ((!"invalid header_mode") ? 1 : 0), __extension__ ({ if (!"invalid header_mode") ; else __assert_fail ("!\"invalid header_mode\"", "df.c", 547, __extension__ __PRETTY_FUNCTION__); }));
    }
}
  ...
  for (col = 0; col < ncolumns; col++)
    {
      ...
      if (columns[col]->field == SIZE_FIELD
                  ...
                  && !(human_output_opts & human_autoscale))))
        {
          ...
          if (rpl_asprintf (&cell, gettext ("%s-%s"), num, header) == -1)
            cell = ((void*)0);
        }
      ...
      columns[col]->width = (((columns[col]->width)>(cell_width))?(columns[col]->width):(cell_width));
    }
}
...
# 700 "df.c"
static void
filter_mount_list (_Bool devices_only)
{
  struct mount_entry *me;


  struct devlist *device_list = ((void*)0);
  int mount_list_size = 0;

  for (me = mount_list; me; me = me->me_next)
    mount_list_size++;

  devlist_table = hash_initialize (mount_list_size, ((void*)0),
                                 devlist_hash,
                                 devlist_compare,
                                 devlist_free);
  if (devlist_table == ((void*)0))
  ...
  for (me = mount_list; me;)
    {
      struct stat buf;
      struct mount_entry *discard_me = ((void*)0);
      ...
      if ((me->me_remote && show_local_fs)
      ...
      else
        {
          ...
          if (seen_dev)
            {
              _Bool target_nearer_root = strlen (seen_dev->me->me_mountdir)
                                        > strlen (me->me_mountdir);

              _Bool source_below_root = seen_dev->me->me_mntroot != ((void*)0)
                                       && me->me_mntroot != ((void*)0)
      ...
      if (discard_me)
      ...
      else
        {
          ...
          device_list = devlist;
          if (hash_insert (devlist_table, devlist) == ((void*)0))
            xalloc_die ();
...
static struct mount_entry const * __attribute__ ((__pure__))
...
{
  return n < (18446744073709551615UL) - 1;
}
# 857 "df.c"
static char const *
df_readable (_Bool negative, uintmax_t n, char *buf,
...
# 880 "df.c"
static void
add_uint_with_neg_flag (uintmax_t *dest, _Bool *dest_neg,
                        uintmax_t src, _Bool src_neg)
{
  if ((!(*dest_neg) == !(src_neg)))
    {
      *dest += src;
...
get_dev (char const *disk, char const *mount_point, char const* file,
         ...
         _Bool me_dummy, _Bool me_remote,
         ...
         _Bool process_all)
{
  ...
  if (!force_fsu && mount_point && ! ((((mount_point)[0]) == '/') || 0 != 0))
    return;
  ...
  if (force_fsu)
  ...
  else if (get_fs_usage (stat_file, disk, &fsu))
    {
      ...
      if (process_all && ((*__errno_location ()) == 13 || (*__errno_location ()) == 2))
        {
          if (! show_all_fs)
            return;

          fstype = "-";
          fsu.fsu_bavail_top_bit_set = 0;
          fsu.fsu_blocksize = fsu.fsu_blocks = fsu.fsu_bfree =
          fsu.fsu_bavail = fsu.fsu_files = fsu.fsu_ffree = (18446744073709551615UL);
        }
      else
        {
          error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, stat_file));
          exit_status = 1;
          return;
        }
    }
      ...
      if (stat (stat_file, &sb) == 0)
        {
          ...
          if (dev_me && ! (strcmp (dev_me->me_devname, disk) == 0)
  ...
  for (col = 0; col < ncolumns; col++)
    {
      char buf[((2 * sizeof (uintmax_t) * 8 * 146 / 485 + 1) * (16 + 1) - 16 + 1 + 3) + 2];
      char *cell;
      ...
      switch (columns[col]->field_type)
        ...
        default:
          v = ((void*)0);
          ((void) sizeof ((!"bad field_type") ? 1 : 0), __extension__ ({ if (!"bad field_type") ; else __assert_fail ("!\"bad field_type\"", "df.c", 1129, __extension__ __PRETTY_FUNCTION__); }));
        }
      ...
      switch (columns[col]->field)
        ...
        case SIZE_FIELD:
        case ITOTAL_FIELD:
          cell = xstrdup (df_readable (0, v->total, buf,
        ...
        case IPCENT_FIELD:
          {
            ...
            if (! known_value (v->used) || ! known_value (v->available))
              ;
                     ...
                     && v->used <= ((uintmax_t) (! (! ((uintmax_t) 0 < (uintmax_t) -1)) ? (uintmax_t) -1 : ((((uintmax_t) 1 << ((sizeof (uintmax_t) * 8) - 2)) - 1) * 2 + 1))) / 100
...
# 1227 "df.c"
        ...
        default:
          ((void) sizeof ((!"unhandled field") ? 1 : 0), __extension__ ({ if (!"unhandled field") ; else __assert_fail ("!\"unhandled field\"", "df.c", 1231, __extension__ __PRETTY_FUNCTION__); }));
        }

      if (!cell)
        ((void) sizeof ((!"empty cell") ? 1 : 0), __extension__ ({ if (!"empty cell") ; else __assert_fail ("!\"empty cell\"", "df.c", 1235, __extension__ __PRETTY_FUNCTION__); }));
...
get_disk (char const *disk)
{
  ...
  _Bool best_match_accessible = 0;
  _Bool eclipsed_device = 0;
  char const *file = disk;

  char *resolved = canonicalize_file_name (disk);
  if (resolved && ((((resolved)[0]) == '/') || 0 != 0))
    disk = resolved;

  size_t best_match_len = (18446744073709551615UL);
  for (me = mount_list; me; me = me->me_next)
    {
      ...
      if ((strcmp (disk, devname) == 0))
        {
          char *last_device = last_device_for_mount (me->me_mountdir);
          eclipsed_device = last_device && ! (strcmp (last_device, devname) == 0);
          ...
          if (! eclipsed_device
              && (! best_match_accessible || len < best_match_len))
            {
              ...
              _Bool this_match_accessible = 0;

              if (stat (me->me_mountdir, &disk_stats) == 0)
                best_match_accessible = this_match_accessible = 1;
  ...
  if (best_match)
    {
      get_dev (best_match->me_devname, best_match->me_mountdir, file, ((void*)0),
               best_match->me_type, best_match->me_dummy,
               best_match->me_remote, ((void*)0), 0);
      return 1;
    }
  else if (eclipsed_device)
    {
      error (0, 0, gettext ("cannot access %s: over-mounted by another device"),
...
get_point (const char *point, const struct stat *statp)
{
            ...
            if (stat (me->me_mountdir, &disk_stats) == 0)
            ...
            else
              {
                ...
                if ((*__errno_location ()) == 5)
                  {
                    error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, me->me_mountdir));
                    exit_status = 1;
                  }
  ...
  if (best_match)
  ...
  else
    {
      ...
      if (mp)
        {
          get_dev (((void*)0), mp, point, ((void*)0), ((void*)0), 0, 0, ((void*)0), 0);
          free (mp);
        }
    }
}
...
{
  if ((((((statp->st_mode)) & 0170000) == (0060000)) || ((((statp->st_mode)) & 0170000) == (0020000)))
      && get_disk (name))
    return;
...
get_all_entries (void)
{
  ...
  for (me = mount_list; me; me = me->me_next)
    get_dev (me->me_devname, me->me_mountdir, ((void*)0), ((void*)0), me->me_type,
             me->me_dummy, me->me_remote, ((void*)0), 1);
}
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Show information about the file system on which each FILE resides,\nor all file systems by default.\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -a, --all             include pseudo, duplicate, inaccessible file systems\n  -B, --block-size=SIZE  scale sizes by SIZE before printing them; e.g.,\n                           '-BM' prints sizes in units of 1,048,576 bytes;\n                           see SIZE format below\n  -h, --human-readable  print sizes in powers of 1024 (e.g., 1023M)\n  -H, --si              print sizes in powers of 1000 (e.g., 1.1G)\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -i, --inodes          list inode information instead of block usage\n  -k                    like --block-size=1K\n  -l, --local           limit listing to local file systems\n      --no-sync         do not invoke sync before getting usage info (default)\n"),stdout);
      ...
      fputs_unlocked (gettext ("      --output[=FIELD_LIST]  use the output format defined by FIELD_LIST,\n                               or print all fields if FIELD_LIST is omitted.\n  -P, --portability     use the POSIX output format\n      --sync            invoke sync before getting usage info\n"),stdout);





      fputs_unlocked (gettext ("      --total           elide all entries insignificant to available space,\n                          and produce a grand total\n"),stdout);



      fputs_unlocked (gettext ("  -t, --type=TYPE       limit listing to file systems of type TYPE\n  -T, --print-type      print file system type\n  -x, --exclude-type=TYPE   limit listing to file systems not of type TYPE\n  -v                    (ignored)\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  const char *msg_mut_excl = gettext ("options %s and %s are mutually exclusive");
  ...
  while (1)
    {
      ...
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "df", "GNU coreutils", Version, ("Torbjorn Granlund"), ("David MacKenzie"), ("Paul Eggert"), (char *) ((void*)0)); exit (0); break;;
      ...
      for (int i = optind; i < argc; ++i)
        {
          if (stat (argv[i], &stats[i - optind]))
            {
              error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, argv[i]));
              exit_status = 1;
              argv[i] = ((void*)0);
            }
          else if (! ((((stats[i - optind].st_mode)) & 0170000) == (0010000)))
            {

              int fd = open (argv[i], 00 | 0400);
              if (0 <= fd)
                close (fd);
            }
        }
  ...
  mount_list =
    read_file_system_list ((fs_select_list != ((void*)0)
                            || fs_exclude_list != ((void*)0)
                            || print_type
  ...
  if (mount_list == ((void*)0))
    {
      ...
      const char *warning = (status == 0 ? gettext ("Warning: ") : "");
  ...
  if (file_systems_processed)
  ...
  else
    {
      ...
      if (exit_status == 0)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"no file systems processed\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("no file systems processed")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("no file systems processed")), ((0) ? (void) 0 : __builtin_unreachable ()))));
    }
