                 ...
                 enum Change_status changed)
{
  ...
  switch (changed)
    ...
    case CH_NO_CHANGE_REQUESTED:
      ...
      printf (fmt, quotearg_style (shell_escape_always_quoting_style, file),
            ...
            if (mode_alloc <= new_mode_len)
              {
                ...
                mode = ((!!sizeof (struct { _Static_assert (sizeof *(mode) == 1, "verify_expr (" "sizeof *(mode) == 1" ", " "x2realloc (mode, &mode_alloc)" ")"); int _gl_dummy; })) ? (x2realloc (mode, &mode_alloc)) : (x2realloc (mode, &mode_alloc)));
              }
            mode[mode_len] = ',';
