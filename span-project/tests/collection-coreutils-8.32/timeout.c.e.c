      ...
      if (verbose)
        {
          ...
          error (0, 0, gettext ("sending signal %s to command %s"),
...
# 260 "timeout.c"
      fputs_unlocked (gettext ("  -v, --verbose  diagnose to stderr any signal sent upon timeout\n"),stdout);
...
parse_duration (const char *str)
{
  ...
  if (! (xstrtod (str, &ep, &duration, cl_strtod) || (*__errno_location ()) == 34)
...
block_cleanup_and_chld (int sigterm, sigset_t *old_set)
{
  ...
  if (sigprocmask (0, &block_set, old_set) != 0)
    error (0, (*__errno_location ()), gettext ("warning: sigprocmask"));
}
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "+k:s:v", long_options, ((void*)0))) != -1)
    {
      switch (c)
        {
        case 'k':
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "timeout", "GNU coreutils", Version, ("Padraig Brady"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (monitored_pid == -1)
  ...
  else
    {
      ...
      while ((wait_result = waitpid (monitored_pid, &status, 1)) == 0)
        sigsuspend (&cleanup_set);
