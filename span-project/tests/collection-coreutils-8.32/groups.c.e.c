  ...
  while ((optc = getopt_long (argc, argv, "", longopts, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "groups", "GNU coreutils", Version, ("David MacKenzie"), ("James Youngman"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (optind == argc)
    {
      ...
      if (!print_group_list (((void*)0), ruid, rgid, egid, 1, ' '))
        ok = 0;
      putchar_unlocked ('\n');
    }
  else
      ...
      for ( ; optind < argc; optind++)
        {
          ...
          if (!print_group_list (argv[optind], ruid, rgid, egid, 1, ' '))
            ok = 0;
          putchar_unlocked ('\n');
        }
    }
