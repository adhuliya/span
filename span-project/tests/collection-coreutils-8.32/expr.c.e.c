...
mbuiter_multi_next (struct mbuiter_multi *iter)
{
  ...
  if (is_basic (*iter->cur.ptr))
    ...
    {
      ((void) sizeof ((mbsinit (&iter->state)) ? 1 : 0), __extension__ ({ if (mbsinit (&iter->state)) ; else __assert_fail ("mbsinit (&iter->state)", "../lib/mbuiter.h", 150, __extension__ __PRETTY_FUNCTION__); }));
      iter->in_shift = 1;
    with_shift:
          ...
          if (iter->cur.bytes == 0)
            {
              ...
              iter->cur.bytes = 1;
              ((void) sizeof ((*iter->cur.ptr == '\0') ? 1 : 0), __extension__ ({ if (*iter->cur.ptr == '\0') ; else __assert_fail ("*iter->cur.ptr == '\\0'", "../lib/mbuiter.h", 178, __extension__ __PRETTY_FUNCTION__); }));
              ((void) sizeof ((iter->cur.wc == 0) ? 1 : 0), __extension__ ({ if (iter->cur.wc == 0) ; else __assert_fail ("iter->cur.wc == 0", "../lib/mbuiter.h", 179, __extension__ __PRETTY_FUNCTION__); }));
            }
          iter->cur.wc_valid = 1;
...
_Static_assert ((18446744073709551615UL) <= (9223372036854775807L *2UL+1UL), "verify (" "SIZE_MAX <= ULONG_MAX" ")");
  ...
  if ((__ctype_get_mb_cur_max ()) > 1)
    {
      ...
      for (((iter).cur.ptr = (s), (iter).in_shift = 0, memset (&(iter).state, '\0', sizeof (mbstate_t)), (iter).next_done = 0); (mbuiter_multi_next (&(iter)), !(((iter).cur).wc_valid && ((iter).cur).wc == 0)); ((iter).cur.ptr += (iter).cur.bytes, (iter).next_done = 0))
        {
          ++idx;
          if ((((iter).cur).bytes) == 1)
            {
          ...
          else
            {
              ...
              for (((aiter).cur.ptr = (accept), (aiter).in_shift = 0, memset (&(aiter).state, '\0', sizeof (mbstate_t)), (aiter).next_done = 0);
...
mbs_logical_substr (const char *s, size_t pos, size_t len)
{
  ...
  size_t llen = ((__ctype_get_mb_cur_max ()) > 1) ? mbslen (s) : blen;

  if (llen < pos || pos == 0 || len == 0 || len == (18446744073709551615UL))
    return xstrdup ("");


  size_t vlen = (((len)<(llen - pos + 1))?(len):(llen - pos + 1));
  ...
  if ((__ctype_get_mb_cur_max ()) == 1)
  ...
  else
    {
      ...
      mbui_iterator_t iter;
      size_t idx=1;
      for (((iter).cur.ptr = (s), (iter).in_shift = 0, memset (&(iter).state, '\0', sizeof (mbstate_t)), (iter).next_done = 0);
           (mbuiter_multi_next (&(iter)), !(((iter).cur).wc_valid && ((iter).cur).wc == 0)) && vlen > 0;
           ((iter).cur.ptr += (iter).cur.bytes, (iter).next_done = 0), ++idx)
        {
          ...
          --vlen;
          vlim = mempcpy (vlim, (iter).cur.ptr, (((iter).cur).bytes));
        }
    }
  *vlim = '\0';
  return v;
...
# 393 "expr.c"
      fputs_unlocked (gettext ("  + TOKEN                    interpret TOKEN as a string, even if it is a\n                               keyword like 'match' or an operator like '/'\n\n  ( EXPRESSION )             value of EXPRESSION\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  parse_long_options (argc, argv, "expr", "GNU coreutils", "8.32",
                      usage, ("Mike Parker"), ("James Youngman"), ("Paul Eggert"), (char const *) ((void*)0));



  unsigned int u_argc = argc;
  if (1 < u_argc && (strcmp (argv[1], "--") == 0))
    {
      --u_argc;
      ++argv;
    }
...
printv (VALUE *v)
{
  switch (v->type)
    {
    case integer:
      __gmpz_out_str (stdout, 10, v->u.i);
      putchar_unlocked ('\n');
      break;
    case string:
      puts (v->u.s);
...
null (VALUE *v)
{
  switch (v->type)
    {
    case integer:
      return ((v->u.i)->_mp_size < 0 ? -1 : (v->u.i)->_mp_size > 0) == 0;
    case string:
      {
        char const *cp = v->u.s;
        if (*cp == '\0')
    ...
    case string:
      {
        ...
        if (! looks_like_integer (s))
          return 0;
        if (__gmpz_init_set_str (v->u.i, s, 10) != 0 && !1)
          ((!!sizeof (struct { _Static_assert (EXPR_FAILURE, "verify_expr (" "EXPR_FAILURE" ", " "(error (EXPR_FAILURE, 34, \"%s\", (s)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_FAILURE, 34, "%s", (s)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_FAILURE, 34, "%s", (s)), ((0) ? (void) 0 : __builtin_unreachable ()))));
        free (s);
        v->type = integer;
        return 1;
      }
...
getsize (mpz_t i)
{
  if (((i)->_mp_size < 0 ? -1 : (i)->_mp_size > 0) < 0)
    return (18446744073709551615UL);
  if (__gmpz_fits_ulong_p (i))
    {
      unsigned long int ul = __gmpz_get_ui (i);
      if (ul < (18446744073709551615UL))
        return ul;
    }
  return (18446744073709551615UL) - 1;
}
...
docolon (VALUE *sv, VALUE *pv)
{
  ...
  re_regs.num_regs = 0;
  re_regs.start = ((void*)0);
  re_regs.end = ((void*)0);

  re_buffer.buffer = ((void*)0);
  re_buffer.allocated = 0;
  re_buffer.fastmap = fastmap;
  re_buffer.translate = ((void*)0);
  ...
  errmsg = rpl_re_compile_pattern (pv->u.s, strlen (pv->u.s), &re_buffer);
  if (errmsg)
    ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, \"%s\", (errmsg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, "%s", (errmsg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, "%s", (errmsg)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  re_buffer.newline_anchor = 0;

  matchlen = rpl_re_match (&re_buffer, sv->u.s, strlen (sv->u.s), 0, &re_regs);
  if (0 <= matchlen)
    {
      ...
      if (re_buffer.re_nsub > 0)
      ...
      else
        {
          ...
          size_t i = ((__ctype_get_mb_cur_max ()) == 1
                      ? matchlen
    ...
    ((!!sizeof (struct { _Static_assert (EXPR_FAILURE, "verify_expr (" "EXPR_FAILURE" ", " "(error (EXPR_FAILURE, (matchlen == -2 ? (*__errno_location ()) : 75), gettext (\"error in regular expression matcher\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_FAILURE, (matchlen == -2 ? (*__errno_location ()) : 75), gettext ("error in regular expression matcher")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_FAILURE, (matchlen == -2 ? (*__errno_location ()) : 75), gettext ("error in regular expression matcher")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (nextarg ("("))
    {
      v = eval (evaluate);
      if (nomoreargs ())
        ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, gettext (\"syntax error: expecting ')' after %s\"), quotearg_n_style (0, locale_quoting_style, *(args-1))), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, gettext ("syntax error: expecting ')' after %s"), quotearg_n_style (0, locale_quoting_style, *(args-1))), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, gettext ("syntax error: expecting ')' after %s"), quotearg_n_style (0, locale_quoting_style, *(args-1))), ((0) ? (void) 0 : __builtin_unreachable ()))));

      if (!nextarg (")"))
        ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, gettext (\"syntax error: expecting ')' instead of %s\"), quotearg_n_style (0, locale_quoting_style, *args)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, gettext ("syntax error: expecting ')' instead of %s"), quotearg_n_style (0, locale_quoting_style, *args)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, gettext ("syntax error: expecting ')' instead of %s"), quotearg_n_style (0, locale_quoting_style, *args)), ((0) ? (void) 0 : __builtin_unreachable ()))));

      return v;
    }

  if (nextarg (")"))
    ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, gettext (\"syntax error: unexpected ')'\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, gettext ("syntax error: unexpected ')'")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, gettext ("syntax error: unexpected ')'")), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (evaluate)
        {
          ...
          if (fxn != multiply && ((r->u.i)->_mp_size < 0 ? -1 : (r->u.i)->_mp_size > 0) == 0)
            ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, gettext (\"division by zero\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, gettext ("division by zero")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, gettext ("division by zero")), ((0) ? (void) 0 : __builtin_unreachable ()))));
          ((fxn == multiply ? __gmpz_mul
            : fxn == divide ? __gmpz_tdiv_q
            : __gmpz_tdiv_r)
           (l->u.i, l->u.i, r->u.i));
        }
      freev (r);
      ...
      if (evaluate)
        {
          if (!toarith (l) || !toarith (r))
            ((!!sizeof (struct { _Static_assert (EXPR_INVALID, "verify_expr (" "EXPR_INVALID" ", " "(error (EXPR_INVALID, 0, gettext (\"non-integer argument\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXPR_INVALID, 0, gettext ("non-integer argument")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXPR_INVALID, 0, gettext ("non-integer argument")), ((0) ? (void) 0 : __builtin_unreachable ()))));
          (fxn == plus ? __gmpz_add : __gmpz_sub) (l->u.i, l->u.i, r->u.i);
        }
      freev (r);
    }
}
