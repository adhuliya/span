...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Create a temporary file or directory, safely, and print its name.\nTEMPLATE must contain at least 3 consecutive 'X's in last component.\nIf TEMPLATE is not specified, use tmp.XXXXXXXXXX, and --tmpdir is implied.\n"),stdout);




      fputs_unlocked (gettext ("Files are created u+rw, and directories u+rwx, minus umask restrictions.\n"),stdout);


      fputs_unlocked ("\n",stdout);
      fputs_unlocked (gettext ("  -d, --directory     create a directory, not a file\n  -u, --dry-run       do not create anything; merely print a name (unsafe)\n  -q, --quiet         suppress diagnostics about file/dir-creation failure\n"),stdout);




      fputs_unlocked (gettext ("      --suffix=SUFF   append SUFF to TEMPLATE; SUFF must not contain a slash.\n                        This option is implied if TEMPLATE does not end in X\n"),stdout);



      fputs_unlocked (gettext ("  -p DIR, --tmpdir[=DIR]  interpret TEMPLATE relative to DIR; if DIR is not\n                        specified, use $TMPDIR if set, else /tmp.  With\n                        this option, TEMPLATE must not be an absolute name;\n                        unlike with -t, TEMPLATE may contain slashes, but\n                        mktemp creates only the final component\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -t                  interpret TEMPLATE as a single file name component,\n                        relative to a directory: $TMPDIR, if set; else the\n                        directory specified via -p; else /tmp [deprecated]\n"),stdout);
...
static int
mkstemp_len (char *tmpl, size_t suff_len, size_t x_len, _Bool dry_run)
{
  return gen_tempname_len (tmpl, suff_len, 0, dry_run ? 2 : 0,
                           x_len);
}

static int
mkdtemp_len (char *tmpl, size_t suff_len, size_t x_len, _Bool dry_run)
{
  return gen_tempname_len (tmpl, suff_len, 0, dry_run ? 2 : 1,
                           x_len);
}


static _Bool stdout_closed;
...
{
  char const *dest_dir;
  char const *dest_dir_arg = ((void*)0);
  _Bool suppress_file_err = 0;
  int c;
  unsigned int n_args;
  char *template;
  char *suffix = ((void*)0);
  _Bool use_dest_dir = 0;
  _Bool deprecated_t_option = 0;
  _Bool create_directory = 0;
  _Bool dry_run = 0;
  int status = 0;
  size_t x_count;
  size_t suffix_len;
  ...
  while ((c = getopt_long (argc, argv, "dp:qtuV", longopts, ((void*)0))) != -1)
    {
      switch (c)
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "mktemp", "GNU coreutils", Version, ("Jim Meyering"), ("Eric Blake"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (suffix)
    {
      ...
      if (!len || template[len - 1] != 'X')
        {
          ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"with --suffix, template %s must end in X\"), quote (template)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("with --suffix, template %s must end in X"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("with --suffix, template %s must end in X"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (suffix_len && last_component (suffix) != suffix)
    {
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid suffix %s, contains directory separator\"), quote (suffix)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid suffix %s, contains directory separator"), quote (suffix)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid suffix %s, contains directory separator"), quote (suffix)), ((0) ? (void) 0 : __builtin_unreachable ()))));


    }
  x_count = count_consecutive_X_s (template, suffix - template);
  if (x_count < 3)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"too few X's in template %s\"), quote (template)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("too few X's in template %s"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("too few X's in template %s"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (deprecated_t_option)
        {
          ...
          if (last_component (template) != template)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid template, %s, contains directory separator\"), quote (template)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid template, %s, contains directory separator"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid template, %s, contains directory separator"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      else
        {
          ...
          if (((((template)[0]) == '/') || 0 != 0))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid template, %s; with --tmpdir,\" \" it may not be absolute\"), quote (template)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid template, %s; with --tmpdir," " it may not be absolute"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid template, %s; with --tmpdir," " it may not be absolute"), quote (template)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (create_directory)
      ...
      if (err != 0)
        {
          if (!suppress_file_err)
            error (0, (*__errno_location ()), gettext ("failed to create directory via template %s"),
                   quote (template));
          status = 1;
        }
    }
  else
    {
      ...
      if (fd < 0 || (!dry_run && close (fd) != 0))
        {
          if (!suppress_file_err)
            error (0, (*__errno_location ()), gettext ("failed to create file via template %s"),
                   quote (template));
          status = 1;
        }
    }
  ...
  if (status == 0)
    {
      ...
      if (!dry_run && ((stdout_closed = 1), close_stream (stdout) != 0))
        {
