...
usage (int status)
{
  ...
  ((void) sizeof ((status == 0) ? 1 : 0), __extension__ ({ if (status == 0) ; else __assert_fail ("status == EXIT_SUCCESS", "echo.c", 40, __extension__ __PRETTY_FUNCTION__); }));
...
main (int argc, char **argv)
{
  ...
  _Bool posixly_correct = getenv ("POSIXLY_CORRECT");
  _Bool allow_options =
    (! posixly_correct
