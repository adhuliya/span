...
# 1 "../lib/diacrit.h" 1
# 20 "../lib/diacrit.h"
extern const char diacrit_base[];
extern const char diacrit_diac[];
# 29 "ptx.c" 2
...
# 22 "../lib/read-file.h"
# 1 "/home/codeman/.itsoflife/mydata/local/packages-live/llvm-clang9.0.0/build/lib/clang/9.0.0/include/stddef.h" 1 3
# 23 "../lib/read-file.h" 2


# 1 "../lib/stdio.h" 1
# 21 "../lib/stdio.h" 3
# 26 "../lib/read-file.h" 2
...
static _Bool gnu_extensions = 1;
static _Bool auto_reference = 0;
static _Bool input_reference = 0;
static _Bool right_reference = 0;
static ptrdiff_t line_width = 72;
static ptrdiff_t gap_size = 3;
static const char *truncation_string = "/";

static const char *macro_name = "xx";
static enum Format output_format = UNKNOWN_FORMAT;


static _Bool ignore_case = 0;
static const char *break_file = ((void*)0);
static const char *only_file = ((void*)0);
static const char *ignore_file = ((void*)0);
...
matcher_error (void)
{
  ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"error in regular expression matcher\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("error in regular expression matcher")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("error in regular expression matcher")), ((0) ? (void) 0 : __builtin_unreachable ()))));
}
      ...
      if (*string == '\\')
        {
          ...
          switch (*string)
            ...
            case 'x':
              value = 0;
              for (length = 0, string++;
                   length < 3 && ((*__ctype_b_loc ())[(int) ((to_uchar (*string)))] & (unsigned short int) _ISxdigit);
                   length++, string++)
...
compile_regex (struct regex_data *regex)
{
  ...
  pattern->buffer = ((void*)0);
  pattern->allocated = 0;
  pattern->fastmap = regex->fastmap;
  pattern->translate = ignore_case ? folded_chars : ((void*)0);

  message = rpl_re_compile_pattern (string, strlen (string), pattern);
  if (message)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"%s (for regexp %s)\"), message, quote (string)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("%s (for regexp %s)"), message, quote (string)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("%s (for regexp %s)"), message, quote (string)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (gnu_extensions)
        {
          ...
          for (character = 0; character < 256; character++)
            word_fastmap[character] = !! ((*__ctype_b_loc ())[(int) ((character))] & (unsigned short int) _ISalpha);
        }
      else
        {
...
swallow_file_in_memory (const char *file_name, BLOCK *block)
{
  ...
  _Bool using_stdin = !file_name || !*file_name || (strcmp (file_name, "-") == 0);
  if (using_stdin)
...
# 779 "ptx.c"
  ...
  if (input_reference)
    {
      while (line_scan < text_buffer->end && ! ((*__ctype_b_loc ())[(int) ((to_uchar (*line_scan)))] & (unsigned short int) _ISspace)) line_scan++;
...
# 809 "ptx.c"
      ...
      if (context_regex.string)
        switch (rpl_re_search (&context_regex.pattern, cursor,
          ...
          case 0:
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"error: regular expression has a match of length zero: %s\"), quote (context_regex.string)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("error: regular expression has a match of length zero: %s"), quote (context_regex.string)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("error: regular expression has a match of length zero: %s"), quote (context_regex.string)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      while (context_end > context_start && ((*__ctype_b_loc ())[(int) ((to_uchar (context_end[-1])))] & (unsigned short int) _ISspace)) context_end--;
      ...
      while (1)
        {
          if (word_regex.string)
            ...
            {
              regoff_t r = rpl_re_search (&word_regex.pattern, cursor,
                                      context_end - cursor,
...
# 943 "ptx.c"
          ...
          if (input_reference && line_start == context_start)
            {
              while (context_start < context_end && ! ((*__ctype_b_loc ())[(int) ((to_uchar (*context_start)))] & (unsigned short int) _ISspace)) context_start++;
              while (context_start < context_end && ((*__ctype_b_loc ())[(int) ((to_uchar (*context_start)))] & (unsigned short int) _ISspace)) context_start++;
            }
      ...
      if (edited_flag[character])
        {
          ...
          diacritic = (diacrit_diac[(unsigned char) (character)]);
          if (diacritic != 0 && output_format == TEX_FORMAT)
            {
              base = (diacrit_base[(unsigned char) (character)]);
              switch (diacritic)
                {
      ...
      for (file_index = 0; file_index < number_input_files; file_index++)
        {
          ...
          reference_width = sprintf (ordinal_string, "%""l" "d", line_ordinal);
...
# 1284 "ptx.c"
  ...
  for (character = 0; character < 256; character++)
    edited_flag[character] = !! ((*__ctype_b_loc ())[(int) ((character))] & (unsigned short int) _ISspace);
  edited_flag['\f'] = 1;
  ...
  switch (output_format)
    ...
    case TEX_FORMAT:
      ...
      for (character = 0200; character < 256; character++)
        edited_flag[character] = (diacrit_diac[(unsigned char) (character)]) != 0;
      break;
    }
}
...
define_all_fields (OCCURS *occurs)
{
  ...
  while (cursor < right_context_end
    ...
    {
      keyafter.end = cursor;
      if (word_regex.string) { regoff_t count; count = rpl_re_match (&word_regex.pattern, cursor, right_context_end - cursor, 0, ((void*)0)); if (count == -2) matcher_error (); cursor += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*cursor)]) while (cursor < right_context_end && word_fastmap[to_uchar (*cursor)]) cursor++; else cursor++;
    }
  ...
  while (keyafter.end > keyafter.start && ((*__ctype_b_loc ())[(int) ((to_uchar (keyafter.end[-1])))] & (unsigned short int) _ISspace)) keyafter.end--;
# 1402 "ptx.c"
  if (-occurs->left > half_line_width + maximum_word_length)
    {
      left_field_start
        = keyafter.start - (half_line_width + maximum_word_length);
      if (word_regex.string) { regoff_t count; count = rpl_re_match (&word_regex.pattern, left_field_start, keyafter.start - left_field_start, 0, ((void*)0)); if (count == -2) matcher_error (); left_field_start += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*left_field_start)]) while (left_field_start < keyafter.start && word_fastmap[to_uchar (*left_field_start)]) left_field_start++; else left_field_start++;
    }
  else
  ...
  while (before.end > before.start && ((*__ctype_b_loc ())[(int) ((to_uchar (before.end[-1])))] & (unsigned short int) _ISspace)) before.end--;

  while (before.start + before_max_width < before.end)
    if (word_regex.string) { regoff_t count; count = rpl_re_match (&word_regex.pattern, before.start, before.end - before.start, 0, ((void*)0)); if (count == -2) matcher_error (); before.start += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*before.start)]) while (before.start < before.end && word_fastmap[to_uchar (*before.start)]) before.start++; else before.start++;

  if (truncation_string)
    {
      cursor = before.start;
      while (cursor > buffer_start && ((*__ctype_b_loc ())[(int) ((to_uchar (cursor[-1])))] & (unsigned short int) _ISspace)) cursor--;
      before_truncation = cursor > left_context_start;
    }
  else
    before_truncation = 0;

  while (before.start < buffer_end && ((*__ctype_b_loc ())[(int) ((to_uchar (*before.start)))] & (unsigned short int) _ISspace)) before.start++;
# 1442 "ptx.c"
  tail_max_width
  ...
  if (tail_max_width > 0)
    {
      ...
      while (tail.start < buffer_end && ((*__ctype_b_loc ())[(int) ((to_uchar (*tail.start)))] & (unsigned short int) _ISspace)) tail.start++;
      ...
      while (tail.end > tail.start && ((*__ctype_b_loc ())[(int) ((to_uchar (tail.end[-1])))] & (unsigned short int) _ISspace)) tail.end--;
    }
  else
    {
  ...
  if (head_max_width > 0)
    {
      ...
      while (head.end > buffer_start && ((*__ctype_b_loc ())[(int) ((to_uchar (head.end[-1])))] & (unsigned short int) _ISspace)) head.end--;

      head.start = left_field_start;
      while (head.start + head_max_width < head.end)
        if (word_regex.string) { regoff_t count; count = rpl_re_match (&word_regex.pattern, head.start, head.end - head.start, 0, ((void*)0)); if (count == -2) matcher_error (); head.start += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*head.start)]) while (head.start < head.end && word_fastmap[to_uchar (*head.start)]) head.start++; else head.start++;
      ...
      while (head.start < head.end && ((*__ctype_b_loc ())[(int) ((to_uchar (*head.start)))] & (unsigned short int) _ISspace)) head.start++;
    }
  else
    {
  ...
  if (auto_reference)
    {
      ...
      reference.end = file_end + sprintf (file_end, ":%""l" "d", line_ordinal);
    }
  else if (input_reference)
    {
      ...
      while (reference.end < right_context_end && ! ((*__ctype_b_loc ())[(int) ((to_uchar (*reference.end)))] & (unsigned short int) _ISspace)) reference.end++;
    }
}
...
output_one_tex_line (void)
{
  ...
  if (word_regex.string) { regoff_t count; count = rpl_re_match (&word_regex.pattern, cursor, keyafter.end - cursor, 0, ((void*)0)); if (count == -2) matcher_error (); cursor += count == -1 ? 1 : count; } else if (word_fastmap[to_uchar (*cursor)]) while (cursor < keyafter.end && word_fastmap[to_uchar (*cursor)]) cursor++; else cursor++;
  key.end = cursor;
  after.start = cursor;
  print_field (key);
  fputs_unlocked ("}{",stdout);
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [OPTION]... [INPUT]...   (without -G)\n  or:  %s -G [OPTION]... [INPUT [OUTPUT]]\n"),


              program_name, program_name);
      fputs_unlocked (gettext ("Output a permuted index, including context, of the words in the input files.\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -A, --auto-reference           output automatically generated references\n  -G, --traditional              behave more like System V 'ptx'\n"),stdout);



      fputs_unlocked (gettext ("  -F, --flag-truncation=STRING   use STRING for flagging line truncations.\n                                 The default is '/'\n"),stdout);



      fputs_unlocked (gettext ("  -M, --macro-name=STRING        macro name to use instead of 'xx'\n  -O, --format=roff              generate output as roff directives\n  -R, --right-side-refs          put references at right, not counted in -w\n  -S, --sentence-regexp=REGEXP   for end of lines or end of sentences\n  -T, --format=tex               generate output as TeX directives\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -W, --word-regexp=REGEXP       use REGEXP to match each keyword\n  -b, --break-file=FILE          word break characters in this FILE\n  -f, --ignore-case              fold lower case to upper case for sorting\n  -g, --gap-size=NUMBER          gap size in columns between output fields\n  -i, --ignore-file=FILE         read ignore word list from FILE\n  -o, --only-file=FILE           read only word list from this FILE\n"),stdout);
      ...
      fputs_unlocked (gettext ("  -r, --references               first field of each line is a reference\n  -t, --typeset-mode               - not implemented -\n  -w, --width=NUMBER             output width in columns, reference excluded\n"),stdout);
...
{
  {"auto-reference", 0, ((void*)0), 'A'},
  {"break-file", 1, ((void*)0), 'b'},
  {"flag-truncation", 1, ((void*)0), 'F'},
  {"ignore-case", 0, ((void*)0), 'f'},
  {"gap-size", 1, ((void*)0), 'g'},
  {"ignore-file", 1, ((void*)0), 'i'},
  {"macro-name", 1, ((void*)0), 'M'},
  {"only-file", 1, ((void*)0), 'o'},
  {"references", 0, ((void*)0), 'r'},
  {"right-side-refs", 0, ((void*)0), 'R'},
  {"format", 1, ((void*)0), 10},
  {"sentence-regexp", 1, ((void*)0), 'S'},
  {"traditional", 0, ((void*)0), 'G'},
  {"typeset-mode", 0, ((void*)0), 't'},
  {"width", 1, ((void*)0), 'w'},
  {"word-regexp", 1, ((void*)0), 'W'},
...
main (int argc, char **argv)
{
         ...
         optchar != (-1))
    {
      switch (optchar)
        ...
        case 'g':
          {
            intmax_t tmp;
            if (! (xstrtoimax (optarg, ((void*)0), 0, &tmp, "") == LONGINT_OK
                   && 0 < tmp && tmp <= (9223372036854775807L)))
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid gap width: %s\"), quote (optarg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid gap width: %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid gap width: %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))));
        ...
        case 'w':
          {
            ...
            if (! (xstrtoimax (optarg, ((void*)0), 0, &tmp, "") == LONGINT_OK
                   && 0 < tmp && tmp <= (9223372036854775807L)))
              ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid line width: %s\"), quote (optarg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid line width: %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid line width: %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))));
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "ptx", "GNU coreutils", Version, proper_name_utf8 ("F. Pinard", "Fran\xc3\xa7ois Pinard"), (char *) ((void*)0)); exit (0); break;;
        }
    }
  ...
  if (optind == argc)
  ...
  else
    {
      ...
      if (optind < argc)
        {
          if (! freopen_safer (argv[optind], "w", stdout))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", quotearg_n_style_colon (0, shell_escape_quoting_style, argv[optind])), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, argv[optind])), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, argv[optind])), ((0) ? (void) 0 : __builtin_unreachable ()))));
          optind++;
        }
