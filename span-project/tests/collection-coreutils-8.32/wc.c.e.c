...
static _Bool print_lines, print_words, print_chars, print_bytes;
...
# 284 "wc.c"
            ...
            {
              off_t hi_pos = end_pos - end_pos % (((0 < (fstatus->st).st_blksize && (fstatus->st).st_blksize <= ((size_t)-1) / 8 + 1) ? (fstatus->st).st_blksize : 512) + 1);
          ...
          do
            {
              ...
              switch (wide_char)
                ...
                default:
                  ...
                  else if (!wide && ((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISprint))
                    {
                      linepos++;
                      if (((*__ctype_b_loc ())[(int) ((to_uchar (*p)))] & (unsigned short int) _ISspace))
                        goto mb_word_separator;
                      in_word = 1;
                    }
                  break;
          ...
          do
            {
              switch (*p++)
                ...
                word_separator:
                  ...
                  break;
                default:
                  if (((*__ctype_b_loc ())[(int) ((to_uchar (p[-1])))] & (unsigned short int) _ISprint))
                    {
                      linepos++;
                      if (((*__ctype_b_loc ())[(int) ((to_uchar (p[-1])))] & (unsigned short int) _ISspace)
  ...
  if (! file || (strcmp (file, "-") == 0))
    {
      ...
      return wc (0, file, fstatus, -1);
    }
  else
    {
      int fd = open (file, 00 | 0);
      if (fd == -1)
        {
          error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, file));
          return 0;
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "clLmw", longopts, ((void*)0))) != -1)
    switch (optc)
      ...
      case GETOPT_VERSION_CHAR: version_etc (stdout, "wc", "GNU coreutils", Version, ("Paul Rubin"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
  ...
  if (files_from)
    {
      ...
      if (optind < argc)
        {
          error (0, 0, gettext ("extra operand %s"), quotearg_style (shell_escape_always_quoting_style, argv[optind]));
          fprintf (stderr, "%s\n",
      ...
      if (fstat (fileno (stream), &st) == 0
          ...
          && st.st_size <= (((10 * 1024 * 1024)<(physmem_available () / 2))?(10 * 1024 * 1024):(physmem_available () / 2)))
        {
          read_tokens = 1;
          readtokens0_init (&tok);
          if (! readtokens0 (stream, &tok) || rpl_fclose (stream) != 0)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"cannot read file names from %s\"), quotearg_style (shell_escape_always_quoting_style, files_from)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("cannot read file names from %s"), quotearg_style (shell_escape_always_quoting_style, files_from)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("cannot read file names from %s"), quotearg_style (shell_escape_always_quoting_style, files_from)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (!file_name)
        {
          switch (ai_err)
            ...
            case AI_ERR_MEM:
              xalloc_die ();
            default:
              ((void) sizeof ((!"unexpected error code from argv_iter") ? 1 : 0), __extension__ ({ if (!"unexpected error code from argv_iter") ; else __assert_fail ("!\"unexpected error code from argv_iter\"", "wc.c", 831, __extension__ __PRETTY_FUNCTION__); }));
            }
        }
