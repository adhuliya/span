...
# 31 "tee.c" 2
# 40 "tee.c"
static _Bool tee_files (int nfiles, char **files);
...
_Static_assert ((sizeof (output_error_args) / sizeof *(output_error_args)) == (sizeof (output_error_types) / sizeof *(output_error_types)) + 1, "verify (" "ARRAY_CARDINALITY (output_error_args) == ARRAY_CARDINALITY (output_error_types) + 1" ")");
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -p                        diagnose errors writing to non pipes\n      --output-error[=MODE]   set behavior on write error.  See MODE below\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "aip", long_options, ((void*)0))) != -1)
    {
      switch (optc)
        ...
        case 'p':
          if (optarg)
            output_error = ((output_error_types) [__xargmatch_internal ("--output-error", optarg, output_error_args, (void const *) (output_error_types), sizeof *(output_error_types), argmatch_die)]);
  ...
  if (close (0) != 0)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), \"%s\", gettext (\"standard input\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), "%s", gettext ("standard input")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), "%s", gettext ("standard input")), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
tee_files (int nfiles, char **files)
{
  ...
  files[0] = bad_cast (gettext ("standard output"));
      ...
      if (descriptors[i] == ((void*)0))
        {
          error (output_error == output_error_exit
                 ...
                 (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, files[i]));
          ok = 0;
        }
      else
        {
            ...
            if (fail)
              {
                error (output_error == output_error_exit
                       || output_error == output_error_exit_nopipe,
                       w_errno, "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, files[i]));
              }
  ...
  for (i = 1; i <= nfiles; i++)
    if (descriptors[i] && rpl_fclose (descriptors[i]) != 0)
      {
        error (0, (*__errno_location ()), "%s", quotearg_n_style_colon (0, shell_escape_quoting_style, files[i]));
        ok = 0;
      }
