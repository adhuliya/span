...
main (int argc, char **argv)
{
      ...
      if (nskip)
        {
          ...
          ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"unknown program %s\"), quote (prog_name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("unknown program %s"), quote (prog_name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("unknown program %s"), quote (prog_name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (optind == 1 && prog_name && !(strcmp (prog_name, "coreutils") == 0))
    error (0, 0, gettext ("unknown program %s"),
           quote (prog_name));
