...
# 24 "../lib/root-dev-ino.h"
struct dev_ino *
get_root_dev_ino (struct dev_ino *root_d_i);
# 30 "chcon.c" 2
# 1 "../lib/selinux-at.h" 1
# 17 "../lib/selinux-at.h"
# 1 "../lib/selinux/selinux.h" 1
# 20 "../lib/selinux/selinux.h" 3
# 18 "../lib/selinux-at.h" 2
...
compute_context_from_mask (char const *context, context_t *ret)
{
  ...
  if (!new_context)
    {
      error (0, (*__errno_location ()), gettext ("failed to create security context: %s"),
             quote (context));
      return 1;
    }
# 120 "chcon.c"
  do { if (specified_user && context_user_set ((new_context), specified_user)) { error (0, (*__errno_location ()), gettext ("failed to set %s security context component to %s"), "user", quote (specified_user)); ok = 0; } } while (0);
  do { if (specified_range && context_range_set ((new_context), specified_range)) { error (0, (*__errno_location ()), gettext ("failed to set %s security context component to %s"), "range", quote (specified_range)); ok = 0; } } while (0);
  do { if (specified_role && context_role_set ((new_context), specified_role)) { error (0, (*__errno_location ()), gettext ("failed to set %s security context component to %s"), "role", quote (specified_role)); ok = 0; } } while (0);
  do { if (specified_type && context_type_set ((new_context), specified_type)) { error (0, (*__errno_location ()), gettext ("failed to set %s security context component to %s"), "type", quote (specified_type)); ok = 0; } } while (0);
...
change_file_context (int fd, char const *file)
{
  ...
  if (specified_context == ((void*)0))
    {
      ...
      if (status < 0 && (*__errno_location ()) != 61)
        {
  ...
  if (file_context == ((void*)0) || ! (strcmp (context_string, file_context) == 0))
    {
      ...
      if (fail)
        {
          errors = 1;
          error (0, (*__errno_location ()), gettext ("failed to change context of %s to %s"),
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "HLPRhvu:r:t:l:", long_options, ((void*)0)))
         != -1)
    {
      switch (optc)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "chcon", "GNU coreutils", Version, ("Russell Coker"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (recurse)
    {
      if (bit_flags == 0x0010)
        {
          if (dereference == 1)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"-R --dereference requires either -H or -L\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("-R --dereference requires either -H or -L")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("-R --dereference requires either -H or -L")), ((0) ? (void) 0 : __builtin_unreachable ()))));
        ...
        {
          if (dereference == 0)
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"-R -h requires -P\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("-R -h requires -P")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("-R -h requires -P")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (reference_file)
  ...
  else
    {
      ...
      if (0 < 0
        ...
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, (*__errno_location ()), gettext (\"invalid context: %s\"), quote (specified_context)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, (*__errno_location ()), gettext ("invalid context: %s"), quote (specified_context)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, (*__errno_location ()), gettext ("invalid context: %s"), quote (specified_context)), ((0) ? (void) 0 : __builtin_unreachable ()))));
