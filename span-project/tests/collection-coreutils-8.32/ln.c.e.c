...
static _Bool dereference_dest_dir_symlinks = 1;
...
{
  {"backup", 2, ((void*)0), 'b'},
  {"directory", 0, ((void*)0), 'F'},
  {"no-dereference", 0, ((void*)0), 'n'},
...
atomic_link (char const *source, int destdir_fd, char const *dest_base)
{
  return (symbolic_link
          ...
          : errnoize (linkat (-100, source, destdir_fd, dest_base,
                              logical ? 0x400 : 0)));
}
# 185 "ln.c"
static _Bool
         ...
         char const *dest, int link_errno)
{
  ...
  int source_status = 1;
  char *backup_base = ((void*)0);
  char *rel_source = ((void*)0);
  int nofollow_flag = logical ? 0 : 0x100;
  if (link_errno < 0)
  ...
  if ((link_errno || dest_set) && !symbolic_link)
    {
      source_status = fstatat (-100, source, &source_stats, nofollow_flag);
      if (source_status != 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to access %s"), quotearg_style (shell_escape_always_quoting_style, source));
          return 0;
        }
    }
  ...
  if (link_errno)
    {
      if (!symbolic_link && !hard_dir_link && ((((source_stats.st_mode)) & 0170000) == (0040000)))
        {
          error (0, 0, gettext ("%s: hard link not allowed for directory"),
                 quotearg_n_style_colon (0, shell_escape_quoting_style, source));
          return 0;
        }
      ...
      _Bool force = (remove_existing_files || interactive
                    || backup_type != no_backups);
      if (force)
        {
          struct stat dest_stats;
          if (fstatat (destdir_fd, dest_base, &dest_stats, 0x100)
              != 0)
            {
              if ((*__errno_location ()) != 2)
                {
                  error (0, (*__errno_location ()), gettext ("failed to access %s"), quotearg_style (shell_escape_always_quoting_style, dest));
                  return 0;
                }
              force = 0;
            }
          else if (((((dest_stats.st_mode)) & 0170000) == (0040000)))
            {
              error (0, 0, gettext ("%s: cannot overwrite directory"), quotearg_n_style_colon (0, shell_escape_quoting_style, dest));
              return 0;
            }
          else if (seen_file (dest_set, dest, &dest_stats))
            {
              ...
              error (0, 0,
                     ...
                     quotearg_n_style (0, shell_escape_always_quoting_style, dest), quotearg_n_style (1, shell_escape_always_quoting_style, source));
              return 0;
            }
          else
            {
              ...
              if (backup_type != no_backups
                  ...
                  : remove_existing_files)
                {
                  ...
                  if (source_status == 0
                      && ((source_stats).st_ino == (dest_stats).st_ino && (source_stats).st_dev == (dest_stats).st_dev)
                      && (source_stats.st_nlink == 1
                          || same_nameat (-100, source,
                                          destdir_fd, dest_base)))
                    {
                      error (0, 0, gettext ("%s and %s are the same file"),
                             quotearg_n_style (0, shell_escape_always_quoting_style, source), quotearg_n_style (1, shell_escape_always_quoting_style, dest));
                      return 0;
                    }
                }

              if (link_errno < 0 || link_errno == 17)
                {
                  if (interactive)
                    {
                      fprintf (stderr, gettext ("%s: replace %s? "),
                               program_name, quotearg_style (shell_escape_always_quoting_style, dest));
                      if (!yesno ())
                        return 1;
                    }
                  ...
                  if (backup_type != no_backups)
                    {
                      ...
                      if (renameat (destdir_fd, dest_base,
                          ...
                          != 0)
                        {
                          ...
                          if (rename_errno != 2)
                           {
                              error (0, rename_errno, gettext ("cannot backup %s"),
      ...
      link_errno
        = (symbolic_link
           ? force_symlinkat (source, destdir_fd, dest_base,
                              force, link_errno)
           : force_linkat (-100, source, destdir_fd, dest_base,
                           logical ? 0x400 : 0,
                           force, link_errno));
          ...
          if (backup_base)
            {
              ...
              quoted_backup = quotearg_n_style (2, shell_escape_always_quoting_style, backup);
              backup_sep = " ~ ";
              free (alloc);
            }
          printf ("%s%s%s %c> %s\n", quoted_backup, backup_sep,
                  quotearg_n_style (0, shell_escape_always_quoting_style, dest), symbolic_link ? '-' : '=',
                  quotearg_n_style (1, shell_escape_always_quoting_style, source));
        }
    }
  else
    {
      error (0, link_errno,
             (symbolic_link
              ? (link_errno != 36 && *source
                 ? gettext ("failed to create symbolic link %s")
                 : gettext ("failed to create symbolic link %s -> %s"))
              : (link_errno == 31
                 ? gettext ("failed to create hard link to %.0s%s")
                 : (link_errno == 122 || link_errno == 17
                    || link_errno == 28 || link_errno == 30)
                 ? gettext ("failed to create hard link %s")
                 : gettext ("failed to create hard link %s => %s"))),
             quotearg_n_style (0, shell_escape_always_quoting_style, dest), quotearg_n_style (1, shell_escape_always_quoting_style, source));

      if (backup_base)
        {
          if (renameat (destdir_fd, backup_base, destdir_fd, dest_base) != 0)
            error (0, (*__errno_location ()), gettext ("cannot un-backup %s"), quotearg_style (shell_escape_always_quoting_style, dest));
        }
    }
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "bdfinrst:vFLPS:T", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "ln", "GNU coreutils", Version, ("Mike Parker"), ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
          break;
        }
  ...
  if (relative && !symbolic_link)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"cannot do --relative without --symbolic\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("cannot do --relative without --symbolic")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("cannot do --relative without --symbolic")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  if (no_target_directory)
    {
      if (target_directory)
        ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"cannot combine --target-directory \" \"and --no-target-directory\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("cannot combine --target-directory " "and --no-target-directory")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("cannot combine --target-directory " "and --no-target-directory")), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  else
    {
      if (n_files == 2 && !target_directory)
        link_errno = atomic_link (file[0], -100, file[1]);
      if (link_errno < 0 || link_errno == 17 || link_errno == 20
          || link_errno == 22)
        {
          char const *d
            = target_directory ? target_directory : file[n_files - 1];
          int flags = (O_PATHSEARCH | 0200000
                       | (dereference_dest_dir_symlinks ? 0 : 0400000));
          destdir_fd = openat_safer (-100, d, flags);
          int err = (*__errno_location ());
          if (!0200000 && 0 <= destdir_fd)
            {
              struct stat st;
              err = (fstat (destdir_fd, &st) != 0 ? (*__errno_location ())
                     : ((((st.st_mode)) & 0170000) == (0040000)) ? 0 : 20);
              if (err != 0)
                {
                  close (destdir_fd);
                  destdir_fd = -1;
  ...
  if (target_directory)
    ...
    ok = do_link (file[0], -100, file[1], file[1], link_errno);
