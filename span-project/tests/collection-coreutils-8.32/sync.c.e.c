...
# 105 "sync.c"
  ...
  if (fdflags == -1
    ...
    {
      error (0, (*__errno_location ()), gettext ("couldn't reset non-blocking mode %s"),
  ...
  if (ret == 1)
    {
      ...
      switch (mode)
        ...
        default:
          ((void) sizeof (("invalid sync_mode") ? 1 : 0), __extension__ ({ if ("invalid sync_mode") ; else __assert_fail ("\"invalid sync_mode\"", "sync.c", 152, __extension__ __PRETTY_FUNCTION__); }));
        }

      if (sync_status < 0)
        {
          error (0, (*__errno_location ()), gettext ("error syncing %s"), quotearg_style (shell_escape_always_quoting_style, file));
          ret = 0;
        }
    }
...
main (int argc, char **argv)
{
  ...
  if (arg_data && arg_file_system)
    {
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"cannot specify both --data and --file-system\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("cannot specify both --data and --file-system")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("cannot specify both --data and --file-system")), ((0) ? (void) 0 : __builtin_unreachable ()))));

    }

  if (!args_specified && arg_data)
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"--data needs at least one argument\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("--data needs at least one argument")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("--data needs at least one argument")), ((0) ? (void) 0 : __builtin_unreachable ()))));
