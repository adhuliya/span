  ...
  while (1)
    {
      ...
      enum { headersize = __builtin_offsetof(struct fiemap, fm_extents) };
      enum { count = (sizeof fiemap_buf - headersize) / sizeof *fm_extents };
      _Static_assert (count > 1, "verify (" "count > 1" ")");
      ...
      ((void) sizeof ((scan->ei_count <= (18446744073709551615UL) - fiemap->fm_mapped_extents) ? 1 : 0), __extension__ ({ if (scan->ei_count <= (18446744073709551615UL) - fiemap->fm_mapped_extents) ; else __assert_fail ("scan->ei_count <= SIZE_MAX - fiemap->fm_mapped_extents", "extent-scan.c", 127, __extension__ __PRETTY_FUNCTION__); }));
      ...
      for (i = 0; i < fiemap->fm_mapped_extents; i++)
        {
          ((void) sizeof ((fm_extents[i].fe_logical <= ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << ((sizeof (off_t) * 8) - 2)) - 1) * 2 + 1))) - fm_extents[i].fe_length) ? 1 : 0), __extension__ ({ if (fm_extents[i].fe_logical <= ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << ((sizeof (off_t) * 8) - 2)) - 1) * 2 + 1))) - fm_extents[i].fe_length) ; else __assert_fail ("fm_extents[i].fe_logical <= OFF_T_MAX - fm_extents[i].fe_length", "extent-scan.c", 142, __extension__ __PRETTY_FUNCTION__); }));


          _Static_assert (sizeof last_ei->ext_flags >= sizeof fm_extents->fe_flags, "verify (" "sizeof last_ei->ext_flags >= sizeof fm_extents->fe_flags" ")");
