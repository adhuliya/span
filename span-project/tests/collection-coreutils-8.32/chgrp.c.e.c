...
char *
gid_to_name (gid_t) __attribute__ ((__malloc__));

char *
uid_to_name (uid_t) __attribute__ ((__malloc__));
  ...
  if (*name)
    {
      ...
      if (grp)
        ...
        {
          uintmax_t tmp;
          if (! (xstrtoumax (name, ((void*)0), 10, &tmp, "") == LONGINT_OK
                 && tmp <= ((gid_t) (! (! ((gid_t) 0 < (gid_t) -1)) ? (gid_t) -1 : ((((gid_t) 1 << ((sizeof (gid_t) * 8) - 2)) - 1) * 2 + 1)))))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid group: %s\"), quote (name)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid group: %s"), quote (name)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid group: %s"), quote (name)), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
usage (int status)
{
  if (status != 0)
    ...
    {
      printf (gettext ("Usage: %s [OPTION]... GROUP FILE...\n  or:  %s [OPTION]... --reference=RFILE FILE...\n"),



              program_name, program_name);
      fputs_unlocked (gettext ("Change the group of each FILE to GROUP.\nWith --reference, change the group of each FILE to that of RFILE.\n\n"),stdout);
      ...
      fputs_unlocked (gettext ("      --reference=RFILE  use RFILE's group rather than specifying a\n                         GROUP value\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  while ((optc = getopt_long (argc, argv, "HLPRcfhv", long_options, ((void*)0)))
         != -1)
    {
      switch (optc)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "chgrp", "GNU coreutils", Version, ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
  ...
  if (reference_file)
  ...
  else
    {
      ...
      chopt.group_name = (*group_name ? xstrdup (group_name) : ((void*)0));
