...
# 31 "touch.c" 2
# 1 "../lib/parse-datetime.h" 1
# 20 "../lib/parse-datetime.h"
# 1 "../lib/time.h" 1
# 21 "../lib/time.h" 3
# 21 "../lib/parse-datetime.h" 2

_Bool parse_datetime (struct timespec *__restrict,
...
_Bool parse_datetime2 (struct timespec *__restrict,
                      char const *, struct timespec const *,
...
static _Bool use_ref;


static _Bool no_dereference;
...
{
  "atime", "access", "use", "mtime", "modify", ((void*)0)
};
             ...
             char const *flex_date, struct timespec const *now)
{
  if (! parse_datetime (result, flex_date, now))
    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid date format %s\"), quote (flex_date)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid date format %s"), quote (flex_date)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid date format %s"), quote (flex_date)), ((0) ? (void) 0 : __builtin_unreachable ()))));
}
...
touch (const char *file)
{
  ...
  if ((strcmp (file, "-") == 0))
  ...
  else if (! (no_create || no_dereference))
    {
      ...
      fd = fd_reopen (0, file,
                      01 | 0100 | 04000 | 0400, (0400 | 0200 | (0400 >> 3) | (0200 >> 3) | ((0400 >> 3) >> 3) | ((0200 >> 3) >> 3)));





      if (fd == -1 && (*__errno_location ()) != 21 && (*__errno_location ()) != 22 && (*__errno_location ()) != 1)
  ...
  if (change_times != (1 | 2))
    {
      ...
      if (change_times == 2)
        newtime[0].tv_nsec = ((1l << 30) - 2l);
      else
        {
          ((void) sizeof ((change_times == 1) ? 1 : 0), __extension__ ({ if (change_times == 1) ; else __assert_fail ("change_times == CH_ATIME", "touch.c", 153, __extension__ __PRETTY_FUNCTION__); }));
          newtime[1].tv_nsec = ((1l << 30) - 2l);
        }
    }
  ...
  ok = (fdutimensat (fd, -100, (fd == 1 ? ((void*)0) : file), t,
                     (no_dereference && fd == -1) ? 0x100 : 0)
        == 0);
  ...
  if (fd == 0)
    {
      if (close (0) != 0)
        {
          error (0, (*__errno_location ()), gettext ("failed to close %s"), quotearg_style (shell_escape_always_quoting_style, file));
          return 0;
        }
    }
  else if (fd == 1)
    {
      ...
      if (!ok && (*__errno_location ()) == 9 && no_create)
        return 1;
    }
      ...
      if (open_errno)
        {
          ...
          error (0, open_errno, gettext ("cannot touch %s"), quotearg_style (shell_escape_always_quoting_style, file));
        }
      else
        {
          if (no_create && (*__errno_location ()) == 2)
            return 1;
          error (0, (*__errno_location ()), gettext ("setting times of %s"), quotearg_style (shell_escape_always_quoting_style, file));
        }
      return 0;
    }
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("Update the access and modification times of each FILE to the current time.\n\nA FILE argument that does not exist is created empty, unless -c or -h\nis supplied.\n\nA FILE argument string of - is handled specially and causes touch to\nchange the times of the file associated with standard output.\n"),stdout);
# 224 "touch.c"
      emit_mandatory_arg_note ();

      fputs_unlocked (gettext ("  -a                     change only the access time\n  -c, --no-create        do not create any files\n  -d, --date=STRING      parse STRING and use it instead of current time\n  -f                     (ignored)\n"),stdout);





      fputs_unlocked (gettext ("  -h, --no-dereference   affect each symbolic link instead of any referenced\n                         file (useful only on systems that can change the\n                         timestamps of a symlink)\n  -m                     change only the modification time\n"),stdout);





      fputs_unlocked (gettext ("  -r, --reference=FILE   use this file's times instead of current time\n  -t STAMP               use [[CC]YY]MMDDhhmm[.ss] instead of current time\n      --time=WORD        change the specified time:\n                           WORD is access, atime, or use: equivalent to -a\n                           WORD is modify or mtime: equivalent to -m\n"),stdout);
      ...
      switch (c)
        ...
        case 't':
          if (! posixtime (&newtime[0].tv_sec, optarg,
                           0 | 2 | 4))
            ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"invalid date format %s\"), quote (optarg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("invalid date format %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("invalid date format %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))));
        ...
        case TIME_OPTION:
          change_times |= ((time_masks) [__xargmatch_internal ("--time", optarg, time_args, (void const *) (time_masks), sizeof *(time_masks), argmatch_die)]);

          break;

        case GETOPT_HELP_CHAR: usage (0); break;;

        case GETOPT_VERSION_CHAR: version_etc (stdout, "touch", "GNU coreutils", Version, ("Paul Rubin"), ("Arnold Robbins"), ("Jim Kingdon"), ("David MacKenzie"), ("Randy Smith"), (char *) ((void*)0)); exit (0); break;;
      ...
      if (! getenv ("POSIXLY_CORRECT"))
        {
          ...
          if (tm)
            error (0, 0,
                   gettext ("warning: 'touch %s' is obsolete; use " "'touch -t %04ld%02d%02d%02d%02d.%02d'"),
  ...
  if (!date_set)
    {
      if (change_times == (1 | 2))
        ...
        newtime[1].tv_nsec = newtime[0].tv_nsec = ((1l << 30) - 1l);
    }
