...
announce_mkdir (char const *dir, void *options)
{
  ...
  if (o->created_directory_format)
    prog_fprintf (stdout, o->created_directory_format, quotearg_style (shell_escape_always_quoting_style, dir));
}
...
make_ancestor (char const *dir, char const *component, void *options)
{
  ...
  if (o->set_security_context && defaultcon (component, 0040000) < 0
  ...
  mode_t user_wx = 0200 | 0100;
  _Bool self_denying_umask = (o->umask_value & user_wx) != 0;
  if (self_denying_umask)
    umask (o->umask_value & ~user_wx);
  int r = mkdir (component, ((0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3)));
  if (self_denying_umask)
    {
...
process_dir (char *dir, struct savewd *wd, void *options)
{
  ...
  if (o->set_security_context)
    {
      if (! o->make_ancestor_function && defaultcon (dir, 0040000) < 0
          && ! ignorable_ctx_err ((*__errno_location ())))
        error (0, (*__errno_location ()), gettext ("failed to set default creation context for %s"),
               quotearg_style (shell_escape_always_quoting_style, dir));
    }
  ...
  if (ret == 0 && o->set_security_context
...
main (int argc, char **argv)
{
  ...
  options.mode = ((0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3));
  options.mode_bits = 0;
  options.created_directory_format = ((void*)0);
  options.set_security_context = 0;
  ...
  while ((optc = getopt_long (argc, argv, "pm:vZ", longopts, ((void*)0))) != -1)
    {
      switch (optc)
          ...
          if (is_smack_enabled ())
          ...
          else if (0 > 0)
            {
              if (optarg)
                ...
                options.set_security_context = 1;
            }
          else if (optarg)
            {
              error (0, 0,
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "mkdir", "GNU coreutils", Version, ("David MacKenzie"), (char *) ((void*)0)); exit (0); break;;
        default:
          usage (1);
        }
    }
      ...
      if (specified_mode)
        {
          ...
          options.mode = mode_adjust (((0400|0200|0100) | ((0400|0200|0100) >> 3) | (((0400|0200|0100) >> 3) >> 3)), 1, umask_value, change,
                                      &options.mode_bits);
          free (change);
        }
      else
