...
main (int argc, char **argv)
{
  ...
  parse_gnu_standard_options_only (argc, argv, "sleep", "GNU coreutils",
                                   Version, 1, usage, ("Jim Meyering"), ("Paul Eggert"),
                                   (char const *) ((void*)0));
  ...
  for (int i = optind; i < argc; i++)
    {
      double s;
      const char *p;
      if (! (xstrtod (argv[i], &p, &s, cl_strtod) || (*__errno_location ()) == 34)
