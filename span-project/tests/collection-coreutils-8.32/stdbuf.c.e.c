...
# 216 "stdbuf.c"
  ...
  while (1)
    {
      ...
      ++path;
      if ( ! *path)
        ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, 0, gettext (\"failed to find %s\"), quote (\"libstdbuf.so\")), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, 0, gettext ("failed to find %s"), quote ("libstdbuf.so")), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, 0, gettext ("failed to find %s"), quote ("libstdbuf.so")), ((0) ? (void) 0 : __builtin_unreachable ()))));
    }
  ...
  if (ret != 0)
    {
      ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, (*__errno_location ()), gettext (\"failed to update the environment with %s\"), quote (LD_PRELOAD)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("failed to update the environment with %s"), quote (LD_PRELOAD)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("failed to update the environment with %s"), quote (LD_PRELOAD)), ((0) ? (void) 0 : __builtin_unreachable ()))));
      ...
      if (stdbuf[i].optarg)
        {
          ...
          if (putenv (var) != 0)
            {
              ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, (*__errno_location ()), gettext (\"failed to update the environment with %s\"), quote (var)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("failed to update the environment with %s"), quote (var)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("failed to update the environment with %s"), quote (var)), ((0) ? (void) 0 : __builtin_unreachable ()))));
  ...
  while ((c = getopt_long (argc, argv, "+i:o:e:", longopts, ((void*)0))) != -1)
    {
      ...
      switch (c)
        ...
        case 'o':
          ...
          ((void) sizeof ((0 <= opt_fileno && opt_fileno < (sizeof (stdbuf) / sizeof *(stdbuf))) ? 1 : 0), __extension__ ({ if (0 <= opt_fileno && opt_fileno < (sizeof (stdbuf) / sizeof *(stdbuf))) ; else __assert_fail ("0 <= opt_fileno && opt_fileno < ARRAY_CARDINALITY (stdbuf)", "stdbuf.c", 336, __extension__ __PRETTY_FUNCTION__); }));
          ...
          if (!(strcmp (optarg, "L") == 0)
            ...
            ((!!sizeof (struct { _Static_assert (EXIT_CANCELED, "verify_expr (" "EXIT_CANCELED" ", " "(error (EXIT_CANCELED, (*__errno_location ()), gettext (\"invalid mode %s\"), quote (optarg)), assume (false))" ")"); int _gl_dummy; })) ? ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("invalid mode %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (EXIT_CANCELED, (*__errno_location ()), gettext ("invalid mode %s"), quote (optarg)), ((0) ? (void) 0 : __builtin_unreachable ()))));

          break;

        case GETOPT_HELP_CHAR: usage (0); break;;

        case GETOPT_VERSION_CHAR: version_etc (stdout, "stdbuf", "GNU coreutils", Version, ("Padraig Brady"), (char *) ((void*)0)); exit (0); break;;
