...
enum
{
  PRESUME_INPUT_PIPE_OPTION = 127 + 1
};
  ...
  if (status != 0)
  ...
  else
    {
      printf (gettext ("Usage: %s [OPTION]... [FILE]...\n"),
              ...
              program_name);
      printf (gettext ("Print the first %d lines of each FILE to standard output.\nWith more than one FILE, precede each with a header giving the file name.\n"), 10);
      ...
      printf (gettext ("  -c, --bytes=[-]NUM       print the first NUM bytes of each file;\n                             with the leading '-', print all but the last\n                             NUM bytes of each file\n  -n, --lines=[-]NUM       print the first NUM lines instead of the first %d;\n                             with the leading '-', print all but the last\n                             NUM lines of each file\n"), 10);
      ...
      fputs_unlocked (gettext ("  -q, --quiet, --silent    never print headers giving file names\n  -v, --verbose            always print headers giving file names\n"),stdout);
...
diagnose_copy_fd_failure (enum Copy_fd_status err, char const *filename)
{
  switch (err)
    ...
    case COPY_FD_READ_ERROR:
      error (0, (*__errno_location ()), gettext ("error reading %s"), quotearg_style (shell_escape_always_quoting_style, filename));
      break;
    case COPY_FD_UNEXPECTED_EOF:
      error (0, (*__errno_location ()), gettext ("%s: file has shrunk too much"), quotearg_n_style_colon (0, shell_escape_quoting_style, filename));
      break;
    default:
      abort ();
    }
...
elseek (int fd, off_t offset, int whence, char const *filename)
{
  ...
  if (new_offset < 0)
    error (0, (*__errno_location ()),
           gettext (whence == 0 ? "%s: cannot seek to offset %s" : "%s: cannot seek to relative offset %s"),
  ...
  if ((18446744073709551615UL) < n_elide_0 + 8192)
    {
      ...
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"%s: number of bytes is too large\"), umaxtostr (n_elide_0, umax_buf)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("%s: number of bytes is too large"), umaxtostr (n_elide_0, umax_buf)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("%s: number of bytes is too large"), umaxtostr (n_elide_0, umax_buf)), ((0) ? (void) 0 : __builtin_unreachable ()))));
          ...
          if (n_lines-- == 0)
            {
              ...
              return 0 <= elseek (fd, pos + n + 1, 0, pretty_filename);
            }
        }
        ...
        if (buffer[bytes_to_write++] == line_end && --lines_to_write == 0)
          {
            ...
            if (lseek (fd, -n_bytes_past_EOL, 1) < 0)
              {
                struct stat st;
                if (fstat (fd, &st) != 0 || ((((st.st_mode)) & 0170000) == (0100000)))
...
static _Bool
head (const char *filename, int fd, uintmax_t n_units, _Bool count_lines,
  ...
  if (elide_from_end)
    {
      ...
      if (! presume_input_pipe && usable_st_size (&st))
        {
          current_pos = elseek (fd, 0, 1, filename);
          if (current_pos < 0)
            return 0;
        }
      if (count_lines)
...
static _Bool
head_file (const char *filename, uintmax_t n_units, _Bool count_lines,
           _Bool elide_from_end)
{
  int fd;
  _Bool ok;
  ...
  if (is_stdin)
  ...
  else
    {
      ...
      if (fd < 0)
        {
          error (0, (*__errno_location ()), gettext ("cannot open %s for reading"), quotearg_style (shell_escape_always_quoting_style, filename));
          return 0;
        }
    }
  ...
  if (!is_stdin && close (fd) != 0)
    {
      error (0, (*__errno_location ()), gettext ("failed to close %s"), quotearg_style (shell_escape_always_quoting_style, filename));
      return 0;
    }
  return ok;
}
...
static uintmax_t
string_to_integer (_Bool count_lines, const char *n_string)
{
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "c:n:qvz0123456789", long_options, ((void*)0)))
         != -1)
    {
      switch (c)
        {
        ...
        case GETOPT_VERSION_CHAR: version_etc (stdout, "head", "GNU coreutils", Version, ("David MacKenzie"), ("Jim Meyering"), (char *) ((void*)0)); exit (0); break;;
  ...
  if ( ! count_lines && elide_from_end && ((off_t) (! (! ((off_t) 0 < (off_t) -1)) ? (off_t) -1 : ((((off_t) 1 << ((sizeof (off_t) * 8) - 2)) - 1) * 2 + 1))) < n_units)
    {
      ...
      ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 75, \"%s: %s\", gettext (\"invalid number of bytes\"), quote (umaxtostr (n_units, umax_buf))), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 75, "%s: %s", gettext ("invalid number of bytes"), quote (umaxtostr (n_units, umax_buf))), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 75, "%s: %s", gettext ("invalid number of bytes"), quote (umaxtostr (n_units, umax_buf))), ((0) ? (void) 0 : __builtin_unreachable ()))));
