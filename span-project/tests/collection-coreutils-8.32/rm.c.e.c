...
static struct option const long_opts[] =
{
  ...
  {"preserve-root", 2, ((void*)0), PRESERVE_ROOT},
...
_Static_assert ((sizeof (interactive_args) / sizeof *(interactive_args)) == (sizeof (interactive_types) / sizeof *(interactive_types)) + 1, "verify (" "ARRAY_CARDINALITY (interactive_args) == ARRAY_CARDINALITY (interactive_types) + 1" ")");
...
usage (int status)
{
  if (status != 0)
  ...
  else
    {
      ...
      fputs_unlocked (gettext ("  -r, -R, --recursive   remove directories and their contents recursively\n  -d, --dir             remove empty directories\n  -v, --verbose         explain what is being done\n"),stdout);
...
main (int argc, char **argv)
{
  ...
  ((void) sizeof ((((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ? 1 : 0), __extension__ ({ if (((status) == RM_OK || (status) == RM_USER_DECLINED || (status) == RM_ERROR)) ; else __assert_fail ("VALID_STATUS (status)", "rm.c", 371, __extension__ __PRETTY_FUNCTION__); }));
