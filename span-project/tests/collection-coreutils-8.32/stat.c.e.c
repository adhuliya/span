    ...
    case 'i':
      {
        ...
        _Static_assert (_Alignof (struct statfs) % _Alignof (fsid_word) == 0, "verify (" "alignof (STRUCT_STATVFS) % alignof (fsid_word) == 0" ")");
        _Static_assert (__builtin_offsetof(struct statfs, f_fsid) % _Alignof (fsid_word) == 0, "verify (" "offsetof (STRUCT_STATVFS, f_fsid) % alignof (fsid_word) == 0" ")");
        _Static_assert (sizeof statfsbuf->f_fsid % _Alignof (fsid_word) == 0, "verify (" "sizeof statfsbuf->f_fsid % alignof (fsid_word) == 0" ")");
  ...
  if (follow_links || !((((statp->st_mode)) & 0170000) == (0120000)))
    {
      ...
      if (!resolved)
        {
          error (0, (*__errno_location ()), gettext ("failed to canonicalize %s"), quotearg_style (shell_escape_always_quoting_style, filename));
  ...
  if (q_style)
    {
      ...
      if (0 <= i)
        ...
        {
          set_quoting_style (((void*)0), shell_escape_always_quoting_style);
          error (0, 0, gettext ("ignoring invalid value of environment " "variable QUOTING_STYLE: %s"), quote (q_style));
                ...
                if (1 < len)
                  {
                    dest[len] = *fmt_char;
                    dest[len + 1] = '\0';
                    ((!!sizeof (struct { _Static_assert (1, "verify_expr (" "1" ", " "(error (1, 0, gettext (\"%s: invalid directive\"), quote (dest)), assume (false))" ")"); int _gl_dummy; })) ? ((error (1, 0, gettext ("%s: invalid directive"), quote (dest)), ((0) ? (void) 0 : __builtin_unreachable ()))) : ((error (1, 0, gettext ("%s: invalid directive"), quote (dest)), ((0) ? (void) 0 : __builtin_unreachable ()))));
...
do_statfs (char const *filename, char const *format)
{
  ...
  if ((strcmp (filename, "-") == 0))
    {
      error (0, 0, gettext ("using %s to denote standard input does not work" " in file system mode"), quotearg_style (shell_escape_always_quoting_style, filename));
         ...
         char const *format2)
{
  ...
  if (0 <= fd)
    ...
    {
      error (0, (*__errno_location ()), gettext ("cannot stat %s"), quotearg_style (shell_escape_always_quoting_style, filename));
      return 0;
    }
  ...
  _Bool fail = print_it (format, fd, filename, print_stat, &pa);
  return ! fail;
}
            ...
            int fd, char const *filename, void const *data)
{
  ...
  switch (m)
    ...
    case 'n':
      ...
      break;
    case 'N':
      out_string (pformat, prefix_len, quotearg_style (get_quoting_style (((void*)0)), filename));
      if (((((statbuf->st_mode)) & 0170000) == (0120000)))
        {
          ...
          printf (" -> ");
          out_string (pformat, prefix_len, quotearg_style (get_quoting_style (((void*)0)), linkname));
          free (linkname);
        }
      break;
    case 'd':
    ...
    case 's':
      ...
      break;
    case 't':
      out_uint_x (pformat, prefix_len, gnu_dev_major (statbuf->st_rdev));
      break;
    case 'T':
      out_uint_x (pformat, prefix_len, gnu_dev_minor (statbuf->st_rdev));
      break;
    case 'B':
  ...
  while ((c = getopt_long (argc, argv, "c:fLt", long_options, ((void*)0))) != -1)
    {
      switch (c)
        ...
        case 0:
          switch (((cached_modes) [__xargmatch_internal ("--cached", optarg, cached_args, (void const *) (cached_modes), sizeof *(cached_modes), argmatch_die)]))
            {
              case cached_never:
                force_sync = 1;
                dont_sync = 0;
