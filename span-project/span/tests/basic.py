...
class SpanBasicTests(unittest.TestCase):
  ...
  def test_AACA_isCwdATestDirectory(self):
    ...
    self.assertEqual(status, 0, msg.FAIL_C_TEST_FILES_NOT_PRESENT)

    status, resultFiles = \
      subp.getstatusoutput("ls spanTest[0-9][0-9][0-9].c.results.py")
    self.assertEqual(status, 0, msg.FAIL_C_RESULT_FILES_NOT_PRESENT)
  ...
  def test_AADA_isClangInPath(self):
    ...
    self.assertEqual(status, 1, msg.FAIL_NO_CLANG_IN_PATH)
  ...
  def test_AAEA_isC2spanirPossible(self):
    ...
    self.assertEqual(status, 0, msg.FAIL_NO_C2SPANIR_SUPPORT)
