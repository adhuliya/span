...
const (
	...
	K_EK_DATA_TYPE       K_EK = 11 // A data type, like int, float, function sig, record etc.
	K_EK_FUNC            K_EK = 12 // A function with/without definition.
	K_EK_FUNC_VARGS      K_EK = 13 // A function with var args like printf().
	K_EK_CLASS           K_EK = 14 // A class type; for future use.
	K_EK_LABEL           K_EK = 15 // In if-then-else statements.
...
const (
	K_IK_INOP            K_IK = 0  // No-operation
	K_IK_IBARRIER        K_IK = 1  // Disconnects two program points.
	K_IK_IASGN_SIMPLE    K_IK = 2  // Simple assignment.
	K_IK_IASGN_BOP       K_IK = 3  // Binary operation assignment.
	K_IK_IASGN_UOP       K_IK = 4  // Unary operation assignment.
	K_IK_IASGN_RHS_DEREF K_IK = 5  // RHS dereference assignment.
	K_IK_IASGN_LHS_DEREF K_IK = 6  // LHS dereference assignment.
	K_IK_IASGN_RHS_ARRAY K_IK = 7  // RHS array assignment.
	K_IK_IASGN_LHS_ARRAY K_IK = 8  // LHS array assignment.
	K_IK_IASGN_CALL      K_IK = 9  // Call assignment.
	K_IK_IASGN_PHI       K_IK = 10 // Phi assignment.
	K_IK_ICALL           K_IK = 11 // Call instruction
	K_IK_IGOTO           K_IK = 12 // Goto instruction
	K_IK_ICOND           K_IK = 13 // IF-THEN-ELSE instruction
	K_IK_ILABEL          K_IK = 14 // Label instruction
	K_IK_IRETURN         K_IK = 15 // Return instruction.
)
...
const (
	...
	K_XK_NOT               K_XK = 15 // A unary logical NOT expression
	...
	K_XK_CALL_1            K_XK = 25 // A call with exactly one argument
	K_XK_CAST              K_XK = 26 // A cast expression
	K_XK_EQ                K_XK = 27 // Equality comparison.
	K_XK_NE                K_XK = 28 // Inequality comparison.
	K_XK_LT                K_XK = 29 // Less than comparison.
	K_XK_GE                K_XK = 30 // Greater than or equal to comparison.
...
const (
	...
	K_VK_VALUE_OTHER   K_VK = 31 // Other value type
)
...
type BitDataType struct {
	...
	Bits       *uint32 `protobuf:"varint,5,opt,name=bits,proto3,oneof" json:"bits,omitempty"`   // Number of bits in the type (for bit fields)
	Size       *uint32 `protobuf:"varint,6,opt,name=size,proto3,oneof" json:"size,omitempty"`   // The logical size of the type (e.g. length of an array)
	Align      *uint32 `protobuf:"varint,7,opt,name=align,proto3,oneof" json:"align,omitempty"` // Number of bytes to align to
	// Used for,
	//  1. Pointee of a pointer.
	//  2. Element of an array.
	//  3. Return type of a function.
	SubType *BitDataType `protobuf:"bytes,8,opt,name=subType,proto3,oneof" json:"subType,omitempty"`
	// Used to name the record or other special types
	Str *string `protobuf:"bytes,9,opt,name=str,proto3,oneof" json:"str,omitempty"`
...
func (x *BitDataType) GetBits() uint32 {
	if x != nil && x.Bits != nil {
		return *x.Bits
	}
	return 0
...
func (x *BitDataType) GetSize() uint32 {
	if x != nil && x.Size != nil {
		return *x.Size
	}
	return 0
...
func (x *BitDataType) GetStr() string {
	if x != nil && x.Str != nil {
		return *x.Str
	}
	return ""
...
type BitEntityInfo struct {
	...
	Dt    *BitDataType           `protobuf:"bytes,2,opt,name=dt,proto3,oneof" json:"dt,omitempty"`
	...
	Str           *string `protobuf:"bytes,4,opt,name=str,proto3,oneof" json:"str,omitempty"`  // for a string literal (for LIT_STR)
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *BitEntityInfo) GetNum() uint64 {
	if x != nil && x.Num != nil {
		return *x.Num
	}
	return 0
...
func (x *BitEntityInfo) GetStr() string {
	if x != nil && x.Str != nil {
		return *x.Str
	}
	return ""
...
type BitTU struct {
	...
	Path          *string                   `protobuf:"bytes,2,opt,name=path,proto3,oneof" json:"path,omitempty"`                                                                             // Path to the source file TU was generated from
	Entities      map[string]*BitEntity     `protobuf:"bytes,3,rep,name=entities,proto3" json:"entities,omitempty" protobuf_key:"bytes,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"` // name to entity id mapping
	EntityInfo    map[uint64]*BitEntityInfo `protobuf:"bytes,4,rep,name=entityInfo,proto3" json:"entityInfo,omitempty" protobuf_key:"varint,1,opt,name=key" protobuf_val:"bytes,2,opt,name=value"`
	Functions     []*BitFunc                `protobuf:"bytes,5,rep,name=functions,proto3" json:"functions,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
const file_span_pkg_spir_spir_proto_rawDesc = "" +
	...
	"\x03col\x18\x02 \x01(\rR\x03col\"\xb3\x03\n" +
	...
	"attributes\x88\x01\x01\x12\x17\n" +
	"\x04bits\x18\x05 \x01(\rH\x03R\x04bits\x88\x01\x01\x12\x17\n" +
	"\x04size\x18\x06 \x01(\rH\x04R\x04size\x88\x01\x01\x12\x19\n" +
	"\x05align\x18\a \x01(\rH\x05R\x05align\x88\x01\x01\x120\n" +
	"\asubType\x18\b \x01(\v2\x11.spir.BitDataTypeH\x06R\asubType\x88\x01\x01\x12\x15\n" +
	"\x03str\x18\t \x01(\tH\aR\x03str\x88\x01\x01\x12\x14\n" +
	"\x05names\x18\n" +
	" \x03(\tR\x05names\x12'\n" +
	"\x05types\x18\v \x03(\v2\x11.spir.BitDataTypeR\x05typesB\t\n" +
	"\a_typeIdB\b\n" +
	"\x06_qtypeB\r\n" +
	"\v_attributesB\a\n" +
	"\x05_bitsB\a\n" +
	...
	"\x04kind\x18\x01 \x01(\x0e2\n" +
	".spir.K_EKR\x04kind\x12&\n" +
	"\x02dt\x18\x02 \x01(\v2\x11.spir.BitDataTypeH\x00R\x02dt\x88\x01\x01\x12\x15\n" +
	"\x03num\x18\x03 \x01(\x04H\x01R\x03num\x88\x01\x01\x12\x15\n" +
	"\x03str\x18\x04 \x01(\tH\x02R\x03str\x88\x01\x01B\x05\n" +
	"\x03_dtB\x06\n" +
	"\x04_numB\x06\n" +
	"\x04_str\"K\n" +
	"\tBitEntity\x12\x0e\n" +
	...
	"\x13_calling_convention\"\x80\x03\n" +
	"\x05BitTU\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x12\x17\n" +
	"\x04path\x18\x02 \x01(\tH\x00R\x04path\x88\x01\x01\x125\n" +
	"\bentities\x18\x03 \x03(\v2\x19.spir.BitTU.EntitiesEntryR\bentities\x12;\n" +
	"\n" +
	"entityInfo\x18\x04 \x03(\v2\x1b.spir.BitTU.EntityInfoEntryR\n" +
	"entityInfo\x12+\n" +
	"\tfunctions\x18\x05 \x03(\v2\r.spir.BitFuncR\tfunctions\x1aL\n" +
	...
	"\x05value\x18\x02 \x01(\v2\x13.spir.BitEntityInfoR\x05value:\x028\x01B\a\n" +
	"\x05_path*\x8c\x02\n" +
	"\x04K_EK\x12\b\n" +
	...
	"\x05CLASS\x10\x0e\x12\t\n" +
	"\x05LABEL\x10\x0f\x12\b\n" +
	...
	"\x04CALL\x10\x17\x12\n" +
	"\n" +
	"\x06CALL_0\x10\x18\x12\n" +
	"\n" +
	"\x06CALL_1\x10\x19\x12\b\n" +
	"\x04CAST\x10\x1a\x12\x06\n" +
	"\x02EQ\x10\x1b\x12\x06\n" +
	"\x02NE\x10\x1c\x12\x06\n" +
	"\x02LT\x10\x1d\x12\x06\n" +
	"\x02GE\x10\x1e\x12\n" +
	"\n" +
	"\x06XOTHER\x10\x1f*\x83\x04\n" +
	"\x04K_VK\x12\b\n" +
	...
	"\n" +
	"\x06STRUCT\x10\x1e\x12\x0f\n" +
	"\vVALUE_OTHER\x10\x1f\x1a\x02\x10\x01*\xf6\x01\n" +
	"\x04K_QK\x12\t\n" +
...
var file_span_pkg_spir_spir_proto_depIdxs = []int32{
	...
	0,  // 3: spir.BitEntityInfo.kind:type_name -> spir.K_EK
	6,  // 4: spir.BitEntityInfo.dt:type_name -> spir.BitDataType
	5,  // 5: spir.BitEntity.loc:type_name -> spir.BitSrcLoc
	2,  // 6: spir.BitExpr.kind:type_name -> spir.K_XK
	8,  // 7: spir.BitExpr.opr1:type_name -> spir.BitEntity
	8,  // 8: spir.BitExpr.opr2:type_name -> spir.BitEntity
	5,  // 9: spir.BitExpr.loc:type_name -> spir.BitSrcLoc
	8,  // 10: spir.BitExpr.exprs:type_name -> spir.BitEntity
	1,  // 11: spir.BitInsn.kind:type_name -> spir.K_IK
	9,  // 12: spir.BitInsn.expr1:type_name -> spir.BitExpr
	9,  // 13: spir.BitInsn.expr2:type_name -> spir.BitExpr
	10, // 14: spir.BitFunc.insns:type_name -> spir.BitInsn
	13, // 15: spir.BitTU.entities:type_name -> spir.BitTU.EntitiesEntry
	14, // 16: spir.BitTU.entityInfo:type_name -> spir.BitTU.EntityInfoEntry
	11, // 17: spir.BitTU.functions:type_name -> spir.BitFunc
	8,  // 18: spir.BitTU.EntitiesEntry.value:type_name -> spir.BitEntity
	7,  // 19: spir.BitTU.EntityInfoEntry.value:type_name -> spir.BitEntityInfo
