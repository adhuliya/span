...
public:
  ...
  bool globalInitsConverted; // True if global inits have been converted to span
                             // statements
  ...
  SlangTranslationUnit()
      : uniqueId{0}, fileName{}, globalInits{}, globalInitsConverted{false},
        currFunc{nullptr}, recordId{0}, varMap{}, varCountMap{}, funcMap{},
  ...
  void dumpSlangIr() {
    ...
    if (this->fileName.size()) {
      ...
      slang::Util::writeToFile(fileName, ss.str());
    } else {
...
public:
  ...
  SpirGenerator(ASTContext *ctx) : Ctx(ctx) {
    stu.labelCount = 0;
    stu.uniqueId = 0;
    stu.globalInitsConverted = false;
  ...
  void handleFunctionDecl(FunctionDecl *D) {
    ...
    if (stu.fileName.size() == 0) {
      stu.fileName = Ctx->getSourceManager().getFilename(D->getBeginLoc()).str();
    }
  ...
  // This function
  void handleGlobalInitializations(FunctionDecl *funcDecl) {
    // All global initializations are stored in a global dummy function
    // as a sequence of statements.
    // This function should be visited only once.
